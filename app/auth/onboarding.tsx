...
import { completeOnboarding, type FamilySetupChoice, type PrimaryRole } from "@/lib/auth";
...
export default function OnboardingScreen() {
  ...
  const [primaryRole, setPrimaryRole] = useState<PrimaryRole>("parent_guardian");
  const [familySetup, setFamilySetup] = useState<FamilySetupChoice>("create");
  const [familyName, setFamilyName] = useState("");
  const [enableCaregiverWorkProfile, setEnableCaregiverWorkProfile] = useState(false);
    ...
    <View style={styles.container}>
      ...
      <View style={styles.row}>
        <Button
          ...
          title={familySetup === "create" ? "Selected: Create family" : "Create family"}
        />
        <Button
          onPress={() => setFamilySetup("join")}
          title={familySetup === "join" ? "Selected: Join family" : "Join family"}
        />
      </View>
      ...
      {familySetup === "create" ? (
        ...
        <Text style={styles.helpText}>Joining a family can be completed after onboarding with an invite.</Text>
      )}

      {primaryRole === "caregiver" ? (
        <View style={styles.switchRow}>
          <Text style={styles.switchLabel}>Enable caregiver work profile</Text>
          <Switch onValueChange={setEnableCaregiverWorkProfile} value={enableCaregiverWorkProfile} />
        </View>
      ) : null}

      {errorMessage ? <Text style={styles.error}>{errorMessage}</Text> : null}
      {isSubmitting ? <ActivityIndicator /> : <Button onPress={handleSubmit} title="Finish onboarding" />}
    </View>
  );
}
