...
import { calendarApprovalStatusLabels, calendarEventTypeLabels, calendarSourceLabels } from "@/constants/calendar";
import { privacyLevelLabels } from "@/constants/permissions";
import { spacing } from "@/constants/spacing";
import { colors } from "@/constants/theme";
import { canViewerSeeEventDetails, getCalendarSmartRoutePlaceholder, getMockCalendarEvents, getSafeEventPreview } from "@/lib/calendar";
...
export default function CalendarEventDetailScreen() {
  ...
  if (!event) {
    return (
      <View style={styles.root}>
        <AppScreen>
          <AppHeader
            action={<QuickActionButton label="Back" onPress={() => openRoute("/tabs/calendar")} toneColor={colors.brand.primary} />}
            eyebrow="Calendar"
  ...
  const canSeeDetails = canViewerSeeEventDetails(event, ["view_calendar", "view_emergency_info"]);
  ...
  return (
    <View style={styles.root}>
      <AppScreen>
        <AppHeader
          action={<QuickActionButton label="Calendar" onPress={() => openRoute("/tabs/calendar")} toneColor={colors.brand.primary} />}
        ...
        <WidgetCard
          ...
          action={<StatusPill label={privacyLevelLabels[event.privacyLevel]} tone={event.privacyLevel === "private" ? "warning" : "default"} />}
          subtitle="Sensitive event details are hidden from cards and previews unless the viewer has permission."
          title="Privacy"
        >
          <Text style={styles.detailText}>{canSeeDetails ? event.description ?? "No event details added." : getSafeEventPreview(event)}</Text>
        </WidgetCard>

        <WidgetCard
          accentColor={colors.status.ai}
          action={<StatusPill label={calendarApprovalStatusLabels[event.approvalStatus]} tone={event.requiresApproval ? "warning" : "success"} />}
        ...
        >
          <View style={styles.badges}>
            ...
            {event.circleName ? <StatusPill label={event.circleName} tone="success" /> : null}
            {event.careProfileName ? <StatusPill label={event.careProfileName} tone="ai" /> : null}
          </View>
