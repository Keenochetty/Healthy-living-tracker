...
import type { CaregiverAssignment, CaregiverAssignmentPermissionKey } from "@/types/caregiver-assignments";
...
export default function CareProfileDetailScreen() {
  ...
  const careProfile = circles.flatMap((circle) => circle.careProfiles).find((profile) => profile.id === careProfileId) ?? null;
  const circle = careProfile ? circles.find((item) => item.id === careProfile.circleId) ?? null : null;
  const privacyLevel = careProfile ? getPrivacyLevelForCareProfile(careProfile) : "private";
  const permissionSummary =
    careProfile && circle
      ? buildPermissionSummary({
          ageAccessStage: careProfile.ageAccessStage,
          currentUserRole: circle.currentUserRole,
          isAssignedCaregiver: careProfile.caregiverAssignmentStatus !== "not_assigned",
          isSelfManagedAdult: careProfile.ageAccessStage === "adult_controlled" && careProfile.profileType === "adult_member",
          privacyLevel
        })
      : null;
  const [assignments, setAssignments] = useState<CaregiverAssignment[]>(() => getMockCaregiverAssignments(typeof careProfileId === "string" ? careProfileId : undefined));

  function handleAction(profile: CareProfile, action: string) {
    setPlaceholderMessage(`${action} for ${profile.displayName} will connect when care records, notes, and calendar workflows are built.`);
  }
  ...
  function handleAssignmentPermissionChange(assignment: CaregiverAssignment, permission: CaregiverAssignmentPermissionKey, enabled: boolean) {
    setAssignments((current) =>
      current.map((item) =>
        item.id === assignment.id ? updateCaregiverAssignmentPermission(item, permission, enabled) : item
      )
    );
    setPlaceholderMessage("Caregiver permission change captured locally. Audit log entry placeholder created.");
  }
  ...
  function handlePermissionChange(category: PermissionCategory, enabled: boolean) {
    const profileId = typeof careProfileId === "string" ? careProfileId : undefined;
    setAuditEvents((current) => [createPermissionAuditPlaceholder(category, enabled, "local-user", profileId), ...current].slice(0, 4));
    setPlaceholderMessage(`${permissionCategoryLabels[category]} changes are captured locally. Audit log entry placeholder created.`);
  }
    ...
    <View style={styles.root}>
      <AppScreen>
        ...
        <WidgetCard
          ...
          action={<StatusPill label={careProfilePrivacyLabels[careProfile.privacyStatus]} tone={careProfile.privacyStatus === "adult_private" ? "success" : "default"} />}
        ...
        >
          <Text style={styles.muted}>
            Caregiver assignment, detailed records, and granular permissions are intentionally placeholders in this phase.
          </Text>
          <Text style={styles.safePreview}>
            {getSafePreview(careProfile.notes ?? "Sensitive notes stay out of preview cards.", careProfile.privacyStatus === "adult_private")}
          </Text>
        </WidgetCard>

        {permissionSummary ? (
          <PrivacySummaryCard summary={permissionSummary} title="Permission summary" />
        ) : null}
        ...
        {permissionSummary ? (
          ...
          >
            <PermissionToggleGroup
              grants={toPermissionGrants(permissionSummary.defaultPermissions, ["view_emergency_info"])}
        ...
        <AdultConsentCard
          consentRequired={shouldRequireConsent(careProfile.profileType, careProfile.ageAccessStage)}
          onRequestConsent={() => handlePermissionChange("manage_privacy", true)}
        />
        ...
        <CaregiverPermissionCard
          assignedOnly={careProfile.caregiverAssignmentStatus !== "not_assigned"}
            ...
            <View style={styles.auditList}>
              {auditEvents.map((event) => (
                <Text key={`${event.createdAt}-${event.category}`} style={styles.muted}>
                  {permissionCategoryLabels[event.category]} {event.enabled ? "enabled" : "disabled"} locally.
                </Text>
              ))}
            </View>
          ) : (
            <Text style={styles.muted}>No permission changes recorded in this local session.</Text>
          )}
        </WidgetCard>
        ...
        <WidgetCard
          ...
          action={
            <QuickActionButton
              ...
              onPress={() => openRoute(`/caregiver/assign?careProfileId=${careProfile.id}`)}
          ...
          <View style={styles.assignmentList}>
            {assignments.map((assignment) => (
              <CaregiverAssignmentCard
                assignment={{ ...assignment, careProfileName: careProfile.displayName, careProfileType: careProfile.profileType, circleName: circle?.name ?? assignment.circleName }}
                key={assignment.id}
                onCall={handleAssignmentCall}
                onEmail={handleAssignmentEmail}
                onManagePermissions={() => setPlaceholderMessage("Manage permissions is represented by grouped toggles on the assignment card for now.")}
