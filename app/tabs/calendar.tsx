...
import { AppHeader, AppIcon, AppScreen, QuickActionButton, StatusPill, StatusSurface, WidgetCard } from "@/components/ui";
import { calendarColorLabels, calendarColorSoftTokens, calendarColorTokens } from "@/constants/calendar";
...
import type { CalendarEvent, CalendarFilter, CalendarViewMode } from "@/types/calendar";
...
export default function CalendarScreen() {
  ...
  const dayEvents = useMemo(() => listEventsForDay(filteredEvents, selectedDate), [filteredEvents, selectedDate]);
  const eventCountByDay = useMemo(() => getEventCountByDay(filteredEvents), [filteredEvents]);
  const eventsByDay = useMemo(() => groupEventsByDay(filteredEvents), [filteredEvents]);
  const careProfiles = circles.flatMap((circle) => circle.careProfiles.map((careProfile) => ({ ...careProfile, circleName: circle.name })));
    ...
    <View style={styles.root}>
      <AppScreen>
        ...
        <WidgetCard
          ...
          action={<StatusPill label={selectedFamily?.name ?? "All circles"} tone="success" />}
        ...
        <View style={styles.filterGroup}>
          {circles.map((circle) => (
            <QuickActionButton
              ...
              toneColor={filter.circleId === circle.id ? colors.brand.primary : colors.text.muted}
            />
          ))}
          {careProfiles.map((careProfile) => (
            <QuickActionButton
              ...
              onPress={() => setFilter({ key: "care_profiles", careProfileId: careProfile.id })}
              toneColor={filter.careProfileId === careProfile.id ? colors.status.ai : colors.text.muted}
            />
          ))}
        </View>
        ...
        <View style={styles.twoColumnGrid}>
          <View style={cardColumnStyle}>
            <WidgetCard
              ...
              title="Upcoming"
            >
              <UpcomingEventsCarousel events={filteredEvents} onOpenEvent={handleOpenEvent} />
            </WidgetCard>
          </View>
          ...
          <View style={cardColumnStyle}>
            <WidgetCard
              ...
              title="Selected day"
            >
              <DayTimeline date={selectedDate} events={dayEvents} onOpenEvent={handleOpenEvent} />
            </WidgetCard>
          </View>
        </View>
        ...
        <WidgetCard
          accentColor={colors.status.success}
          action={<QuickActionButton label={expandedPanels.colors ? "Hide" : "Show"} onPress={() => togglePanel("colors")} toneColor={colors.status.success} />}
          subtitle="Event colour meanings match the calendar foundation rules."
          title="Event colours"
        >
          {expandedPanels.colors ? (
            <View style={styles.legendGrid}>
              {(Object.keys(calendarColorLabels) as Array<keyof typeof calendarColorLabels>).map((colorKey) => (
                <View key={colorKey} style={[styles.legendItem, { backgroundColor: calendarColorSoftTokens[colorKey] }]}>
                  <View style={[styles.legendDot, { backgroundColor: calendarColorTokens[colorKey] }]} />
                  <Text style={styles.legendText}>{calendarColorLabels[colorKey]}</Text>
                </View>
              ))}
            </View>
          ) : (
            <Text style={styles.ruleText}>Tap Show to expand the colour guide.</Text>
          )}
        </WidgetCard>

        <WidgetCard
          accentColor={colors.status.system}
          action={<QuickActionButton label={expandedPanels.privacy ? "Hide" : "Show"} onPress={() => togglePanel("privacy")} toneColor={colors.status.system} />}
        ...
        >
          {expandedPanels.privacy ? (
            <View style={styles.ruleList}>
              <Text style={styles.ruleText}>Private events remain hidden from summaries unless explicitly allowed.</Text>
              <Text style={styles.ruleText}>Caregiver events require approval when they affect a care profile.</Text>
              <Text style={styles.ruleText}>Women's health events default to {privacyLevelLabels.private.toLowerCase()}.</Text>
            </View>
          ) : (
            <Text style={styles.ruleText}>Tap Show to view privacy and source notes.</Text>
          )}
        </WidgetCard>

        <Modal transparent visible={Boolean(dayMenuDate)} animationType="fade">
          <Pressable style={styles.modalBackdrop} onPress={() => setDayMenuDate(null)}>
            <View style={styles.dayMenuCard}>
              <Text style={styles.modalTitle}>Day actions</Text>
              <Text style={styles.modalText}>{dayMenuDate?.toDateString()}</Text>
              <QuickActionButton label="Add event" onPress={() => dayMenuDate && openAddEventForDate(dayMenuDate)} toneColor={colors.brand.primary} />
            </View>
          </Pressable>
        </Modal>

        <Modal transparent visible={Boolean(selectedEvent)} animationType="fade">
              ...
              {selectedEvent ? (
                <View style={styles.eventModalContent}>
                  <View style={styles.modalHeaderRow}>
                    ...
                    <StatusPill label={privacyLevelLabels[selectedEvent.privacyLevel]} tone={selectedEvent.privacyLevel === "private" ? "warning" : "default"} />
                  </View>
                  <Text style={styles.modalText}>{getSafeEventPreview(selectedEvent)}</Text>
                  <View style={styles.legendGrid}>
                    {selectedEvent.circleName ? <StatusPill label={selectedEvent.circleName} tone="success" /> : null}
                    {selectedEvent.careProfileName ? <StatusPill label={selectedEvent.careProfileName} tone="ai" /> : null}
                    <StatusPill label={selectedEvent.approvalStatus} tone={selectedEvent.requiresApproval ? "warning" : "default"} />
                  </View>
                  <TextInput
                    multiline
                    ...
                    value={eventNote}
                  />
                  <QuickActionButton label="Add note" onPress={addEventNote} toneColor={colors.brand.primary} />
