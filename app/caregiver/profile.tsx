...
const placeholderCaregiver: CaregiverProfile = {
  ...
  experienceSummary:
    "Experienced caregiver focused on calm routines, safe handoffs, basic medication reminders, and family-approved care notes.",
  firstName: "Maya",
...
export default function CaregiverProfileScreen() {
  ...
  const [assignments, setAssignments] = useState<CaregiverAssignment[]>(() =>
    getMockCaregiverAssignments("placeholder-dad-care-profile"),
  );
  const assignedCareProfiles: AssignedCareProfileSummary[] = assignments.map(
    (assignment) => ({
      circleName: assignment.circleName ?? "Care Circle",
      displayName: assignment.careProfileName ?? "Assigned care profile",
      grantedPermissionCount: Object.values(assignment.permissions).filter(
        Boolean,
      ).length,
  ...
  function handleAssignmentPermissionChange(
    ...
    setPlaceholderMessage(
      "Permission change captured locally. Audit log placeholder created.",
    );
  }
      ...
      <AppScreen>
        <AppHeader
          action={
            <QuickActionButton
              label="Edit"
              onPress={() => openRoute("/caregiver/edit-profile")}
        ...
        <CaregiverCard
          ...
          onOpenAssignedProfile={(profile) =>
            openRoute(`/care-profiles/${profile.id}`)
          }
          onOpen={() => undefined}
          onShare={() =>
            setPlaceholderMessage(
              "Sharing caregiver cards is a placeholder for now.",
            )
          }
        />
        ...
        <WidgetCard
          ...
          action={
            <StatusPill
              label={`${assignedCareProfiles.length} profiles`}
              tone="success"
            />
          }
              ...
              <View key={profile.id} style={styles.assignedRow}>
                ...
                <View style={styles.assignedCopy}>
                  ...
                  <Text style={styles.muted}>
                    {profile.circleName} - assigned access only
                  </Text>
                </View>
                <QuickActionButton
                  label="Open"
                  onPress={() => openRoute(`/care-profiles/${profile.id}`)}
          ...
          <View style={styles.assignedList}>
            {assignments.map((assignment) => (
              <CaregiverAssignmentCard
                ...
                onManagePermissions={() =>
                  setPlaceholderMessage(
                    "Manage permissions is a placeholder on the caregiver profile. Admin controls live on assignment screens.",
                  )
                }
                onOpenProfile={(item) =>
                  openRoute(`/care-profiles/${item.careProfileId}`)
                }
                ...
                onRevoke={(item) => {
                  ...
                  setPlaceholderMessage(
                    "Revoke access captured locally. Backend revocation and audit logs come later.",
                  );
                }}
              />
            ))}
        ...
        >
          <Text style={styles.muted}>
            References are represented as a placeholder and are not verified in
