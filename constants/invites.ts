...
export const inviteMethodDescriptions = {
  ...
  qr_placeholder:
    "Reserve space for QR invites without generating a real QR code yet.",
  share_link:
    "Generate a placeholder invite link that can be copied or shared.",
  sms_whatsapp_placeholder:
    "Prepare SMS or WhatsApp sharing without real native share integration.",
} as const satisfies Record<InviteMethod, string>;
...
export const invitePermissionPresetDescriptions = {
  adult_family_member:
    "Can see shared calendar and shared updates. Private health data is not shared by default.",
  caregiver:
    "Assigned-only access will be configured after acceptance. No full family access.",
  custom: "Placeholder for manual permission toggles.",
  parent_guardian:
    "Can manage dependents, child emergency info, and child schedules.",
...
export const invitePermissionPresetDetails = {
  adult_family_member: [
    ...
    "View shared updates",
    "No private health access",
  ],
  caregiver: [
    "Assigned-only access",
    "No full family access",
    "Care profiles assigned later",
  ],
  custom: ["Manual toggles placeholder", "Admin reviews before acceptance"],
  parent_guardian: [
    "Manage dependents",
    "View child emergency info",
...
export const invitePermissionPresetDefaults = {
  ...
  InvitePermissionPreset,
  readonly CircleInvite["defaultPermissions"][number][]
>;

export function formatInvitePermissions(
  permissions: CircleInvite["defaultPermissions"],
) {
  return permissions
    .map((permission) => circlePermissionLabels[permission])
    .join(", ");
}
