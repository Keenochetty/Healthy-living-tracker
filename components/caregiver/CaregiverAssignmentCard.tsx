...
import {
  countGrantedCaregiverPermissions,
...
export function CaregiverAssignmentCard({
  ...
  const permissionCount = countGrantedCaregiverPermissions(
    assignment.permissions,
  );
  const permissionSummary = summarizeCaregiverPermissions(
    assignment.permissions,
  );
  ...
  return (
    <View style={styles.card}>
      <View style={styles.header}>
        ...
        <View style={styles.copy}>
          <View style={styles.titleRow}>
            ...
            <StatusPill
              label={caregiverAssignmentStatusLabels[assignment.status]}
          ...
          <Text style={styles.meta}>
            Assigned-only access. No full Family/Care Circle browsing.
          </Text>
          <View style={styles.badges}>
            {assignment.careProfileType ? (
              <StatusPill
                label={careProfileTypeLabels[assignment.careProfileType]}
                tone="ai"
              />
            ) : null}
            <StatusPill label={`${permissionCount} granted`} tone="success" />
            <StatusPill
              label={`${assignment.auditLog.length} audit placeholders`}
            />
            {assignment.permissions.canUseEmergencyButton ||
            assignment.permissions.canViewEmergencyContacts ? (
      ...
      <View style={styles.summaryBox}>
        ...
        <Text style={styles.meta}>
          Sensitive adult/private health information remains hidden unless
          explicitly granted.
        </Text>
      </View>
      ...
      {assignment.startDate || assignment.endDate ? (
        <View style={styles.dateRow}>
          {assignment.startDate ? (
            <StatusPill label={`Starts ${assignment.startDate}`} />
          ) : null}
          {assignment.endDate ? (
            <StatusPill label={`Ends ${assignment.endDate}`} tone="warning" />
          ) : null}
        </View>
      ) : null}
      ...
      {showPermissionToggles && onPermissionChange ? (
        <CaregiverAssignmentPermissions
          onChange={(permissionKey, enabled) =>
            onPermissionChange(assignment, permissionKey, enabled)
          }
      ...
      <View style={styles.actions}>
        ...
        <QuickActionButton
          icon={
            <AppIcon color={colors.status.emergency} name="lock" size={18} />
          }
          label="Revoke access"
