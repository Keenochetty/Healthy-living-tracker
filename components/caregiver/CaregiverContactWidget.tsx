...
export function CaregiverContactWidget({ caregiver, onShare, onStatus }: CaregiverContactWidgetProps) {
  async function handleCall() {
    try {
      await openPhoneCall(caregiver.cellNumber);
    } catch (error) {
      onStatus?.(error instanceof Error ? error.message : "Unable to start call.");
    }
  }

  async function handleEmail() {
    try {
      await openEmail(caregiver.email, `Caregiver profile: ${caregiver.firstName} ${caregiver.lastName}`);
    } catch (error) {
      onStatus?.(error instanceof Error ? error.message : "Unable to open email.");
    }
  }
  ...
  return (
    ...
    >
      <View style={styles.contactRows}>
        ...
        <View style={styles.contactRow}>
          <AppIcon color={colors.brand.primary} name="notifications" size={20} />
        ...
        <View style={styles.actions}>
          ...
          <QuickActionButton
            ...
            label="Share card"
            onPress={onShare ?? (() => onStatus?.("Sharing caregiver cards is a placeholder for now."))}
