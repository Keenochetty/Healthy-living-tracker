...
export default async function PregnancyPage() {
  ...
  const pregnancyMembers = data.members.filter((member) =>
    ["pregnant_mother", "postpartum_mother"].includes(member.profile_type),
  );
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Due date, pregnancy week, trimester, symptoms, blood pressure, scans,
          antenatal visits, and reports.
        </p>
      </div>
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Metric
          title="Symptom logs"
          value={
            data.healthLogs.filter((log) => log.category === "symptom").length
          }
        />
      ...
      <div className="grid gap-4 xl:grid-cols-[1fr_420px]">
        <ActivityList
          ...
          documents={data.documents.filter((d) =>
            ["scan_image", "doctor_report", "lab_result"].includes(d.category),
          )}
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Use health logs for symptoms, blood pressure, scans, and antenatal
              notes.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <HealthLogForm
              members={
                pregnancyMembers.length ? pregnancyMembers : data.members
              }
            />
          </CardContent>
        </Card>
