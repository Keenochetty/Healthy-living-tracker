...
const memberOptions = (members: FamilyMember[]) =>
  ...
  return (
    <form action={action} className="space-y-4">
      ...
      <Field
        label="Title"
        name="title"
        placeholder="Headache, blood pressure, mood check"
        required
      />
      ...
      <SelectField
        label="Privacy"
        name="privacy_level"
        options={privacyLevels.map((value) => ({
          value,
          label: privacyLevelLabels[value],
        }))}
...
export function CaregiverActivityForm({
  members,
}: {
  members: FamilyMember[];
}) {
  ...
  const childOptions = (children.length ? children : members).map((member) => ({
    value: member.id,
    label: member.name,
  }));
  ...
  return (
    <form action={action} className="space-y-4">
      ...
      <SelectField
        label="Quick action"
        name="activity_type"
        options={caregiverActivityTypes.map((value) => ({
          value,
          label: caregiverActivityLabels[value],
        }))}
      ...
      <SelectField
        label="Urgency"
        name="urgency"
        options={notificationUrgencies.map((value) => ({
          value,
          label: notificationUrgencyLabels[value],
        }))}
      ...
      <Field
        label="Title"
        name="title"
        placeholder="Lunch finished, mild temperature, nap started"
        required
      />
  ...
  return (
    <form action={action} className="space-y-4">
      ...
      <Field
        label="Type"
        name="reminder_type"
        placeholder="appointment, medicine, vaccination"
        required
      />
