...
export function InvitePermissionPresetCard({
  onPress,
  preset,
  selected = false,
}: InvitePermissionPresetCardProps) {
  const toneColor =
    preset === "caregiver"
      ? colors.status.warning
      : preset === "parent_guardian"
  ...
  return (
    ...
    >
      <View style={[styles.iconShell, selected && styles.selectedIcon]}>
        <AppIcon
          color={selected ? toneColor : colors.text.secondary}
          name={preset === "caregiver" ? "care" : "family"}
          size={22}
        />
      </View>
      ...
      <View style={styles.copy}>
        <View style={styles.titleRow}>
          <Text style={styles.title}>
            {invitePermissionPresetLabels[preset]}
          </Text>
          {selected ? <StatusPill label="Selected" tone="success" /> : null}
        </View>
        <Text style={styles.description}>
          {invitePermissionPresetDescriptions[preset]}
        </Text>
        <View style={styles.detailRow}>
          {invitePermissionPresetDetails[preset].map((detail) => (
            <StatusPill
              key={detail}
              label={detail}
              tone={preset === "caregiver" ? "warning" : "default"}
            />
          ))}
        </View>
      </View>
