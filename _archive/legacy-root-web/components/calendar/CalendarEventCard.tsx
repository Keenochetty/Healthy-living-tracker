  ...
  return (
    ...
    >
      <View
        style={[
          styles.colorRail,
          { backgroundColor: calendarColorTokens[event.colour] },
        ]}
      />
      <View style={styles.body}>
        <View style={styles.header}>
          <View
            style={[
              styles.iconShell,
              { backgroundColor: calendarColorSoftTokens[event.colour] },
            ]}
          >
            <AppIcon
              color={calendarColorTokens[event.colour]}
              name={event.icon}
              size={22}
            />
          </View>
          <View style={styles.titleGroup}>
            ...
            <Text style={styles.time}>
              {event.allDay
                ? "All day"
                : `${formatCalendarTime(event.startTime)}${event.endTime ? ` - ${formatCalendarTime(event.endTime)}` : ""}`}
            </Text>
          </View>
        ...
        <Text style={styles.description}>
          {sensitive
            ? event.safePreview
            : (event.description ?? event.safePreview)}
        </Text>

        <View style={styles.badges}>
          <StatusPill
            label={privacyLevelLabels[event.privacyLevel]}
            tone={sensitive ? "warning" : "default"}
          />
          <StatusPill
        ...
        <View style={styles.links}>
          {event.circleName ? (
            <Text style={styles.linkText}>Circle: {event.circleName}</Text>
          ) : null}
          {event.careProfileName ? (
            <Text style={styles.linkText}>
              Care profile: {event.careProfileName}
            </Text>
          ) : null}
          {event.profileName ? (
            <Text style={styles.linkText}>Profile: {event.profileName}</Text>
          ) : null}
          {event.location ? (
            <Text style={styles.linkText}>Location: {event.location}</Text>
          ) : null}
        </View>
        ...
        <QuickActionButton
          label="Open"
          onPress={() => onOpen?.(event)}
          toneColor={calendarColorTokens[event.colour]}
        />
      </View>
    </Pressable>
  );
