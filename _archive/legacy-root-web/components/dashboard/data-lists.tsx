...
export function FamilyCards({ members }: { members: FamilyMember[] }) {
  if (members.length === 0) {
    return (
      <EmptyState
        message="Create a family member profile to start tracking health data."
      ...
      {members.map((member) => (
        <Card key={member.id}>
          <CardHeader>
            ...
            <CardDescription>
              {member.relationship ?? member.profile_type.replaceAll("_", " ")}
            </CardDescription>
          </CardHeader>
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Appointments, medicine, vaccinations, and care tasks.
        </CardDescription>
      </CardHeader>
          ...
          >
            <div>
              ...
              <p className="text-sm text-slate-400">
                {reminder.family_members?.name ?? "Family"} -{" "}
...
export function ActivityList({
  ...
  const items = [
    ...healthLogs.map((log) => ({
      ...
      icon: HeartPulse,
      title: log.title,
      detail: `${trackingCategoryLabels[log.category as keyof typeof trackingCategoryLabels] ?? log.category} - ${log.family_members?.name ?? ""}`,
      date: log.logged_at,
    })),
    ...medicineLogs.map((log) => ({
      ...
      detail: `${log.dosage ?? "Dose"} - ${log.family_members?.name ?? ""}`,
      date: log.taken_at,
    })),
    ...temperatureLogs.map((log) => ({
      ...
      detail: `Temperature - ${log.family_members?.name ?? ""}`,
    ...
    ...doctorVisits.map((visit) => ({
      ...
      icon: Calendar,
      title: visit.reason,
      detail: `${visit.doctor_name ?? "Doctor visit"} - ${visit.family_members?.name ?? ""}`,
      date: visit.visit_at,
    })),
    ...documents.map((document) => ({
      ...
      detail: `${documentCategoryLabels[document.category as keyof typeof documentCategoryLabels] ?? document.category} - ${document.family_members?.name ?? ""}`,
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Upcoming doses from recent medicine logs.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-3">
        {alerts.length === 0 ? (
          <p className="text-sm text-slate-400">
            No upcoming medication alerts.
          </p>
        ) : null}
        {alerts.map((alert) => (
          <div
          ...
          >
            ...
            <p className="text-sm text-slate-400">
              {alert.family_members?.name ?? "Family member"} - next dose{" "}
              {formatDate(alert.next_dose_at!)}
            </p>
          </div>
        ))}
      </CardContent>
