...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: userData, error: userError } = await client.auth.getUser(userJwt);
    ...
    for (const [table, ownerColumn] of EXPORT_TABLES) {
      const result = await client.from(table).select("*").eq(ownerColumn, userData.user.id).limit(5000);
    ...
    return respond({ error: error instanceof Error ? error.message : "Data export failed.", ok: false }, 500);
  }
});
