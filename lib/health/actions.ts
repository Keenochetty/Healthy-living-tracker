...
function db(
  client: Awaited<ReturnType<typeof ensureDefaultFamily>>["supabase"],
) {
...
export async function createFamilyMemberAction(
  ...
  if (!parsed.success)
    return fail(
      parsed.error.issues[0]?.message ?? "Check the family member details.",
    );
...
export async function createHealthLogAction(
  _state: ActionState,
  formData: FormData,
): Promise<ActionState> {
  ...
  if (!parsed.success)
    return fail(parsed.error.issues[0]?.message ?? "Check the log details.");
  ...
  const { error } = await db(supabase)
    ...
    .insert({
      ...
      logged_at:
        nullableText(formData.get("logged_at")) ?? new Date().toISOString(),
      payload: {
...
export async function createCaregiverActivityAction(
  _state: ActionState,
  formData: FormData,
): Promise<ActionState> {
  const parsed = caregiverActivitySchema.safeParse(
    Object.fromEntries(formData),
  );
  if (!parsed.success)
    return fail(
      parsed.error.issues[0]?.message ??
        "Check the caregiver activity details.",
    );
  ...
  if (shareWithParents) {
    ...
    const notifications = (recipients ?? []).map(
      (recipient: { user_id: string }) => ({
        ...
        title: parsed.data.title,
        message:
          nullableText(formData.get("notes")) ?? "Caregiver activity update.",
      }),
    );
...
export async function createMedicineLogAction(
  _state: ActionState,
  formData: FormData,
): Promise<ActionState> {
  ...
  if (!parsed.success)
    return fail(
      parsed.error.issues[0]?.message ?? "Check the medicine details.",
    );
  ...
  const { error } = await db(supabase)
    ...
    .insert({
      ...
      taken_at:
        nullableText(formData.get("taken_at")) ?? new Date().toISOString(),
...
export async function createTemperatureLogAction(
  ...
  if (!parsed.success)
    return fail(
      parsed.error.issues[0]?.message ?? "Check the temperature details.",
    );
  ...
  const { error } = await db(supabase)
    ...
    .insert({
      ...
      measured_at:
        nullableText(formData.get("measured_at")) ?? new Date().toISOString(),
...
export async function createDoctorVisitAction(
  _state: ActionState,
  formData: FormData,
): Promise<ActionState> {
  const parsed = doctorVisitSchema.safeParse(Object.fromEntries(formData));
  if (!parsed.success)
    return fail(parsed.error.issues[0]?.message ?? "Check the visit details.");
...
export async function uploadDocumentAction(
  ...
  if (!z.string().uuid().safeParse(familyMemberId).success)
    return fail("Select a family member.");
  if (!z.enum(documentCategories).safeParse(category).success)
    return fail("Select a document category.");
...
export async function askAiHealthAction(
  _state: ActionState,
  formData: FormData,
): Promise<ActionState> {
  const parsed = aiPromptSchema.safeParse(Object.fromEntries(formData));
  if (!parsed.success)
    return fail(parsed.error.issues[0]?.message ?? "Check your question.");

  if (!process.env.OPENAI_API_KEY) {
    return fail(
      "OPENAI_API_KEY is not configured. ChatGPT Free cannot be used as an app API backend.",
    );
  }
  ...
  if (!chatId) {
    const { data: chat, error } = await anySupabase
      ...
      .insert({
        ...
        category_filters: parsed.data.category_filter
          ? [parsed.data.category_filter]
          : [],
      })
      .select("id")
      .single();
  ...
  await anySupabase.from("ai_messages").insert({
    ...
    attachment_document_id: nullableText(
      formData.get("attachment_document_id"),
    ),
  });
