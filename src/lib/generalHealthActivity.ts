...
export function createGeneralHealthActivity(
  draft: GeneralHealthLogDraft,
  profileId: string,
  profileName: string,
): GeneralHealthActivityEntry {
  ...
  const base = {
    ...
    id: `local-health-activity-${draft.recordedAt}-${localActivitySequence}`,
    profileId,
    profileName,
  };
  ...
  if (draft.logType === "vitals") {
    const readings = [
      ...
      draft.systolic && draft.diastolic
        ? `Blood pressure ${draft.systolic} / ${draft.diastolic} mmHg`
        : "",
  ...
  if (draft.logType === "weight") {
    return {
      ...base,
      details: { notes: draft.notes, weight: draft.weight },
      summary: `${draft.weight} kg`,
      title: "Weight added",
      type: "weight",
    };
  }
  if (draft.logType === "temperature") {
    return {
      ...
      summary: `${draft.temperature} °C · ${draft.method}`,
  ...
  return {
    ...
    summary: draft.noteTitle || draft.details || "Health note",
