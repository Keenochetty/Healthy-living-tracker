...
import type { AppLockSettings, SecurityActivity, TotpEnrollment } from "@/types/security";
...
export async function reauthenticate(password: string) {
  ...
  if (!email) throw new Error("Sign in with email before completing this security action.");
  const { error } = await supabase.auth.signInWithPassword({ email, password });
  if (error) throw new Error("Re-authentication failed. Check your current password.");
}
...
export async function startTotpEnrollment(): Promise<TotpEnrollment> {
  const { data, error } = await supabase.auth.mfa.enroll({ factorType: "totp", friendlyName: "HealthSync authenticator" });
...
export async function disableTotp(factorId: string, currentPassword: string, code: string) {
  await reauthenticate(currentPassword);
  const { error: verifyError } = await supabase.auth.mfa.challengeAndVerify({ factorId, code: code.trim() });
  if (verifyError) throw new Error("A fresh authenticator code is required to disable two-step verification.");
...
export async function getBiometricAvailability() {
  ...
  return { available: hasHardware && isEnrolled, hasHardware, isEnrolled, types };
}
...
export async function getAppLockSettings(): Promise<AppLockSettings> {
  try {
    ...
    return value ? JSON.parse(value) as AppLockSettings : { enabled: false, timing: "immediately" };
  } catch {
...
export async function saveAppLockSettings(settings: AppLockSettings) {
  ...
  await addSecurityActivity(settings.enabled ? "App lock enabled" : "App lock disabled");
  return settings;
}
...
export async function addSecurityActivity(title: string) {
  ...
  const next = [{ createdAt: new Date().toISOString(), id: `security-${Date.now()}`, title }, ...current].slice(0, 20);
