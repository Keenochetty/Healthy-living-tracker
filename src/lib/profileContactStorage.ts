...
export async function getProfileContactDetails(): Promise<ProfileContactDetails> {
  ...
  try {
    ...
    const local = stored
      ? {
          ...fallback,
          ...(JSON.parse(stored) as Partial<ProfileContactDetails>),
        }
      : fallback;
    ...
    const [{ data: profile }, { data: contacts }] = await Promise.all([
      supabase
        .from("profiles")
        .select(
          "avatar_url,date_of_birth,display_name,email,full_name,phone,preferred_contact_method",
        )
    ...
    return {
      ...
      emergencyContacts:
        contacts?.map(mapEmergencyContact) ?? local.emergencyContacts,
      fullName: profile?.full_name ?? local.fullName,
      phone: profile?.phone ?? local.phone,
      preferredContactMethod:
        profile?.preferred_contact_method ?? local.preferredContactMethod,
    };
  } catch {
    return fallback;
  }
...
export async function saveProfileContactDetails(
  details: ProfileContactDetails,
) {
  await AsyncStorage.setItem(STORAGE_KEY, JSON.stringify(details));
  await updateUserPreferences({
    displayName: details.displayName.trim() || details.fullName.trim(),
  });
  const { data: userData } = await supabase.auth.getUser();
  ...
  if (profileError)
    throw new Error(
      "Could not save your profile. Your local changes are still available.",
    );
  ...
  if (deleteError)
    throw new Error(
      "Profile saved, but emergency contacts could not be updated.",
    );
