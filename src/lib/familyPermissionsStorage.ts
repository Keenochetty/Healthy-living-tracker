...
type CaregiverInput = Partial<Omit<CaregiverProfile, "createdAt" | "id" | "isActive" | "isEmergencyContact" | "permissions" | "updatedAt">> & {
...
export async function createFamilyCircle(input: CreateFamilyCircleInput) {
  ...
  await writeJsonArray(FAMILY_CIRCLES_KEY, [circle, ...circles.filter((item) => item.id !== circle.id)]);
...
export async function updateFamilyCircle(id: string, partial: Partial<Omit<FamilyCircle, "createdAt" | "id" | "ownerUserId">>) {
  const circles = await getFamilyCircles();
  const updated = circles.map((circle) =>
    circle.id === id ? { ...circle, ...partial, updatedAt: new Date().toISOString() } : circle
  );

  await writeJsonArray(FAMILY_CIRCLES_KEY, updated);
  await createHealthAuditLog({ action: "family_circle_updated", circleId: id, relatedRealm: "family", relatedId: id });
...
export async function deleteFamilyCircle(id: string) {
  ...
  await writeJsonArray(FAMILY_CIRCLES_KEY, circles.filter((item) => item.id !== id));
  await createHealthAuditLog({ action: "family_circle_deleted", circleId: id, relatedRealm: "family", relatedId: id });
...
export async function createHealthProfile(input: HealthProfileInput) {
  ...
  const profile: HealthProfile = {
    ...
    isManagedProfile: profileType === "child" || profileType === "teen" || profileType === "dependent",
    profileType,
    teenPrivacyTransitionMode: profileType === "teen" ? "parent_full_input" : undefined,
    updatedAt: now,
...
export async function getProfileById(id: string) {
  return (await getAllHealthProfiles()).find((profile) => profile.id === id) ?? null;
}

export async function updateHealthProfile(id: string, partial: Partial<Omit<HealthProfile, "createdAt" | "createdByUserId" | "id">>) {
  const profiles = await getAllHealthProfiles();
  const updated = profiles.map((profile) =>
    profile.id === id ? normalizeProfileAge({ ...profile, ...partial, updatedAt: new Date().toISOString() }) : profile
  );

  await writeJsonArray(HEALTH_PROFILES_KEY, updated);
  await createHealthAuditLog({ action: "health_profile_updated", relatedRealm: "profiles", relatedId: id, targetProfileId: id });

  return updated.find((profile) => profile.id === id) ?? null;
}

export async function convertChildToTeen(profileId: string, mode: TeenPrivacyTransitionMode = "parent_full_input") {
...
export async function getProfilesVisibleToUser() {
  ...
  return profiles.filter((profile) => profile.profileType !== "caregiver_contact");
}
...
export async function getActiveProfile() {
  ...
  const self = profiles.find((profile) => profile.profileType === "self") ?? profiles[0] ?? null;
...
export async function setActiveProfile(profileId: string) {
  ...
  await createHealthAuditLog({ action: "active_profile_changed", relatedRealm: "profiles", relatedId: profileId, targetProfileId: profileId });
...
export async function addFamilyCircleMember(input: FamilyMemberInput) {
  ...
  const member: FamilyCircleMember = {
    ...
    joinedAt: input.inviteStatus === "active" || !input.inviteStatus ? now : undefined,
  ...
  await createHealthAuditLog({ action: "family_member_added", circleId: input.circleId, relatedRealm: "family", relatedId: member.id, targetProfileId: input.profileId });
...
export async function getFamilyCircleMembers(circleId?: string) {
  ...
  return circleId ? members.filter((member) => member.circleId === circleId) : members;
}

export async function updateFamilyMemberRole(memberId: string, role: FamilyRole) {
  const members = await getAllFamilyCircleMembers();
  const updated = members.map((member) =>
    member.id === memberId ? { ...member, role, updatedAt: new Date().toISOString() } : member
  );
  const member = updated.find((item) => item.id === memberId);

  await writeJsonArray(FAMILY_MEMBERS_KEY, updated);
  await createHealthAuditLog({ action: "family_member_role_updated", circleId: member?.circleId, relatedRealm: "family", relatedId: memberId, targetProfileId: member?.profileId });
...
export async function removeFamilyCircleMember(memberId: string) {
  ...
  await writeJsonArray(FAMILY_MEMBERS_KEY, members.filter((item) => item.id !== memberId));
  await createHealthAuditLog({ action: "family_member_removed", circleId: member?.circleId, relatedRealm: "family", relatedId: memberId, targetProfileId: member?.profileId });
...
export async function createFamilyInvite(input: FamilyInviteInput) {
  ...
  await createHealthAuditLog({ action: "family_invite_created", circleId: input.circleId, relatedRealm: "invites", relatedId: invite.id });

  return invite;
}

export async function getPendingInvites() {
  return (await getFamilyInvites()).filter((invite) => invite.status === "pending");
}
...
export async function updateProfilePermission(id: string, partial: Partial<Omit<ProfilePermission, "createdAt" | "id">>) {
  const permissions = await getProfilePermissions();
  const updated = permissions.map((permission) =>
    permission.id === id
      ? {
          ...permission,
          ...partial,
          ...(partial.permissionLevel ? permissionBooleans(partial.permissionLevel) : {}),
  ...
  await createHealthAuditLog({ action: "profile_permission_updated", circleId: permission?.circleId, relatedRealm: "permissions", relatedId: id, targetProfileId: permission?.targetProfileId });
...
export async function revokeProfilePermission(id: string) {
  ...
  await writeJsonArray(PROFILE_PERMISSIONS_KEY, permissions.filter((item) => item.id !== id));
  await createHealthAuditLog({ action: "profile_permission_revoked", circleId: permission?.circleId, relatedRealm: "permissions", relatedId: id, targetProfileId: permission?.targetProfileId });

  return permission;
}

export async function getPermissionsForProfile(profileId: string) {
  return (await getProfilePermissions()).filter((permission) => permission.targetProfileId === profileId);
}

export async function getPermissionsForUser(userId = LOCAL_USER_ID) {
  return (await getProfilePermissions()).filter((permission) => permission.grantedToUserId === userId);
}

export async function canViewProfileRealm(profileId: string, category: PermissionCategory, userId = LOCAL_USER_ID) {
  return hasPermission(profileId, category, VIEW_LEVELS, userId);
}

export async function canAddToProfileRealm(profileId: string, category: PermissionCategory, userId = LOCAL_USER_ID) {
  return hasPermission(profileId, category, ADD_LEVELS, userId);
}

export async function canEditProfileRealm(profileId: string, category: PermissionCategory, userId = LOCAL_USER_ID) {
  return hasPermission(profileId, category, EDIT_LEVELS, userId);
}

export async function canManageProfileRealm(profileId: string, category: PermissionCategory, userId = LOCAL_USER_ID) {
  return hasPermission(profileId, category, MANAGE_LEVELS, userId);
}

export async function filterDataByPermission<T extends { profileId?: string; userId?: string }>(
  items: T[],
  category: PermissionCategory,
  userId = LOCAL_USER_ID
) {
...
export async function createCaregiverProfile(input: CaregiverInput) {
  ...
  await createHealthAuditLog({ action: "caregiver_profile_created", circleId: caregiver.circleId, relatedRealm: "caregivers", relatedId: caregiver.id, targetProfileId: caregiver.assignedProfileId });

  return caregiver;
}

export async function getCaregiversForCircle(circleId: string) {
  return (await getAllFamilyCaregivers()).filter((caregiver) => caregiver.circleId === circleId);
}

export async function getCaregiversForProfile(profileId: string) {
  return (await getAllFamilyCaregivers()).filter((caregiver) => caregiver.assignedProfileId === profileId);
}

export async function updateCaregiverProfile(id: string, partial: Partial<Omit<CaregiverProfile, "createdAt" | "id">>) {
  const caregivers = await getAllFamilyCaregivers();
  const updated = caregivers.map((caregiver) =>
    caregiver.id === id ? { ...caregiver, ...partial, updatedAt: new Date().toISOString() } : caregiver
  );
  const caregiver = updated.find((item) => item.id === id);

  await writeJsonArray(FAMILY_CAREGIVERS_KEY, updated);
  await createHealthAuditLog({ action: "caregiver_profile_updated", circleId: caregiver?.circleId, relatedRealm: "caregivers", relatedId: id, targetProfileId: caregiver?.assignedProfileId });
...
export async function getCaregiverAllowedTasks(caregiverId: string) {
  const caregiver = (await getAllFamilyCaregivers()).find((item) => item.id === caregiverId);
  ...
  return [
    caregiver.permissions.includes("reminders") ? "View assigned reminders" : null,
    caregiver.permissions.includes("medication") ? "Mark allowed medication reminders" : null,
...
export async function createEmergencyInfoCard(input: Partial<Omit<EmergencyInfoCard, "createdAt" | "id" | "updatedAt">> & {
  fullName: string;
  profileId: string;
}) {
  const now = new Date().toISOString();
  ...
  await writeJsonArray(EMERGENCY_INFO_KEY, [card, ...cards.filter((item) => item.profileId !== card.profileId)]);
  await createHealthAuditLog({ action: "emergency_info_created", relatedRealm: "emergency_info", relatedId: card.id, targetProfileId: card.profileId });

  return card;
}

export async function getEmergencyInfoCard(profileId: string) {
  return (await getEmergencyInfoCards()).find((card) => card.profileId === profileId) ?? null;
}

export async function updateEmergencyInfoCard(profileId: string, partial: Partial<Omit<EmergencyInfoCard, "createdAt" | "id" | "profileId">>) {
  ...
  const updated = cards.map((card) =>
    card.profileId === profileId ? { ...card, ...partial, updatedAt: new Date().toISOString() } : card
  );

  await writeJsonArray(EMERGENCY_INFO_KEY, updated);
  await createHealthAuditLog({ action: "emergency_info_updated", relatedRealm: "emergency_info", relatedId: current.id, targetProfileId: profileId });

  return updated.find((card) => card.profileId === profileId) ?? null;
}

export async function getEmergencyInfoForViewer(profileId: string, viewerUserId = LOCAL_USER_ID) {
  const card = await getEmergencyInfoCard(profileId);

  if (!card) return null;
  if (card.visibility === "locked") return null;
  if (card.visibility === "private" && profileId !== LOCAL_PROFILE_ID) return null;
  if (card.visibility === "emergency_contacts" || card.visibility === "caregiver_allowed") {
    const allowed = await canViewProfileRealm(profileId, "emergency_info", viewerUserId);
...
export async function createHealthAuditLog(input: Omit<HealthAuditLog, "actorUserId" | "createdAt" | "id"> & { actorUserId?: string }) {
...
export async function getAuditLogsForProfile(profileId: string) {
  return (await readJsonArray<HealthAuditLog>(AUDIT_LOGS_KEY)).filter((log) => log.targetProfileId === profileId);
}

export async function getAuditLogsForCircle(circleId: string) {
  return (await readJsonArray<HealthAuditLog>(AUDIT_LOGS_KEY)).filter((log) => log.circleId === circleId);
}

export async function getPinnedHealthWidgetsForProfile(profileId: string, fallbackWidgetKeys: WidgetKey[]) {
  const preferences = await getProfileWidgetPreferences();
  const profilePreference = preferences.find((item) => item.profileId === profileId);

  return uniqueWidgetKeys(profilePreference?.widgetKeys.length ? profilePreference.widgetKeys : fallbackWidgetKeys);
}

export async function pinHealthWidgetForProfile(profileId: string, widgetKey: WidgetKey, fallbackWidgetKeys: WidgetKey[]) {
  const current = await getPinnedHealthWidgetsForProfile(profileId, fallbackWidgetKeys);
...
export async function unpinHealthWidgetForProfile(profileId: string, widgetKey: WidgetKey, fallbackWidgetKeys: WidgetKey[]) {
  const current = await getPinnedHealthWidgetsForProfile(profileId, fallbackWidgetKeys);

  return saveProfileWidgetKeys(profileId, current.filter((key) => key !== widgetKey));
}
...
export function getPermissionCategoryForWidget(widgetKey: WidgetKey): PermissionCategory {
  if (["calories_today", "protein_today", "water_today", "food_diary_status", "food_log", "water"].includes(widgetKey)) return "nutrition";
  if (["workout", "steps", "workout_plan"].includes(widgetKey)) return "workout";
  if (["medication", "medication_due_today", "next_medication", "medication_taken_today", "missed_medication", "medication_schedule_status", "medication_schedule"].includes(widgetKey)) return "medication";
  if (widgetKey.includes("supplement")) return "supplements";
  if (["recent_record", "upcoming_follow_up", "prescription_refill", "next_vaccine", "lab_follow_up", "pinned_health_record", "records_needing_attention"].includes(widgetKey)) return "records";
  if (["today_reminders", "next_reminder", "overdue_items", "upcoming_appointment", "timeline_today"].includes(widgetKey)) return "calendar";
  if (["cycle", "cycle_private", "cycle_day", "period_expected", "period_active", "fertile_window_estimate", "estimated_ovulation", "symptoms_today", "mood_today", "contraception_reminder", "contraception_status", "contraception_caution", "womens_health_privacy_status"].includes(widgetKey)) return "womens_health";
  if (["mens_health_check_in", "mens_energy_stress", "testicular_check_reminder", "prostate_discussion_reminder", "fertility_note", "mens_doctor_question", "mens_private_reminder", "mens_health_privacy_status"].includes(widgetKey)) return "mens_health";
  if (["pregnancy_week", "pregnancy_due_date", "pregnancy_next_appointment", "pregnancy_symptom_log", "pregnancy_medication_review", "pregnancy_question", "pregnancy_record", "pregnancy_privacy_status"].includes(widgetKey)) return "pregnancy";
  if (["baby_feed", "next_feed", "last_feed", "sleep_today", "last_sleep", "last_diaper", "weight_latest", "growth_check", "baby_medicine_due", "solid_food_tried", "milestone_check", "baby_note", "baby_today"].includes(widgetKey)) return "baby_child";
  if (["steps_today", "distance_today", "last_synced_workout", "sleep_last_night", "active_calories", "synced_weight", "sync_status"].includes(widgetKey)) return "device_sync";
  if (["weight", "biometric_goal_weight", "sleep", "energy", "mood", "resting_heart_rate", "blood_pressure", "blood_glucose", "digestion", "symptoms"].includes(widgetKey)) return "biometrics";
  return "overview";
}
...
async function hasPermission(profileId: string, category: PermissionCategory, levels: PermissionLevel[], userId: string) {
  const profile = await getProfileById(profileId);

  if (!profile) return false;
  if (profile.id === LOCAL_PROFILE_ID || profile.userId === userId || profile.createdByUserId === userId) return true;
  if (profile.isManagedProfile && !profile.isAdultControlled) return true;
  if (category === "device_sync" && profile.id !== LOCAL_PROFILE_ID) return false;
  ...
  return permissions.some((permission) =>
    ...
    (permission.grantedToUserId === userId || permission.grantedToProfileId === LOCAL_PROFILE_ID || permission.role)
  );
}

async function getAllHealthProfiles() {
  const stored = await getStoredProfiles();
  const [preferences, children] = await Promise.all([getUserPreferences(), getChildProfiles()]);
  const now = new Date().toISOString();
  ...
  const childProfiles = children.map<HealthProfile>((child) =>
    normalizeProfileAge({
      ...
      teenPrivacyTransitionMode: child.profileType === "teen" ? "parent_full_input" : undefined,
      updatedAt: child.updatedAt
    })
  );
  const merged = [selfProfile, ...childProfiles, ...stored.filter((profile) => profile.id !== LOCAL_PROFILE_ID && profile.legacySource !== "child")];
  const unique = new Map<string, HealthProfile>();

  merged.forEach((profile) => unique.set(profile.id, normalizeProfileAge(profile)));
...
async function getAllFamilyCircleMembers() {
  const storedMembers = await readJsonArray<FamilyCircleMember>(FAMILY_MEMBERS_KEY);

  if (storedMembers.length) return storedMembers.sort(sortNewest);

  const [circle, profiles] = await Promise.all([getFamilyCircles().then((circles) => circles[0]), getProfilesVisibleToUser()]);
  const now = new Date().toISOString();
  const members = profiles.map<FamilyCircleMember>((profile) => ({
    ...
    role: profile.profileType === "self" ? "owner" : profile.profileType === "teen" ? "teen" : profile.profileType === "child" ? "child" : "adult_member",
    updatedAt: now,
...
async function updateInviteStatus(inviteId: string, status: FamilyInvite["status"]) {
  const invites = await getFamilyInvites();
  ...
  await createHealthAuditLog({ action: `family_invite_${status}`, circleId: invite?.circleId, relatedRealm: "invites", relatedId: inviteId });
...
async function revokeManagedProfilePermissions(profileId: string) {
  ...
  await writeJsonArray(PROFILE_PERMISSIONS_KEY, permissions.filter((permission) => permission.targetProfileId !== profileId));
}
...
async function getAllFamilyCaregivers() {
  ...
  const legacy = legacySummaries.map<CaregiverProfile>((summary) => ({
    ...
    ratePerHour: summary.rates.find((rate) => rate.rateType === "hourly")?.amount,
    role: summary.caregiver.services[0],
    updatedAt: summary.caregiver.updatedAt
  }));
  const byId = new Map<string, CaregiverProfile>();

  [...legacy, ...stored].forEach((caregiver) => byId.set(caregiver.id, caregiver));
...
async function saveProfileWidgetKeys(profileId: string, widgetKeys: WidgetKey[]) {
  const now = new Date().toISOString();
  ...
  await writeJsonArray(PROFILE_WIDGETS_KEY, [preference, ...preferences.filter((item) => item.profileId !== profileId)]);
...
function normalizeProfileAge(profile: HealthProfile): HealthProfile {
  ...
  if (age >= 18 && (profile.profileType === "child" || profile.profileType === "teen" || profile.isManagedProfile)) {
    return {
      ...profile,
      adultControlActivatedAt: profile.adultControlActivatedAt ?? new Date().toISOString(),
      isAdultControlled: true,
      isManagedProfile: false,
      profileType: "adult"
    };
  ...
  if (age >= 13 && profile.profileType === "child") {
    return {
      ...profile,
      profileType: "teen",
      teenPrivacyTransitionMode: profile.teenPrivacyTransitionMode ?? "parent_full_input"
    };
  }
...
function isAdultProfile(profileType: ProfileType) {
  return profileType === "self" || profileType === "adult" || profileType === "partner" || profileType === "elder";
}
...
function permissionBooleans(permissionLevel: PermissionLevel) {
  return {
    ...
    canView: VIEW_LEVELS.includes(permissionLevel) || permissionLevel === "emergency_only"
  };
}
