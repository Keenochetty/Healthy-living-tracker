...
const UNIQUE_HEALTH_WIDGET_KEYS = Array.from(
  new Set(HEALTH_WIDGET_KEYS),
) as WidgetKey[];

export function isNutritionWidget(widgetKey: WidgetKey) {
  return NUTRITION_WIDGET_KEYS.includes(
    widgetKey as (typeof NUTRITION_WIDGET_KEYS)[number],
  );
}

export function isMedicationWidget(widgetKey: WidgetKey) {
  return MEDICATION_WIDGET_KEYS.includes(
    widgetKey as (typeof MEDICATION_WIDGET_KEYS)[number],
  );
}

export function isSupplementWidget(widgetKey: WidgetKey) {
  return SUPPLEMENT_WIDGET_KEYS.includes(
    widgetKey as (typeof SUPPLEMENT_WIDGET_KEYS)[number],
  );
}

function toHealthWidget(
  widgetKey: WidgetKey,
  orderIndex: number,
  isPinned = true,
...
export async function unpinHealthWidget(widgetKey: WidgetKey) {
  ...
  await updateUserPreferences({
    enabledWidgets: preferences.enabledWidgets.filter(
      (key) => key !== widgetKey,
    ),
  });
...
export async function reorderHealthWidgets(widgetKeys: WidgetKey[]) {
  ...
  const requestedWidgets = widgetKeys.filter((widgetKey) =>
    HEALTH_WIDGET_KEYS.includes(
      widgetKey as (typeof HEALTH_WIDGET_KEYS)[number],
    ),
  );
...
export async function getAvailableNutritionWidgets() {
  ...
  return availableWidgets.filter((widget) =>
    isNutritionWidget(widget.widgetKey),
  );
}

export async function getPinnedHealthWidgetsForProfile(profileId: string) {
  const preferences = await getUserPreferences();
  const widgetKeys = await getProfileWidgetKeys(
    profileId,
  ...
  return widgetKeys
    ...
    .map((widgetKey, orderIndex) =>
      toHealthWidget(widgetKey, orderIndex, true, profileId),
    );
}
...
export async function calculateWidgetValueForProfile(
  profileId: string,
  widgetKey: WidgetKey,
) {
...
export {
  getWidgetRouteType as getBiometricWidgetRouteType,
  isBabyWidget,
  isBiometricWidget,
  isHealthCalendarWidget,
  isMensHealthWidget,
