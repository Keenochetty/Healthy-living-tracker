...
const LAB_RESULT_RECORDS_STORAGE_KEY =
  "family_health_phase12_lab_result_records";
const PRESCRIPTION_RECORDS_STORAGE_KEY =
  "family_health_phase12_prescription_records";
const HEALTH_RECORD_FOLDERS_STORAGE_KEY =
  "family_health_phase12_health_record_folders";
const HEALTH_RECORD_REMINDERS_STORAGE_KEY =
  "family_health_phase12_health_record_reminders";
...
const DEFAULT_FOLDERS = [
  {
    color: "#fef3c7",
    icon: "documents",
    id: "folder-prescriptions",
    name: "Prescriptions",
  },
  {
    color: "#e0f2fe",
    icon: "documents",
    id: "folder-lab-results",
    name: "Lab Results",
  },
  {
    color: "#ecfdf5",
    icon: "doctor",
    id: "folder-doctor-visits",
    name: "Doctor Visits",
  },
  {
    color: "#fce7f3",
  ...
  },
  {
    color: "#f3e8ff",
    icon: "documents",
    id: "folder-child-records",
    name: "Child Records",
  },
  {
    color: "#fee2e2",
    icon: "medication",
    id: "folder-medication-labels",
    name: "Medication Labels",
  },
  {
    color: "#ccfbf1",
    icon: "health",
    id: "folder-supplement-labels",
    name: "Supplement Labels",
  },
  {
    color: "#f8fafc",
    icon: "documents",
    id: "folder-general-notes",
    name: "General Notes",
  },
] as const;
...
type CreateHealthRecordInput = Partial<
  Omit<
    HealthRecord,
    | "allowedViewerIds"
    | "createdAt"
...
type CreateDoctorVisitInput = Partial<
  Omit<
    DoctorVisit,
    | "allowedViewerIds"
    | "createdAt"
...
type CreateVaccineRecordInput = Partial<
  Omit<
    VaccineRecord,
    | "allowedViewerIds"
    | "createdAt"
...
type CreateLabResultInput = Partial<
  Omit<
    LabResultRecord,
    | "allowedViewerIds"
    | "createdAt"
...
type CreatePrescriptionInput = Partial<
  Omit<
    PrescriptionRecord,
    | "allowedViewerIds"
    | "createdAt"
...
type CreateFolderInput = Partial<
  Omit<
    HealthRecordFolder,
    "createdAt" | "id" | "isDefault" | "profileId" | "updatedAt" | "userId"
  >
> & {
  name: string;
};

type CreateReminderInput = Partial<
  Omit<
    HealthRecordReminder,
...
export async function createHealthRecord(input: CreateHealthRecordInput) {
  ...
  if (record.reminderDate) {
    await createHealthRecordReminder({
      ...
      type:
        record.expiryDate === record.reminderDate
          ? "document_expiry"
          : "general",
    });
  }
...
export async function updateHealthRecord(
  id: string,
  partial: Partial<
    Omit<HealthRecord, "createdAt" | "id" | "profileId" | "userId">
  >,
) {
  const records = await getHealthRecords();
  const updatedRecords = records.map((record) =>
    record.id === id
      ? {
          ...record,
          ...partial,
          tags: partial.tags ? normalizeTags(partial.tags) : record.tags,
...
export async function deleteHealthRecord(id: string) {
  ...
  await Promise.all([
    ...
    writeJsonArray(
      HEALTH_RECORD_REMINDERS_STORAGE_KEY,
      reminders.filter((item) => item.relatedRecordId !== id),
    ),
  ]);
...
export async function searchHealthRecords(query: string) {
  ...
  return records.filter((record) => {
    const relatedVisit = visits.find(
      (visit) => visit.id === record.relatedVisitId,
    );
    const relatedPrescription = prescriptions.find(
      (prescription) => prescription.documentId === record.id,
    );
    const haystack = [
      record.title,
      record.type,
  ...
  return records.filter(
    (record) =>
      ...
      (filters.isPinned === undefined ||
        record.isPinned === filters.isPinned) &&
      (filters.hasFile === undefined ||
        Boolean(record.fileUrl) === filters.hasFile) &&
      (filters.hasReminder === undefined ||
        Boolean(record.reminderDate || record.expiryDate) ===
          filters.hasReminder) &&
...
export async function prepareHealthRecordUpload(
  fileType: HealthRecordFileType = "unknown",
) {
  return {
    fileType,
    localOnly: true,
    message:
      "File upload storage will be connected later. You can still organize this record manually.",
  };
}

export async function saveHealthRecordFile(
  recordId: string,
  fileUrl: string,
  fileType: HealthRecordFileType = "unknown",
) {
...
export async function deleteHealthRecordFile(recordId: string) {
  return updateHealthRecord(recordId, {
    fileType: "unknown",
    fileUrl: undefined,
  });
}
...
export async function updateDoctorVisit(
  id: string,
  partial: Partial<
    Omit<DoctorVisit, "createdAt" | "id" | "profileId" | "userId">
  >,
) {
  const visits = await getDoctorVisits();
  const updatedVisits = visits.map((visit) =>
    visit.id === id
      ? { ...visit, ...partial, updatedAt: new Date().toISOString() }
      : visit,
  );
...
export async function deleteDoctorVisit(id: string) {
  ...
  await Promise.all([
    writeJsonArray(
      ...
      visits.filter((item) => item.id !== id),
    ),
    writeJsonArray(
      HEALTH_RECORD_REMINDERS_STORAGE_KEY,
      reminders.filter((item) => item.relatedVisitId !== id),
    ),
  ]);
...
export async function linkRecordToVisit(recordId: string, visitId: string) {
  ...
  if (visit) {
    await updateDoctorVisit(visitId, {
      relatedDocumentIds: Array.from(
        new Set([...visit.relatedDocumentIds, recordId]),
      ),
    });
  }
}
...
export async function getVaccineRecords() {
  return (await readJsonArray<VaccineRecord>(VACCINE_RECORDS_STORAGE_KEY)).sort(
    sortByUpdatedAt,
  );
}
...
export async function updateVaccineRecord(
  id: string,
  partial: Partial<
    Omit<VaccineRecord, "createdAt" | "id" | "profileId" | "userId">
  >,
) {
  return updateStoredRecord(
    VACCINE_RECORDS_STORAGE_KEY,
    id,
    partial,
    getVaccineRecords,
  );
...
export async function getLabResultRecords() {
  return (
    await readJsonArray<LabResultRecord>(LAB_RESULT_RECORDS_STORAGE_KEY)
  ).sort(sortByUpdatedAt);
}

export async function updateLabResultRecord(
  id: string,
  partial: Partial<
    Omit<LabResultRecord, "createdAt" | "id" | "profileId" | "userId">
  >,
) {
  return updateStoredRecord(
    LAB_RESULT_RECORDS_STORAGE_KEY,
    id,
    partial,
    getLabResultRecords,
  );
...
export async function createPrescriptionRecord(input: CreatePrescriptionInput) {
  ...
  await writeJsonArray(PRESCRIPTION_RECORDS_STORAGE_KEY, [
    prescription,
    ...records,
  ]);
...
export async function getPrescriptionRecords() {
  return (
    await readJsonArray<PrescriptionRecord>(PRESCRIPTION_RECORDS_STORAGE_KEY)
  ).sort(sortByUpdatedAt);
}
...
export async function updatePrescriptionRecord(
  id: string,
  partial: Partial<
    Omit<PrescriptionRecord, "createdAt" | "id" | "profileId" | "userId">
  >,
) {
  return updateStoredRecord(
    PRESCRIPTION_RECORDS_STORAGE_KEY,
    id,
    partial,
    getPrescriptionRecords,
  );
...
export async function linkPrescriptionToMedication(
  prescriptionId: string,
  medicationId: string,
) {
  return updatePrescriptionRecord(prescriptionId, {
...
export async function getHealthRecordFolders() {
  const folders = await readJsonArray<HealthRecordFolder>(
    HEALTH_RECORD_FOLDERS_STORAGE_KEY,
  );
  const folderIds = new Set(folders.map((folder) => folder.id));
  const now = new Date().toISOString();
  const defaults = DEFAULT_FOLDERS.filter(
  ...
  if (defaults.length) {
    await writeJsonArray(HEALTH_RECORD_FOLDERS_STORAGE_KEY, [
      ...defaults,
      ...folders,
    ]);
  }

  return [...defaults, ...folders].sort(
    (left, right) =>
...
export async function updateHealthRecordFolder(
  id: string,
  partial: Partial<
    Omit<
      HealthRecordFolder,
      "createdAt" | "id" | "isDefault" | "profileId" | "userId"
    >
  >,
) {
  return updateStoredRecord(
...
export async function createHealthRecordReminder(input: CreateReminderInput) {
  ...
  await writeJsonArray(HEALTH_RECORD_REMINDERS_STORAGE_KEY, [
    reminder,
    ...reminders,
  ]);
...
export async function getHealthRecordReminders() {
  const reminders = await readJsonArray<HealthRecordReminder>(
    HEALTH_RECORD_REMINDERS_STORAGE_KEY,
  );
  ...
  return reminders
    ...
    .sort(
      (left, right) =>
        new Date(left.reminderDate).getTime() -
        new Date(right.reminderDate).getTime(),
    );
}
...
export async function getUpcomingRecordReminders(limit = 5) {
  ...
  return reminders
    .filter(
      (reminder) =>
        reminder.status === "upcoming" || reminder.status === "missed",
    )
    .slice(0, limit);
}
...
export function isRecordWidget(
  widgetKey: WidgetKey,
): widgetKey is RecordWidgetKey {
...
export async function calculateRecordWidgetValue(widgetKey: WidgetKey) {
  ...
  switch (widgetKey) {
    ...
    case "pinned_health_record":
      return summary.pinnedRecords.length
        ? `${summary.pinnedRecords.length} pinned`
        : "None";
...
export async function getRecordsOverviewSummary(): Promise<RecordsOverviewSummary> {
  const [records, visits, reminders, vaccines, labs, prescriptions] =
    await Promise.all([
      getHealthRecords(),
      getDoctorVisits(),
  ...
  const labFollowUps = labs
    .filter((lab) => lab.followUpDate && lab.followUpDate >= today)
    .slice(0, 3);
  const prescriptionRefills = prescriptions
    .filter(
      (prescription) =>
        prescription.refillReminderDate &&
        prescription.refillReminderDate >= today,
    )
    .slice(0, 3);

  return {
    labFollowUps,
    nextVaccine: vaccines.filter(
      (vaccine) => vaccine.nextDoseDate && vaccine.nextDoseDate >= today,
    )[0],
    pinnedRecords: records.filter((record) => record.isPinned).slice(0, 5),
    prescriptionRefills,
    recentRecords: records.slice(0, 5),
    recentVisits: visits.slice(0, 3),
    recordsNeedingAttention: reminders.filter(
      (reminder) =>
        reminder.status === "missed" || reminder.status === "upcoming",
    ).length,
