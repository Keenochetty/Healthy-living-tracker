...
const PERSONALIZATION_PREFERENCES_KEY =
  "family_health_phase20_personalization_preferences";
...
export const HEALTH_MODULE_OPTIONS: HealthModuleOption[] = [
  moduleOption(
    "calendar",
    "Calendar / Timeline",
    "Reminders, appointments and health timeline.",
    ["planning"],
    true,
    "Private by default",
    ["today_reminders", "next_reminder", "upcoming_appointment"],
  ),
  moduleOption(
    "records",
    "Records",
    "Documents, visits, prescriptions and follow-ups.",
    ["personal_health"],
    true,
    "Private",
    ["recent_record", "upcoming_follow_up", "prescription_refill"],
  ),
  moduleOption(
    "nutrition",
    "Food / Nutrition",
    "Meals, water, targets and reports.",
    ["food"],
    false,
    "Optional",
    [
  ...
  moduleOption(
    "workout",
    "Workout",
    "Plans, sessions and fitness progress.",
    ["fitness"],
    false,
    "Optional",
    ["workout", "steps_today", "workout_reminder"],
  ),
  moduleOption(
    "biometrics",
    "Biometrics",
    "Weight, sleep, mood, energy and vitals.",
    ["personal_health"],
    false,
    "Private",
    ["weight", "sleep", "energy"],
  ...
  moduleOption(
    "medication",
    "Medication",
    "Schedules, logs and private reminders.",
    ["personal_health"],
    false,
    "Private",
    ["medication_due", "next_medication", "medication_taken_today"],
  ),
  moduleOption(
    "supplements",
    "Supplements",
    "Supplement schedules, logs and labels.",
    ["personal_health"],
    false,
    "Private",
    ["supplement_due", "next_supplement"],
  ),
  moduleOption(
    "womens_health",
    "Women's Health",
    "Cycle, symptoms and contraception tracking.",
    ["pregnancy_cycle"],
    false,
    "Private",
    ["cycle_day", "period_expected", "contraception_reminder"],
    true,
  ),
  moduleOption(
    "mens_health",
    "Men's Health",
    "Private check-ins, reminders and questions.",
    ["mens_health"],
    false,
    "Private",
    [
  ...
  moduleOption(
    "pregnancy",
    "Pregnancy Mode",
    "Pregnancy weeks, appointments and questions.",
    ["pregnancy_cycle"],
    false,
    "Private",
    [
  ...
  moduleOption(
    "baby_child",
    "Baby / Child",
    "Feeding, sleep, diapers, milestones and care.",
    ["child_baby"],
    false,
    "Parent managed",
    ["next_feed", "last_diaper", "sleep_today", "next_vaccine"],
    true,
  ),
  moduleOption(
    "device_sync",
    "Device Sync",
    "Prepare Apple Health or Health Connect imports.",
    ["personal_health"],
    false,
    "Off by default",
    ["sync_status", "steps_today", "sleep_last_night"],
  ),
  moduleOption(
    "ai_assistant",
    "AI Assistant",
    "Draft-first help with logging and questions.",
    ["ai_assistant"],
    false,
    "Consent required",
    ["ai_suggestion"],
  ...
  moduleOption(
    "family_circles",
    "Family Circles",
    "Optional sharing, caregivers and family permissions.",
    ["circle", "caregiver"],
    false,
    "Optional sharing",
    ["family_status", "caregiver_active"],
  ),
];
...
export async function createInitialHealthProfile(
  input: InitialHealthProfileInput,
) {
  const country = getCountryByName(input.country ?? "South Africa");
  const unitSystem = input.unitSystem ?? "metric";
  const units =
    unitSystem === "imperial" ? imperialUnits() : country.defaultUnits;
...
export async function enableHealthModule(moduleKey: HealthModuleKey) {
  ...
  return updateOnboardingState({
    selectedModules: unique([...state.selectedModules, moduleKey]),
  });
}
...
export async function disableHealthModule(moduleKey: HealthModuleKey) {
  const state = await getOnboardingState();
  return updateOnboardingState({
    selectedModules: state.selectedModules.filter((item) => item !== moduleKey),
  });
}
...
export function applyDefaultModulesFromGoal(
  goal?: MainHealthGoal,
): HealthModuleKey[] {
  switch (goal) {
    case "fitness":
...
export function getSuggestedWidgetsForModules(
  modules: HealthModuleKey[],
  layout: HomeLayoutPreference = "simple",
) {
  const suggested = modules.flatMap(
    (moduleKey) =>
      HEALTH_MODULE_OPTIONS.find((module) => module.key === moduleKey)
        ?.suggestedWidgets ?? [],
  );
  return applyHomeLayoutPreset(
    layout,
    unique(["today_reminders", "next_reminder", ...suggested]) as WidgetKey[],
  );
}
...
export function applyHomeLayoutPreset(
  ...
  return unique([...(presets[layout] ?? []), ...widgets]).slice(
    0,
    12,
  ) as WidgetKey[];
}
...
export async function getPersonalizationPreferences(): Promise<PersonalizationPreferences> {
  const stored = await readJson<PersonalizationPreferences>(
    PERSONALIZATION_PREFERENCES_KEY,
  );
...
export async function saveInitialNotificationChoice(
  ...
  await updateNotificationSettings({
    ...
    permissionStatus:
      choice === "device_notifications" ? "not_requested" : "not_requested",
  });
...
export async function setupNotificationCategoriesFromOnboarding(
  choice: NotificationOnboardingChoice,
) {
...
export async function saveInitialAssistantConsent(
  enabled: boolean,
  categories: string[] = [],
) {
  await updateAssistantSettings({
    allowedDataCategories: enabled
      ? (["nutrition", "workout", "calendar", "records"].filter((category) =>
...
async function applySelectedModules(modules: HealthModuleKey[]) {
  const appModules = unique([
    ...CORE_MODULE_KEYS,
    ...modules.flatMap(
      (module) =>
        HEALTH_MODULE_OPTIONS.find((option) => option.key === module)
          ?.appModuleKeys ?? [],
    ),
  ]) as AppModuleKey[];
  const preferences = await getUserPreferences();
...
async function saveModulePreferences(modules: HealthModuleKey[]) {
  const now = new Date().toISOString();
  const preferences: ModulePreference[] = HEALTH_MODULE_OPTIONS.map(
    (module) => ({
      createdAt: now,
