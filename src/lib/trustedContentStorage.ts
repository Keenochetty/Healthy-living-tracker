...
export async function createTrustedSource(
  input: Omit<TrustedSource, "createdAt" | "id" | "updatedAt">,
) {
  ...
  await createHealthContentAuditLog({
    action: "source_created",
    sourceId: source.id,
    newValue: { sourceOrganization: source.sourceOrganization },
  });
...
export async function getTrustedSources(includeSeeds = true) {
  ...
  (includeSeeds ? [...seedTrustedSources(), ...sources] : sources).forEach(
    (source) => byId.set(source.id, source),
  );

  return Array.from(byId.values()).sort((left, right) =>
    left.sourceOrganization.localeCompare(right.sourceOrganization),
  );
}
...
export async function updateTrustedSource(
  id: string,
  partial: Partial<TrustedSource>,
) {
  const sources = await getTrustedSources(false);
  const updated = sources.map((source) =>
    source.id === id
      ? { ...source, ...partial, updatedAt: new Date().toISOString() }
      : source,
  );
...
export function validateSourceTrustLevel(
  ...
  if (
    ...
    source.sourceTier === "disallowed"
  )
    return false;
  if (riskLevel === "high" || riskLevel === "critical")
    return source.sourceTier === "tier_1" || source.sourceTier === "tier_2";
  return true;
}

export async function createTrustedHealthContentCard(
  input: Omit<
    TrustedHealthContentCard,
    "createdAt" | "id" | "status" | "updatedAt" | "versionNumber"
  > & {
...
export async function getTrustedHealthContentCards(includeSeeds = true) {
  ...
  (includeSeeds ? [...seedTrustedContentCards(), ...cards] : cards).forEach(
    (card) => byId.set(card.id, markExpiredIfNeeded(card)),
  );
...
export async function getPublishedContentByRealm(realm: HealthContentRealm) {
  ...
  return cards.filter((card) => {
    ...
    const isFreshEnough =
      card.status === "published" && !isPastReviewDate(card);
    ...
    return (
      ...
      isFreshEnough &&
      highRiskAllowed &&
      validateSourceTrustLevel(source, card.riskLevel)
    );
  });
}
...
export async function getContentCardById(id: string) {
  return (
    (await getTrustedHealthContentCards()).find((card) => card.id === id) ??
    null
  );
}

export async function updateTrustedHealthContentCard(
  id: string,
  ...
  const updated = cards.map((card) =>
    card.id === id
      ? {
          ...
          versionNumber: partial.versionNumber ?? card.versionNumber + 1,
        }
      : card,
  );
...
export async function approveContentCard(id: string) {
  ...
  if (!card || check?.overallStatus !== "pass") {
    return updateTrustedHealthContentCard(id, {
      status: check?.overallStatus === "blocked" ? "blocked" : "review_needed",
    });
  }

  return updateTrustedHealthContentCard(id, {
    approvedBy: LOCAL_USER_ID,
    reviewedBy: LOCAL_USER_ID,
    status: "approved",
  });
}
...
export async function runHealthContentQualityCheck(contentCardId: string) {
  ...
  const unsafeWordingFound = card
    ? checkUnsafeWording(`${card.title} ${card.shortSummary} ${card.fullText}`)
        .length > 0
    : true;
  const medicalClaimFound = card
    ? checkMedicalClaims(card.fullText).length > 0
    : true;
  const plainLanguagePassed = card ? checkPlainLanguage(card.fullText) : false;
  const hasTrustedSource = Boolean(card?.sourceId && source);
  const sourceApproved = validateSourceTrustLevel(
    source,
  ...
  await writeJsonArray(QUALITY_KEY, [
    check,
    ...checks.filter((item) => item.contentCardId !== contentCardId),
  ]);
...
export function checkPlainLanguage(text: string) {
  return (
    estimateReadingLevel(text) <= 10 &&
    text
      .split(/[.!?]/)
      .filter((sentence) => sentence.trim().split(/\s+/).length > 28).length ===
      0
  );
}
...
export function estimateReadingLevel(text: string) {
  ...
  return Math.min(
    14,
    Math.max(4, Math.round(6 + (longWords / Math.max(1, words.length)) * 20)),
  );
}
...
export function validateContentRiskRules(
...
export async function getContentReviewQueue() {
  return (await getTrustedHealthContentCards()).filter(
    (card) =>
      card.status === "draft" ||
      card.status === "review_needed" ||
      card.status === "source_needed",
  );
}
...
export async function getDisclaimerByKey(key: string) {
  return (
    (await getDisclaimers()).find((disclaimer) => disclaimer.key === key) ??
    null
  );
}
...
export async function createHealthContentDisclaimer(
  input: Omit<HealthContentDisclaimer, "createdAt" | "id" | "updatedAt">,
) {
  const now = new Date().toISOString();
  ...
  await createHealthContentAuditLog({
    ...
    newValue: { key: disclaimer.key },
  });

  return disclaimer;
}

export async function updateHealthContentDisclaimer(
  id: string,
  partial: Partial<HealthContentDisclaimer>,
) {
  const disclaimers = await getDisclaimers(false);
  const updated = disclaimers.map((disclaimer) =>
    disclaimer.id === id
      ? { ...disclaimer, ...partial, updatedAt: new Date().toISOString() }
      : disclaimer,
  );
...
export async function getApprovedSourceBackedAnswer(
  topic: string,
  ...
  if (!cards.length) {
    return {
      cards,
      message:
        "I do not have a trusted source saved for this topic yet. Please speak to a healthcare professional or check an official health source.",
    };
  }
...
export async function getSourceCardsForTopic(
  topic: string,
  realm?: HealthContentRealm,
) {
  const tokens = topic
  ...
  const cards = realm
    ? await getPublishedContentByRealm(realm)
    : (await getTrustedHealthContentCards()).filter(
        (card) => card.status === "published" && !isPastReviewDate(card),
      );
  const sources = await getTrustedSources();
  ...
  return cards.filter((card) => {
    const source = sources.find((item) => item.id === card.sourceId) ?? null;
    const haystack =
      `${card.title} ${card.shortSummary} ${card.topicTags.join(" ")}`.toLowerCase();
    return (
      validateSourceTrustLevel(source, card.riskLevel) &&
      (tokens.length === 0 || tokens.some((token) => haystack.includes(token)))
    );
  });
}

export async function requireTrustedSourceForHighRiskTopic(
  topic: string,
  realm?: HealthContentRealm,
) {
  const cards = await getSourceCardsForTopic(topic, realm);
  ...
  return cards.some(
    ...
    ? cards
    : cards.filter(
        (card) => card.riskLevel === "low" || card.riskLevel === "medium",
      );
}

export async function logAiContentUsage(contentCardIds: string[]) {
  await Promise.all(
    contentCardIds.map((contentCardId) =>
      createHealthContentAuditLog({
        action: "ai_used_content_card",
        contentCardId,
      }),
    ),
  );
...
export async function createHealthContentAuditLog(
  input: Omit<HealthContentAuditLog, "createdAt" | "id">,
) {
...
async function getDisclaimers(includeSeeds = true) {
  ...
  (includeSeeds ? [...seedDisclaimers(), ...stored] : stored).forEach(
    (disclaimer) => byKey.set(disclaimer.key, disclaimer),
  );
...
function buildQualityNote(input: {
  freshnessValid: boolean;
  riskRulesValid: boolean;
  ...
  return (
    [
      input.sourceApproved
        ? null
        : "Source is not approved for this risk level.",
...
function seedTrustedContentCards(): TrustedHealthContentCard[] {
  return [
    seedCard(
      "content-nutrition-food-diary",
      "Food diary basics",
      "Food logs help you track patterns based on what you enter.",
      "A food diary records meals, snacks, water, and notes. It supports tracking and conversation preparation. It is not a diet prescription.",
      "nutrition",
    ...
    seedCard(
      "content-med-label",
      "Medication label tracking",
      "Medication tracking should follow labels and professional instructions.",
      "Use this app to organize medication names, schedules, and questions. Always follow your prescription label, product label, pharmacist, doctor, nurse, clinic, or healthcare professional's instructions.",
      "medication",
      ["label", "medication", "pharmacist"],
      "high",
    ...
    seedCard(
      "content-pregnancy-appointments",
      "Pregnancy appointment preparation",
      "Pregnancy logs can help you prepare questions for your clinic.",
      "Pregnancy information is for education and organization only. Appointment timing and concerns should be confirmed with your doctor, midwife, nurse, clinic, or healthcare professional.",
      "pregnancy",
      ...
      "pregnancy",
      { ageGroup: "pregnancy", pregnancyRelevant: true },
    ),
    seedCard(
      "content-baby-safe-sleep",
      "Baby sleep notes",
      "Baby sleep tracking is for logs and discussion only.",
      "Baby and child information is for education and tracking only. Every child is different. If you are concerned, speak to a pediatrician, clinic, nurse, doctor, or healthcare professional.",
      "baby_child",
      ["baby", "sleep", "pediatrician"],
      "high",
      "source-aap",
      "AAP / HealthyChildren",
      "https://www.healthychildren.org/",
      "baby_child",
      { ageGroup: "baby", childRelevant: true },
    ),
    seedCard(
      "content-mens-testicular",
      "Testicular self-check education",
      "Checking can help you notice changes to discuss with a professional.",
      "This content supports noticing changes and preparing questions. It does not diagnose symptoms or rule out health conditions.",
      "mens_health",
      ["testicular", "self-check", "doctor"],
      "high",
      "source-nhs",
      "NHS",
    ...
    seedCard(
      "content-womens-contraception",
      "Contraception tracking notes",
      "Tracking is for organization and education only.",
      "Contraception tracking is for organization and education only. Always follow your product leaflet, prescription label, clinic guidance, or healthcare professional's advice.",
      "womens_health",
      ["contraception", "period", "clinic"],
      "high",
      "source-owh",
      "Office on Women’s Health",
      "https://www.womenshealth.gov/",
...
function seedDisclaimers(): HealthContentDisclaimer[] {
  ...
  return [
    make(
      "general",
      "General education",
      "This information is for general education and tracking only. It is not medical advice.",
      [
        "general",
        "nutrition",
        "workout",
    ...
    make(
      "medication",
      "Medication",
      "Always follow your prescription label, product label, pharmacist, doctor, nurse, clinic, or healthcare professional's instructions.",
    ...
    make(
      "pregnancy",
      "Pregnancy",
      "Pregnancy information is for education and organization only. Confirm concerns with your doctor, midwife, nurse, clinic, or healthcare professional.",
      ["pregnancy"],
      ["high", "critical"],
    ),
    make(
    ...
    make(
      ...
      ["general", "pregnancy", "baby_child", "biometrics", "medication"],
      ["critical"],
    ),
    make(
      "contraception",
    ...
    make(
      "ai",
      "AI",
      "AI suggestions may be incomplete or incorrect. Review and confirm before saving.",
      ["ai_assistant"],
