...
import { createBabyDiaperLog, createBabyFeedingLog, getVisibleBabyProfilesForViewer } from "@/lib/babyChildStorage";
import { completeWorkoutSession, getTodayFitnessSummary, createWorkoutSession } from "@/lib/fitnessStorage";
import {
  canViewProfileRealm,
...
import { createHealthRecord, getHealthRecords } from "@/lib/healthRecordsStorage";
import { createDoseLog } from "@/lib/medicationSupplementStorage";
import { createMensHealthQuestion, getMensHealthReportSummary } from "@/lib/mensHealthStorage";
import { createNutritionEntry, getTodayNutritionSummary, toNutritionDateKey } from "@/lib/nutritionStorage";
import { createPregnancyQuestion, getPregnancyProfile } from "@/lib/pregnancyStorage";
import { getSourceCardsForTopic as getGovernedSourceCardsForTopic, logAiContentUsage } from "@/lib/trustedContentStorage";
import { createHealthReminder } from "@/services/reminders/reminderEngine";
import { createPeriodLog, createSymptomLog, getWomensHealthTodaySummary } from "@/lib/womensHealthStorage";
import type { PermissionCategory } from "@/types/familyPermissions";
import type { HealthContentRealm, TrustedHealthContentCard } from "@/types/trustedContent";
import type {
  AssistantActionType,
  AssistantAuditLog,
...
const SENSITIVE_FOOTER = "This is for tracking and education only and is not medical advice. Speak to a healthcare professional if unsure.";
const DRAFT_FOOTER = "Review and confirm before saving. AI suggestions may be incomplete or incorrect.";
const EMERGENCY_FOOTER = "If this feels urgent or severe, contact local emergency services or a healthcare professional immediately.";
...
export const ASSISTANT_CONSENT_CATEGORIES: Array<{ key: AssistantDataCategory; label: string; sensitive: boolean }> = [
  ...
  { key: "medication_supplements", label: "Medication/Supplements", sensitive: true },
  { key: "records", label: "Records/Documents", sensitive: true },
  { key: "mens_health", label: "Men’s Health", sensitive: true }
];

export async function getAssistantSettings(profileId = LOCAL_PROFILE_ID): Promise<AssistantSettings> {
  const settings = (await readJsonArray<AssistantSettings>(SETTINGS_KEY)).find((item) => item.profileId === profileId);
  ...
  return {
    ...
    sensitiveCategoryConsent: Object.fromEntries(SENSITIVE_CATEGORIES.map((category) => [category, false])),
    updatedAt: now,
    userId: LOCAL_USER_ID
  };
}

export async function updateAssistantSettings(partial: Partial<AssistantSettings> & { profileId?: string }) {
  const current = await getAssistantSettings(partial.profileId ?? LOCAL_PROFILE_ID);
  ...
  await writeJsonArray(SETTINGS_KEY, [next, ...settings.filter((item) => item.profileId !== next.profileId)]);

  return next;
}

export async function checkAssistantPermission(category: AssistantDataCategory, profileId = LOCAL_PROFILE_ID) {
  ...
  if (SENSITIVE_CATEGORIES.includes(category) && !settings.sensitiveCategoryConsent[category]) return false;

  return validateAssistantCanAccessData([category], profileId);
}

export async function createAssistantConversation(input: { mode: AssistantMode; title?: string; profileId?: string }) {
  const now = new Date().toISOString();
  ...
  const conversations = await readJsonArray<AssistantConversation>(CONVERSATIONS_KEY);
...
export async function createAssistantMessage(input: Omit<AssistantMessage, "createdAt" | "id" | "profileId" | "userId"> & { profileId?: string }) {
  ...
  await writeJsonArray(
    ...
    conversations.map((conversation) =>
      conversation.id === conversationId
        ? { ...conversation, isDeleted: true, updatedAt: new Date().toISOString() }
        : conversation
    )
  );
}
...
export async function clearAssistantHistory(profileId = LOCAL_PROFILE_ID) {
  ...
  await writeJsonArray(CONVERSATIONS_KEY, conversations.filter((conversation) => conversation.profileId !== profileId));
  await writeJsonArray(MESSAGES_KEY, messages.filter((message) => message.profileId !== profileId));
}

export function classifyAssistantRequestRisk(text: string, mode: AssistantMode): AssistantRiskCategory {
  ...
  if (mode === "pregnancy" || /\bpregnan|midwife|miscarriage|contraction/i.test(text)) return "pregnancy";
  if (mode === "baby_child" || /\bbaby|child|pediatric|diaper|formula\b/i.test(text)) return "baby_child";
  if (mode === "medication_supplement" || /\bmedication|medicine|pill|dose|supplement|contraception\b/i.test(text)) return "medication_supplement";
  if (/\bsymptom|pain|bleeding|glucose|blood pressure|fertility|sexual|period\b/i.test(text)) return "sensitive_health";
  return "low_risk";
}

export function detectUrgentEmergencyLanguage(text: string) {
  return /\b(severe chest pain|trouble breathing|can't breathe|cannot breathe|anaphylaxis|severe allergic|seizure|unconscious|severe bleeding|not breathing|self[- ]?harm|suicidal|severe dehydration|pregnancy severe bleeding|severe pain)\b/i.test(text);
}

export function blockUnsafeMedicalAdvice(text: string) {
  return /\b(diagnose|do i have|what disease|prescribe|what dose|dosage|increase my dose|stop taking|start taking|is it safe|safe or unsafe|normal or abnormal|interpret my lab|interpret scan|do i have cancer|low testosterone|am i pregnant|contraception failed|baby delayed)\b/i.test(text);
}
...
export function enforceAssistantSafetyRules(text: string, mode: AssistantMode) {
  ...
  if (riskCategory === "blocked_medical_advice") {
    return {
      blocked: true,
      message: "I can help you organize logs, summaries, and questions, but I cannot diagnose, prescribe, dose, interpret results, or decide whether something is safe or unsafe.",
      riskCategory
    };
  }
...
export function buildSafetyFooter(riskCategory: AssistantRiskCategory, mode: AssistantMode) {
  if (riskCategory === "emergency_or_urgent") return EMERGENCY_FOOTER;
  if (mode === "baby_child" || riskCategory === "baby_child") return `${SENSITIVE_FOOTER} This does not replace a pediatrician or clinic.`;
  if (mode === "pregnancy" || riskCategory === "pregnancy") return `${SENSITIVE_FOOTER} This does not replace your doctor, midwife, nurse, or clinic.`;
  if (mode === "medication_supplement" || riskCategory === "medication_supplement") return `${SENSITIVE_FOOTER} Always follow your label and healthcare professional's instructions.`;
  if (riskCategory === "sensitive_health") return SENSITIVE_FOOTER;
  return GENERAL_FOOTER;
}

export async function validateAssistantCanAccessData(categories: AssistantDataCategory[], profileId = LOCAL_PROFILE_ID) {
  const permissionCategories = new Set<PermissionCategory>();

  categories.forEach((category) => permissionCategories.add(toPermissionCategory(category)));

  const checks = await Promise.all(Array.from(permissionCategories).map((category) => canViewProfileRealm(profileId, category)));

  return checks.every(Boolean);
}

export async function handleAssistantPrompt(input: { mode: AssistantMode; prompt: string; profileId?: string }): Promise<AssistantRequestResult> {
  ...
  const sourceCards = await getTrustedSourceCardsForTopic(input.prompt, input.mode);
  ...
  const allowed = await validateAssistantCanAccessData(dataCategories, profileId);
  const consented = await Promise.all(dataCategories.map((category) => checkAssistantPermission(category, profileId)));
  ...
  const draft = actionType === "answer" || actionType === "summarize"
    ? undefined
  ...
  return {
    ...
    message,
    riskCategory: safety.riskCategory,
    safetyFooter: draft ? DRAFT_FOOTER : buildSafetyFooter(safety.riskCategory, input.mode),
    sourceCards
  };
}

export async function createAssistantDraft(input: Omit<AssistantDraft, "createdAt" | "id" | "status" | "updatedAt" | "userId">) {
  const now = new Date().toISOString();
...
export async function updateAssistantDraft(id: string, partial: Partial<AssistantDraft>) {
  const drafts = await readJsonArray<AssistantDraft>(DRAFTS_KEY);
  const updated = drafts.map((draft) =>
    draft.id === id ? { ...draft, ...partial, status: partial.status ?? "edited", updatedAt: new Date().toISOString() } : draft
  );
...
export async function confirmAssistantDraft(id: string) {
  const draft = (await readJsonArray<AssistantDraft>(DRAFTS_KEY)).find((item) => item.id === id);
...
export async function getAssistantDrafts(status?: AssistantDraft["status"]) {
  ...
  return drafts.filter((draft) => !status || draft.status === status).sort(sortNewest);
}

export async function saveDraftToTargetRealm(draft: AssistantDraft): Promise<{ id?: string } | null> {
  ...
  switch (draft.targetRealm) {
    case "nutrition":
      return createNutritionEntry({
        ...
        mealGroup: "breakfast",
        notes: String(payload.notes ?? "Assistant-created draft confirmed by user."),
    ...
    case "pregnancy":
      return createPregnancyQuestion({
        category: "symptoms",
        question: String(payload.question ?? "Question for my healthcare professional"),
        status: "draft"
      });
    case "baby_child": {
    ...
    case "medication_supplement":
      return createDoseLog({
        ...
        itemType: String(payload.itemType ?? "medication") === "supplement" ? "supplement" : "medication",
        notes: String(payload.notes ?? "Assistant-created draft confirmed by user."),
...
export async function summarizeTodayHealth() {
  const [nutrition, fitness] = await Promise.all([getTodayNutritionSummary(), getTodayFitnessSummary()]);
...
export async function summarizeWorkout() {
  ...
  return summary.latestWorkout ? `Latest workout: ${summary.latestWorkout.title}.` : "No workout logged today.";
}
...
export async function summarizePregnancyPrep() {
  ...
  return profile?.status === "active" ? "Pregnancy Mode is active. I can help prepare draft questions for your appointment." : "Pregnancy Mode is not active.";
}

export async function summarizeRecords() {
  const records = await getHealthRecords();
  return records.length ? `${records.length} records saved. I can help prepare a draft summary without interpreting results.` : "No records saved yet.";
}
...
export async function getTrustedSourceCardsForTopic(text: string, mode: AssistantMode): Promise<AssistantSourceCard[]> {
  const cards = await getGovernedSourceCardsForTopic(text, modeToContentRealm(mode));
...
export async function answerFromTrustedSourceCards(text: string, mode: AssistantMode) {
...
export async function createAssistantAuditLog(input: Omit<AssistantAuditLog, "createdAt" | "id" | "userId"> & { userId?: string }) {
...
export async function getAssistantAuditLogs(profileId = LOCAL_PROFILE_ID) {
  return (await readJsonArray<AssistantAuditLog>(AUDIT_KEY)).filter((log) => log.profileId === profileId);
}

function inferActionType(mode: AssistantMode, text: string): AssistantActionType {
  if (/\bquestion|ask|doctor|midwife|pharmacist|pediatrician\b/i.test(text)) return "prepare_questions";
  if (/\bsummarize|summary|show me|today|week\b/i.test(text)) return "summarize";
  if (/\bremind|appointment|calendar\b/i.test(text) || mode === "calendar_helper") return "create_draft_reminder";
  if (/\bnote|record|document\b/i.test(text) || mode === "records_helper") return "create_draft_note";
...
function getTargetRealm(mode: AssistantMode, text: string) {
  ...
  if (mode === "calendar_helper" || /\bremind|appointment\b/i.test(text)) return "calendar";
  ...
  if (/\bfood|meal|breakfast|lunch|dinner|toast|egg\b/i.test(text)) return "nutrition";
  if (/\bworkout|pushup|run|walk|sets?\b/i.test(text)) return "workout";
  return "records";
}

function parseDraftPayload(text: string, mode: AssistantMode, actionType: AssistantActionType): Record<string, unknown> {
  if (actionType === "prepare_questions") {
    return { question: buildQuestions(text, mode === "pregnancy" ? "midwife" : mode === "baby_child" ? "pediatrician" : "doctor")[0] };
  }

  if (actionType === "create_draft_reminder") {
    return { dueAt: new Date().toISOString(), notes: text, title: extractTitle(text, "Assistant reminder") };
  }

  if (mode === "food_logger" || getTargetRealm(mode, text) === "nutrition") {
    return { calories: 0, foodName: extractFoodName(text), notes: text, quantity: extractFirstNumber(text) || 1, unit: "serving" };
  }

  if (mode === "workout_logger") {
    return { durationMinutes: 0, notes: text, title: extractTitle(text, "Workout log") };
  }

  if (mode === "baby_child") {
    return { amountMl: extractFirstNumber(text), logType: /\bdiaper\b/i.test(text) ? "diaper" : "feeding", notes: text };
  }

  if (mode === "womens_health") {
    return { date: toNutritionDateKey(new Date()), notes: text, symptom: /\bcramp/i.test(text) ? "cramps" : undefined };
  }

  if (mode === "medication_supplement") {
    return { itemName: extractTitle(text, "Medication or supplement"), itemType: /\bsupplement\b/i.test(text) ? "supplement" : "medication", notes: text };
  }

  return { notes: text, title: extractTitle(text, "Assistant note") };
}

function getDataCategoriesForMode(mode: AssistantMode, text: string): AssistantDataCategory[] {
  switch (mode) {
    case "food_logger":
      return ["nutrition"];
    case "workout_logger":
    ...
    default:
      return ["nutrition", "workout", "calendar"].filter((category) => category !== undefined) as AssistantDataCategory[];
  }
}
...
function toPermissionCategory(category: AssistantDataCategory): PermissionCategory {
  if (category === "medication_supplements") return "medication";
  return getPermissionCategoryForWidget(category === "nutrition" ? "food_log" : category === "workout" ? "workout" : category === "calendar" ? "today_reminders" : category === "records" ? "recent_record" : category === "baby_child" ? "baby_today" : category === "mens_health" ? "mens_health_check_in" : category === "pregnancy" ? "pregnancy_week" : category === "womens_health" ? "cycle_day" : "weight");
}

async function buildAssistantSummary(mode: AssistantMode, text: string, sourceCards: AssistantSourceCard[]) {
  ...
  if (/trusted|source|learn|explain/i.test(text)) return sourceCards.length ? answerFromTrustedSourceCards(text, mode) : fallbackToProfessionalConfirmation();
...
function modeToContentRealm(mode: AssistantMode): HealthContentRealm | undefined {
  switch (mode) {
    case "food_logger":
      return "nutrition";
    case "workout_logger":
...
function toAssistantSourceCard(card: TrustedHealthContentCard): AssistantSourceCard {
  return {
...
function extractTitle(text: string, fallback: string) {
  return text.replace(/\b(log|add|remind me|mark)\b/gi, "").trim().slice(0, 80) || fallback;
}
