...
import {
  getPregnancyAppointments as getLegacyPregnancyAppointments,
  getPregnancyProfile as getLegacyPregnancyProfile,
  getPregnancySymptomLogs as getLegacyPregnancySymptomLogs,
...
const PREGNANCY_APPOINTMENTS_KEY =
  "family_health_phase15b_pregnancy_appointments";
...
const PREGNANCY_SHARE_PERMISSIONS_KEY =
  "family_health_phase15b_pregnancy_share_permissions";
...
export function isPregnancyWidget(widgetKey: WidgetKey) {
  return PREGNANCY_WIDGET_KEYS.includes(
    widgetKey as (typeof PREGNANCY_WIDGET_KEYS)[number],
  );
}
...
export async function enablePregnancyMode(
  input: Partial<PregnancyProfile> = {},
) {
  ...
  const profile: PregnancyProfile = {
    ...
    dateBasis:
      input.dateBasis ?? current?.dateBasis ?? inferDateBasis(input, current),
...
export async function getPregnancyProfile(
  ...
  const stored = (
    await readJsonArray<PregnancyProfile>(PREGNANCY_PROFILES_KEY)
  ...
  const legacyProfile: PregnancyProfile = {
    ...
    dateBasis: legacy.estimatedDueDate
      ? "estimated_due_date"
      : legacy.lastPeriodStartDate
...
async function savePregnancyProfile(profile: PregnancyProfile) {
  const profiles = await readJsonArray<PregnancyProfile>(
    PREGNANCY_PROFILES_KEY,
  );
  await writeJsonArray(PREGNANCY_PROFILES_KEY, [
    profile,
    ...profiles.filter((item) => item.profileId !== profile.profileId),
  ]);
  return profile;
}
...
export function calculatePregnancyAgeFromDueDate(estimatedDueDate?: string) {
  ...
  const ageDays = Math.max(
    0,
    Math.floor((today.getTime() - pregnancyStart.getTime()) / 86400000),
  );
...
export function calculateTrimester(
  weekNumber?: number,
): PregnancyWeekSummary["trimester"] {
...
export async function calculatePregnancyWeekSummary(
  profile = undefined as PregnancyProfile | undefined,
): Promise<PregnancyWeekSummary> {
  ...
  const daysUntilDueDate = estimatedDueDate
    ? Math.ceil(
        (parseDate(estimatedDueDate).getTime() -
...
export async function getPregnancyAppointments() {
  ...
  const [stored, legacy] = await Promise.all([
    ...
    getLegacyPregnancyAppointments().catch(() => []),
  ]);
...
export async function updatePregnancyAppointment(
  id: string,
  partial: Partial<PregnancyAppointment>,
) {
  const appointments = await readStoredAppointments();
...
export async function linkPregnancyAppointmentToCalendar(id: string) {
  return updatePregnancyAppointment(id, {
    notes: "Linked to Calendar / Timeline.",
  });
}

export async function linkPregnancyAppointmentToRecords(
  id: string,
  recordIds: string[] = [],
) {
...
export async function createPregnancySymptomLog(
  input: Omit<
    PregnancySymptomLog,
    | "createdAt"
    | "id"
...
export async function getPregnancySymptomsByDate(date: string) {
  return (
    await getPregnancySymptomsByRange(parseDate(date), parseDate(date))
  ).filter((log) => log.loggedAt.slice(0, 10) === date.slice(0, 10));
}

export async function getPregnancySymptomsByRange(
  ...
  const [stored, legacy] = await Promise.all([
    readStoredSymptoms(),
    getLegacyPregnancySymptomLogs().catch(() => []),
  ]);
  const legacyMapped = legacy.map<PregnancySymptomLog>((log) => ({
    ...
    profileId: LOCAL_PROFILE_ID,
    severity:
      log.severity === "urgent" || log.severity === "strong"
        ? "severe"
        : log.severity,
...
export async function updatePregnancySymptomLog(
  id: string,
  partial: Partial<PregnancySymptomLog>,
) {
  const logs = await readStoredSymptoms();
...
export async function deletePregnancySymptomLog(id: string) {
  const logs = await readStoredSymptoms();
  await writeJsonArray(
    PREGNANCY_SYMPTOMS_KEY,
    logs.filter((log) => log.id !== id),
  );
}

export async function createPregnancyQuestion(
  input: Omit<
    PregnancyQuestion,
    | "createdAt"
    | "id"
    ...
    | "updatedAt"
    | "userId"
  > & { status?: PregnancyQuestion["status"] },
) {
  const profile = await ensurePregnancyProfile();
  const now = new Date().toISOString();
...
export async function updatePregnancyQuestion(
  id: string,
  partial: Partial<PregnancyQuestion>,
) {
  const questions = await readStoredQuestions();
  const updated = questions.map((question) =>
    question.id === id
      ? { ...question, ...partial, updatedAt: new Date().toISOString() }
      : question,
  );
...
export async function markPregnancyQuestionAnswered(
  id: string,
  answerNotes?: string,
) {
  return updatePregnancyQuestion(id, {
...
export async function getPregnancyNutritionSummary() {
  ...
  return summary.foodLogCount
    ? `${summary.foodLogCount} meal logs, ${Math.round(summary.waterMl)} ml water`
...
export async function getPregnancyWorkoutSummary() {
  ...
  return summary.latestWorkout
    ...
    : `${summary.activeMinutesToday ?? 0} active minutes logged`;
}

export async function getPregnancyRecordsSummary() {
  const summary = await getRecordsOverviewSummary();
  return summary.recentRecords.length
    ? `${summary.recentRecords.length} recent records`
    : "No records added yet";
}

export async function generatePregnancyCalendarEvents(
  startDate: Date,
  endDate: Date,
) {
  const profile = await getPregnancyProfile();
  if (!profile || profile.status !== "active")
    return [] as PregnancyCalendarOverlay[];
  ...
  appointments
    .filter((item) => isDateInRange(item.scheduledAt.slice(0, 10), start, end))
    .forEach((appointment) => {
      overlays.push(
        makeOverlay(
          appointment.scheduledAt.slice(0, 10),
          appointmentTypeToEvent(appointment.appointmentType),
          appointment.title,
          "#a855f7",
          appointment.id,
          profile.profileId,
  ...
  questions
    .filter((question) =>
      isDateInRange(question.createdAt.slice(0, 10), start, end),
    )
    .forEach((question) => {
      overlays.push(
        makeOverlay(
  ...
  if (
    week.estimatedDueDate &&
    isDateInRange(week.estimatedDueDate, start, end)
  ) {
    overlays.push(
      makeOverlay(
        week.estimatedDueDate,
...
export async function getPregnancyOverlaysForViewer(
  startDate: Date,
  endDate: Date,
) {
...
export async function createPregnancySharePermission(
  input: Omit<PregnancySharePermission, "createdAt" | "id" | "updatedAt">,
) {
  const now = new Date().toISOString();
  ...
  const permissions = await readJsonArray<PregnancySharePermission>(
    PREGNANCY_SHARE_PERMISSIONS_KEY,
  );
  await writeJsonArray(PREGNANCY_SHARE_PERMISSIONS_KEY, [
    permission,
    ...permissions.filter(
      (item) =>
        !(
          item.ownerProfileId === input.ownerProfileId &&
...
export async function updatePregnancySharePermission(
  id: string,
  partial: Partial<PregnancySharePermission>,
) {
  const permissions = await readJsonArray<PregnancySharePermission>(
    PREGNANCY_SHARE_PERMISSIONS_KEY,
  );
  const updated = permissions.map((permission) =>
    permission.id === id
      ? { ...permission, ...partial, updatedAt: new Date().toISOString() }
      : permission,
  );
...
export async function canViewPregnancyCategory(
  category: PregnancySharePermission["category"],
) {
  const profile = await getPregnancyProfile();
  ...
  return permissions.some(
    (permission) =>
      permission.ownerProfileId === profile.profileId &&
      permission.category === category &&
      permission.permissionLevel !== "none",
  );
}

export async function filterPregnancyDataByPermission<T>(
  items: T[],
  category: PregnancySharePermission["category"],
) {
...
export async function getTrustedPregnancyLearnCards(): Promise<
  PregnancyTrustedLearnCard[]
> {
  const disclaimer =
    "This card is educational and does not replace advice from a doctor, midwife, nurse, clinic, pharmacist, or healthcare professional.";
  return [
    {
      ...
      sourceName: "ACOG",
      summary:
        "Pregnancy dating and estimated due dates are commonly counted from the first day of the last menstrual period. Confirm dating with your healthcare professional.",
    ...
    },
    {
      ...
      sourceName: "NHS",
      summary:
        "Antenatal care can include appointments, checks, screening tests, and discussions with a midwife or doctor.",
    ...
    },
    {
      ...
      sourceName: "NHS",
      summary:
        "NHS week-by-week content can help you prepare questions about baby development and pregnancy preparation.",
    ...
    },
    {
      ...
      sourceName: "CDC",
      summary:
        "Vaccine information should be discussed with a healthcare professional. This app stores questions and reminders only.",
    ...
    },
    {
      ...
      sourceName: "NICHD",
      summary:
        "Pregnancy is often described in trimesters. This app uses trimester labels only as an estimate based on entered dates.",
...
export async function saveTrustedPregnancySourceContent(
  card: PregnancyTrustedLearnCard,
) {
  return card;
}
...
export async function calculatePregnancyWidgetValue(widgetKey: WidgetKey) {
  ...
  switch (widgetKey) {
    case "pregnancy_week":
      return profile?.status === "active" && week.weekNumber
    ...
    case "pregnancy_privacy_status":
      return profile?.privacy === "shared_selected"
        ? "Shared selected"
        : "Private";
    default:
      return "Pregnancy";
...
function getEstimatedDueDate(profile: PregnancyProfile) {
  ...
  if (
    profile.dateBasis === "last_menstrual_period" &&
    profile.lastMenstrualPeriodDate
  ) {
    return calculateEstimatedDueDateFromLmp(profile.lastMenstrualPeriodDate);
  }
  if (
    (profile.dateBasis === "conception_date" ||
      profile.dateBasis === "ivf_date") &&
    (profile.conceptionDate || profile.ivfDate)
  ) {
    return toDateKey(
      addDays(parseDate(profile.conceptionDate ?? profile.ivfDate ?? ""), 266),
    );
  }
  return undefined;
}
...
function inferDateBasis(
  input: Partial<PregnancyProfile>,
  current?: PregnancyProfile | null,
): PregnancyDateBasis {
  if (input.estimatedDueDate ?? current?.estimatedDueDate)
    return "estimated_due_date";
  if (input.lastMenstrualPeriodDate ?? current?.lastMenstrualPeriodDate)
...
function appointmentTypeToEvent(
  type: PregnancyAppointment["appointmentType"],
): PregnancyEventType {
...
>(items: T[]) {
  return [...items].sort(
    (left, right) =>
      ...
      new Date(
        left.loggedAt ?? left.scheduledAt ?? left.updatedAt ?? left.createdAt,
      ).getTime(),
  );
}
