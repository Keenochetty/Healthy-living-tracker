...
function sortByLoggedAt<T extends { loggedAt?: string; createdAt: string }>(items: T[]) {
  return [...items].sort(
    (left, right) =>
...
export async function updateChildProfile(
  ...
  const updatedChildren = children.map((child) =>
    child.id === childId ? { ...child, ...partial, updatedAt: new Date().toISOString() } : child
  );
...
export async function deleteChildProfile(childId: string) {
  ...
  await writeJsonArray(CHILD_PROFILES_KEY, children.filter((item) => item.id !== childId));
...
export async function updateFeedLog(
  ...
  const logs = await readJsonArray<BabyFeedLog>(FEED_LOGS_KEY);
  const updatedLogs = logs.map((log) => (log.id === logId ? { ...log, ...partial } : log));
...
export async function deleteFeedLog(logId: string) {
  ...
  await writeJsonArray(FEED_LOGS_KEY, logs.filter((log) => log.id !== logId));
}
...
export async function deleteChildSleepLog(logId: string) {
  ...
  await writeJsonArray(SLEEP_LOGS_KEY, logs.filter((log) => log.id !== logId));
}
...
export async function addDiaperLog(input: Omit<DiaperLog, "id" | "loggedAt" | "createdAt">) {
  const now = new Date().toISOString();
  const log: DiaperLog = {
    ...input,
    createdAt: now,
...
export async function updateMilestone(
  ...
  return updatedMilestones.find((milestone) => milestone.id === milestoneId) ?? null;
}

export async function deleteMilestone(milestoneId: string) {
  const milestones = await readJsonArray<ChildMilestone>(MILESTONES_KEY);

  await writeJsonArray(MILESTONES_KEY, milestones.filter((item) => item.id !== milestoneId));
}
...
export async function updateVaccinationRecord(
  ...
  const updatedRecords = records.map((record) =>
    record.id === recordId ? { ...record, ...partial, updatedAt: new Date().toISOString() } : record
  );
...
export async function deleteVaccinationRecord(recordId: string) {
  ...
  await writeJsonArray(VACCINATIONS_KEY, records.filter((item) => item.id !== recordId));
}
...
export async function getChildSummary(childId: string): Promise<ChildSummary | null> {
  ...
  const [latestFeed, latestSleep, latestDiaper, latestGrowth, milestones, records] =
    await Promise.all([
...
export async function getAllChildSummaries() {
  const children = await getChildProfiles();
  const summaries = await Promise.all(children.map((child) => getChildSummary(child.id)));

  return summaries.filter((summary): summary is ChildSummary => Boolean(summary));
}
