...
export function isBabyWidget(
  widgetKey: WidgetKey,
): widgetKey is (typeof BABY_WIDGET_KEYS)[number] {
  return BABY_WIDGET_KEYS.includes(
    widgetKey as (typeof BABY_WIDGET_KEYS)[number],
  );
}
...
export async function updateBabyChildProfile(
  childProfileId: string,
  partial: Partial<ChildProfile>,
) {
...
export async function getBabyFeedingLogsByDate(
  childProfileId: string,
  date: string,
) {
  return (await getFeedLogs(childProfileId)).filter(
    (log) => log.loggedAt.slice(0, 10) === date.slice(0, 10),
  );
}

export async function getBabyFeedingLogsByRange(
  childProfileId: string,
  startDate: Date,
  endDate: Date,
) {
  return (await getFeedLogs(childProfileId)).filter((log) =>
    isWithinRange(log.loggedAt, startDate, endDate),
  );
}

export async function updateBabyFeedingLog(
  logId: string,
  partial: Partial<BabyFeedingLog>,
) {
  return updateFeedLog(logId, {
    ...partial,
    updatedAt: new Date().toISOString(),
  });
}
...
export async function deleteBabyFeedingLog(logId: string) {
  const logs = await readJsonArray<BabyFeedingLog>(
    "family_health_child_feed_logs",
  );
  await AsyncStorage.setItem(
    "family_health_child_feed_logs",
    JSON.stringify(logs.filter((log) => log.id !== logId)),
  );
}

export async function calculateFeedingSummary(
  childProfileId: string,
  ...
  return {
    count: logs.length,
    latest,
    totalAmountMl: logs.reduce(
      (sum, log) => sum + (log.finishedAmountMl ?? log.offeredAmountMl ?? 0),
      0,
    ),
  };
}
...
export async function createBabySleepLog(input: {
  ...
  const duration =
    ...
    calculateMinutes(input.startedAt, input.endedAt) ??
    1;
  ...
  const logs = await readJsonArray<import("@/types/child").BabySleepLog>(
    "family_health_child_sleep_logs",
  );
...
export async function getBabySleepLogsByDate(
  childProfileId: string,
  date: string,
) {
  return (await getSleepLogs(childProfileId)).filter(
    (log) => log.loggedAt.slice(0, 10) === date.slice(0, 10),
  );
}

export async function getBabySleepLogsByRange(
  childProfileId: string,
  startDate: Date,
  endDate: Date,
) {
  return (await getSleepLogs(childProfileId)).filter((log) =>
    isWithinRange(log.loggedAt, startDate, endDate),
  );
}

export async function updateBabySleepLog(
  logId: string,
  partial: Partial<import("@/types/child").BabySleepLog>,
) {
  const logs = await readJsonArray<import("@/types/child").BabySleepLog>(
    "family_health_child_sleep_logs",
  );
...
export async function calculateBabySleepSummary(
  childProfileId: string,
  date = toDateKey(new Date()),
) {
...
export async function createBabyDiaperLog(input: {
  ...
  const log = await addDiaperLog({
    childId: input.childProfileId,
    diaperType: input.diaperType,
    notes: input.notes,
  });
  const logs = await readJsonArray<DiaperLog>(
    "family_health_child_diaper_logs",
  );
  const updated = logs.map((item) =>
    item.id === log.id
      ? {
...
export async function getBabyDiaperLogsByDate(
  childProfileId: string,
  date: string,
) {
  return (await getDiaperLogs(childProfileId)).filter(
    (log) => log.loggedAt.slice(0, 10) === date.slice(0, 10),
  );
}

export async function getBabyDiaperLogsByRange(
  childProfileId: string,
  startDate: Date,
  endDate: Date,
) {
  return (await getDiaperLogs(childProfileId)).filter((log) =>
    isWithinRange(log.loggedAt, startDate, endDate),
  );
}

export async function updateBabyDiaperLog(
  logId: string,
  partial: Partial<DiaperLog>,
) {
  const logs = await readJsonArray<DiaperLog>(
    "family_health_child_diaper_logs",
  );
...
export async function createBabyGrowthLog(input: {
  ...
  const logs = await readJsonArray<GrowthMeasurement>(
    "family_health_child_growth_logs",
  );
  const updated = logs.map((item) =>
    item.id === log.id
      ? {
...
export async function updateBabyGrowthLog(
  logId: string,
  partial: Partial<GrowthMeasurement>,
) {
  const logs = await readJsonArray<GrowthMeasurement>(
    "family_health_child_growth_logs",
  );
...
export async function prepareWhoGrowthChartData(childProfileId: string) {
  return {
    ...
    message:
      "Reference curves are not shown until verified source data is implemented correctly.",
  };
}

export async function getMilestoneChecklistByAge(
  childProfileId: string,
  ageMonths = 2,
) {
  return getSeedMilestones()
    .filter(
      (milestone) =>
        milestone.childProfileId === "seed" &&
        milestone.ageCheckpointMonths <= ageMonths + 1,
    )
...
export async function updateBabyMilestoneLog(
  milestoneId: string,
  partial: Parameters<typeof updateMilestone>[1],
) {
...
export async function getMilestoneProgressSummary(childProfileId: string) {
  ...
  return {
    ...
    observedCount: logs.filter(
      (log) => !log.status || log.status === "observed",
    ).length,
...
export async function createBabySolidFoodLog(
  input: Omit<
    BabySolidFoodLog,
...
export async function getBabySolidFoodLogs(childProfileId: string) {
  return (await readJsonArray<BabySolidFoodLog>(SOLIDS_KEY))
    ...
    .sort(
      (left, right) =>
        new Date(right.triedAt).getTime() - new Date(left.triedAt).getTime(),
    );
}

export async function updateBabySolidFoodLog(
  id: string,
  partial: Partial<BabySolidFoodLog>,
) {
  const logs = await readJsonArray<BabySolidFoodLog>(SOLIDS_KEY);
  const updated = logs.map((log) =>
    log.id === id
...
export async function getAllergenWatchSummary(childProfileId: string) {
  ...
  const reactionCount = logs.filter((log) =>
    Boolean(log.reactionNote?.trim()),
  ).length;
  return {
    allergenCount: new Set(
      logs.map((log) => log.allergenCategory).filter(Boolean),
    ).size,
    reactionCount,
    triedCount: logs.length,
  };
}

export async function createBabyMedicineLog(
  input: Omit<
    BabyMedicineLog,
    "createdAt" | "id" | "profileId" | "updatedAt" | "userId"
  >,
...
export async function updateBabyMedicineLog(
  id: string,
  partial: Partial<BabyMedicineLog>,
) {
  const logs = await readJsonArray<BabyMedicineLog>(MEDICINE_KEY);
  const updated = logs.map((log) =>
    log.id === id
...
export async function createBabyVaccineRecord(
  input: Omit<VaccinationRecord, "createdAt" | "id" | "updatedAt">,
) {
...
export async function updateBabyVaccineRecord(
  recordId: string,
  partial: Parameters<typeof updateVaccinationRecord>[1],
) {
  return updateVaccinationRecord(recordId, partial);
}

export async function deleteBabyVaccineRecord(recordId: string) {
  const records = await readJsonArray<VaccinationRecord>(
    "family_health_child_vaccinations",
  );
  await AsyncStorage.setItem(
    "family_health_child_vaccinations",
...
export async function linkBabyVaccineToRecords(
  recordId: string,
  documentId?: string,
) {
...
export async function getBabyReportSummary(
  childProfileId: string,
  range: BabyReportSummary["range"] = "today",
): Promise<BabyReportSummary> {
  const { start, end } = getRange(range);
  const [feeds, sleeps, diapers, growth, solids, medicine, vaccines] =
    await Promise.all([
  ...
  return {
    ...
    medicineLogsCount: medicine.filter((log) =>
      isWithinRange(log.loggedAt, start, end),
    ).length,
    range,
    solidsTriedCount: solids.filter((log) =>
      isWithinRange(log.triedAt, start, end),
    ).length,
    totalSleepMinutes: sleeps.reduce(
      (sum, log) => sum + (log.durationMinutes ?? 0),
      0,
    ),
...
export async function getBabyTimelineSummary(childProfileId: string) {
  const events = await generateBabyCalendarEvents(
    addDays(new Date(), -7),
    new Date(),
  );
  return {
    latestEvent: events.find(
...
export async function getBabyCareSummary(childProfileId: string) {
  ...
  return {
    ...
    feeding,
    growth,
    medicineDueCount: medicine.filter((log) => log.status === "due").length,
    milestone,
    sleep,
    solids,
    vaccines,
...
export async function generateBabyCalendarEvents(
  startDate: Date,
  endDate: Date,
): Promise<BabyCalendarEvent[]> {
  ...
  const allEvents = await Promise.all(
    profiles.map(async (profile) => {
      ...
      return [
        ...feeds
          .filter((log) => isWithinRange(log.loggedAt, startDate, endDate))
          .map((log) =>
            makeEvent(
              profile.id,
              log.id,
        ...
        ...milestones
          .filter(
            (log) =>
              log.achievedAt &&
              isWithinRange(log.achievedAt, startDate, endDate),
          )
          .map((log) =>
            makeEvent(
              profile.id,
              log.id,
              log.achievedAt ?? log.createdAt,
              "milestone",
              log.title,
              "#facc15",
            ),
        ...
        ...solids
          .filter((log) => isWithinRange(log.triedAt, startDate, endDate))
          .map((log) =>
            makeEvent(
              profile.id,
              log.id,
...
export async function getBabyEventsForDate(date: string) {
  return await generateBabyCalendarEvents(
    new Date(`${date.slice(0, 10)}T00:00:00`),
    new Date(`${date.slice(0, 10)}T23:59:59`),
  );
}

export async function getBabyEventsForChildByDate(
  childProfileId: string,
  date: string,
) {
  return (await getBabyEventsForDate(date)).filter(
    (event) => event.childProfileId === childProfileId,
  );
}

export async function linkBabyEventsToMainCalendar() {
  return generateBabyCalendarEvents(
    addDays(new Date(), -30),
    addDays(new Date(), 30),
  );
}
...
export async function calculateBabyWidgetValue(widgetKey: WidgetKey) {
  ...
  switch (widgetKey) {
    ...
    case "last_feed":
    case "baby_feed":
      return care.feeding.latest
        ? relativeTime(care.feeding.latest.loggedAt)
        : "No feed";
    case "sleep_today":
      return `${Math.round((care.sleep.totalMinutes / 60) * 10) / 10}h`;
    case "last_sleep":
      return care.sleep.latest
        ? relativeTime(care.sleep.latest.loggedAt)
        : "No sleep";
    case "last_diaper":
      return care.diaper
        ? `${care.diaper.diaperType}, ${relativeTime(care.diaper.loggedAt)}`
        : "No diaper";
    case "weight_latest":
    ...
    case "next_vaccine":
      return (
        care.vaccines.find(
          (record) => record.scheduledDate || record.nextDoseDate,
        )?.vaccineName ?? "No date"
      );
...
export async function pinBabyWidget(widgetKey: WidgetKey) {
  ...
  await AsyncStorage.setItem(
    BABY_WIDGETS_KEY,
    JSON.stringify([...widgets, widgetKey]),
  );
...
export async function unpinBabyWidget(widgetKey: WidgetKey) {
  ...
  await AsyncStorage.setItem(
    BABY_WIDGETS_KEY,
    JSON.stringify(widgets.filter((key) => key !== widgetKey)),
  );
  return getPinnedBabyWidgets();
}
  ...
  return [
    {
      category: "milestones",
      disclaimer:
        "Educational only. Discuss development concerns with a pediatrician or healthcare professional.",
      id: "cdc-milestones",
      ...
      sourceUrl: "https://www.cdc.gov/act-early/milestones-app/index.html",
      summary:
        "CDC milestone checklists track development from 2 months to 5 years and are not a substitute for standardized developmental screening.",
    ...
    },
    {
      ...
      summary:
        "WHO Child Growth Standards provide reference charts for early childhood growth tracking.",
      title: "Child growth standards",
    },
    {
      category: "safe_sleep",
      disclaimer:
        "Safe sleep information is educational. Speak to your pediatrician or clinic if unsure.",
      id: "cdc-safe-sleep",
      lastCheckedAt: "2026-06-04",
      sourceOrganization: "CDC",
      sourceUrl:
      ...
      summary:
        "CDC safe sleep education includes back sleeping, a firm flat surface, and keeping soft bedding out of the sleep area.",
    ...
    },
    {
      ...
      summary:
        "HealthyChildren explains AAP safe sleep policy for parents and caregivers.",
      title: "AAP safe sleep policy explained",
    },
    {
      category: "vaccines",
      disclaimer:
        "Use this to record vaccine information from your clinic card or healthcare provider.",
      id: "cdc-vaccine-records",
      lastCheckedAt: "2026-06-04",
      sourceOrganization: "CDC",
      sourceUrl: "https://www.cdc.gov/vaccines/parents/index.html",
      summary:
        "Trusted vaccine information should be reviewed with your clinic or healthcare professional.",
      title: "Vaccine record keeping",
    },
    {
      category: "care",
      disclaimer:
        "For organization only. Discuss care questions with your clinic or healthcare professional.",
      id: "clinic-questions-source-pending",
      lastCheckedAt: "Source to be added",
      sourceOrganization: "Source to be added",
      sourceUrl: "",
      summary:
        "Keep a short list of questions you want to discuss during a clinic visit.",
...
function toLegacyFeedType(type: BabyFeedingType): FeedType {
  ...
  if (type === "bottle_breast_milk" || type === "pumping")
...
function inferProfileType(dateOfBirth?: string): ChildProfile["profileType"] {
  ...
  const months = Math.floor(
    (Date.now() - new Date(dateOfBirth).getTime()) / 2629800000,
  );
