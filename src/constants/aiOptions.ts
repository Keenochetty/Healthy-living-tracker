...
export const AI_JOB_TYPE_OPTIONS: Array<{
  ...
  {
    description: "Create a draft medication reminder from instructions you approve.",
    emoji: "Pill",
...
export const AI_FOOD_DISCLAIMER = "Food and nutrition values are estimates only.";
...
export function getAiJobTypeLabel(jobType: AiJobType) {
  return AI_JOB_TYPE_OPTIONS.find((option) => option.key === jobType)?.label ?? jobType;
}
