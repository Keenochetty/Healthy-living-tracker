...
const FEEDING_TYPES: BabyFeedingType[] = [
  "breastfeeding",
  "bottle_formula",
  "bottle_breast_milk",
  "mixed",
...
export default function BabyChildRealm() {
  ...
  const loadRealm = useCallback(async () => {
    ...
    const nextSelectedId =
      selectedChildId ?? asParam(params.childId) ?? nextProfiles[0]?.id;
    const selected =
  ...
  if (isLoading && !selectedProfile) {
    return (
      <>
        <AppMainLayout
          subtitle="Private baby care tracker"
          title="Baby / Child"
        >
          <BabyLoadingState />
        </AppMainLayout>
  ...
  if (!selectedProfile) {
    return (
      <>
        <AppMainLayout
          subtitle="Private parent/guardian setup"
          title="Baby / Child"
        >
          <SetupState
  ...
  return (
    <>
      <AppMainLayout showHeader={false}>
        ...
        {isCaregiverView ? (
          <AppCard style={styles.caregiverCard}>
            <Text style={styles.caregiverText}>
              You are viewing assigned care information.
            </Text>
          </AppCard>
        ) : null}
          ...
          <>
            <BabyOverview
              care={{
                ...
                dirtyDiaperCount: data.diaperLogs.filter(
                  (log) =>
                    log.diaperType === "dirty" || log.diaperType === "mixed",
                ).length,
                feedCount: data.careSummary?.feeding.count ?? 0,
                feedTotalMl: data.careSummary?.feeding.totalAmountMl ?? 0,
                growthLatest: data.careSummary?.growth?.weight
                  ? `${data.careSummary.growth.weight} kg`
                  : undefined,
                lastDiaper: data.careSummary?.diaper,
                lastFeed: data.careSummary?.feeding.latest
                  ? {
                      amountMl:
                        data.careSummary.feeding.latest.finishedAmountMl,
                ...
                lastSleep: data.careSummary?.sleep.latest
                  ? {
                      durationMinutes:
                        data.careSummary.sleep.latest.durationMinutes,
                ...
                wetDiaperCount: data.diaperLogs.filter(
                  (log) =>
                    log.diaperType === "wet" || log.diaperType === "mixed",
                ).length,
              }}
        ...
        {activeTab === "growth" ? (
          <GrowthTab data={data} onSheet={setSheetMode} />
        ) : null}
        ...
        <FeedSheet
          key={`${selectedProfile.id}-feed-${sheetMode === "feed"}`}
          childId={selectedProfile.id}
          childName={selectedProfile.displayName}
          onClose={() => setSheetMode(null)}
          onSaved={afterSaved}
          visible={sheetMode === "feed"}
        />
        <SleepSheet
          key={`${selectedProfile.id}-sleep-${sheetMode === "sleep"}`}
          childId={selectedProfile.id}
          childName={selectedProfile.displayName}
          onClose={() => setSheetMode(null)}
          onSaved={afterSaved}
          visible={sheetMode === "sleep"}
        />
        <DiaperSheet
          key={`${selectedProfile.id}-diaper-${sheetMode === "diaper"}`}
          childId={selectedProfile.id}
          childName={selectedProfile.displayName}
          onClose={() => setSheetMode(null)}
          onSaved={afterSaved}
          visible={sheetMode === "diaper"}
        />
        <GrowthSheet
          key={`${selectedProfile.id}-growth-${sheetMode === "growth"}`}
          childId={selectedProfile.id}
          childName={selectedProfile.displayName}
          onClose={() => setSheetMode(null)}
          onSaved={afterSaved}
          visible={sheetMode === "growth"}
        />
        <SolidFoodSheet
          key={`${selectedProfile.id}-solid-${sheetMode === "solid"}`}
          childId={selectedProfile.id}
          childName={selectedProfile.displayName}
          onClose={() => setSheetMode(null)}
          onSaved={afterSaved}
          visible={sheetMode === "solid"}
        />
        <MedicineSheet
          key={`${selectedProfile.id}-medicine-${sheetMode === "medicine"}`}
          childId={selectedProfile.id}
          childName={selectedProfile.displayName}
          onClose={() => setSheetMode(null)}
          onSaved={afterSaved}
          visible={sheetMode === "medicine"}
        />
        <VaccineSheet
          key={`${selectedProfile.id}-vaccine-${sheetMode === "vaccine"}`}
          childId={selectedProfile.id}
          childName={selectedProfile.displayName}
          onClose={() => setSheetMode(null)}
          onSaved={afterSaved}
          visible={sheetMode === "vaccine"}
        />
        <NoteSheet
          key={`${selectedProfile.id}-note-${sheetMode === "note"}`}
          childName={selectedProfile.displayName}
          onClose={() => setSheetMode(null)}
          onSaved={afterSaved}
          profile={selectedProfile}
...
function FeedingTab({
  ...
  return (
    <SectionFrame
      button="Log feed"
      empty={
        !data.feedingLogs.length
          ? {
              message: "Log the first feed when you are ready.",
              title: "Start feeding tracking",
            }
          : undefined
      }
    ...
    >
      <View style={styles.metricGrid}>
        <MetricCard
          label="Last feed"
          value={
            data.careSummary?.feeding.latest
              ? formatAgo(data.careSummary.feeding.latest.loggedAt)
              : "Start today"
          }
        />
        <MetricCard
          label="Feed count"
          value={`${data.careSummary?.feeding.count ?? 0}`}
        />
        <MetricCard
          label="Bottle total"
          value={`${data.careSummary?.feeding.totalAmountMl ?? 0} ml`}
        />
        <MetricCard
          label="Notes"
          value={
            data.feedingLogs.some((log) => log.notes) ? "Added" : "Optional"
          }
        />
      </View>
      <List
        items={data.feedingLogs.map(
          (log) =>
            `${formatValue(log.feedingType ?? log.feedType)} - ${log.finishedAmountMl ?? log.durationMinutes ?? ""}${log.finishedAmountMl ? " ml" : " min"} - ${formatAgo(log.loggedAt)}`,
        )}
      />
    </SectionFrame>
  );
  ...
  return (
    <SectionFrame
      ...
      empty={
        !data.sleepLogs.length
          ? {
              message: "Add naps or night sleep when it helps.",
    ...
    >
      <View style={styles.metricGrid}>
        <MetricCard
          label="Sleep today"
          value={formatMinutes(data.careSummary?.sleep.totalMinutes ?? 0)}
        />
        <MetricCard
          label="Nap count"
          value={`${data.careSummary?.sleep.napCount ?? 0}`}
        />
        <MetricCard
          label="Last sleep"
          value={
            data.careSummary?.sleep.latest
              ? formatAgo(data.careSummary.sleep.latest.loggedAt)
              : "Start today"
          }
        />
      </View>
      <ChartCard
        title="Sleep blocks"
        values={data.sleepLogs.map((log) =>
          Math.min(1, log.durationMinutes / 180),
        )}
      />
      <List
        items={data.sleepLogs.map(
          (log) =>
            `${formatValue(log.sleepType ?? "unknown")} - ${formatMinutes(log.durationMinutes)} - ${formatAgo(log.loggedAt)}`,
        )}
      />
    </SectionFrame>
  );
...
function DiapersTab({
  ...
  return (
    <SectionFrame
      button="Log diaper"
      empty={
        !data.diaperLogs.length
          ? {
              message: "Add wet, dirty, or mixed diaper notes.",
              title: "Track diapers if helpful",
            }
          : undefined
      }
    ...
    >
      <View style={styles.metricGrid}>
        <MetricCard
          label="Last diaper"
          value={
            data.careSummary?.diaper
              ? `${formatValue(data.careSummary.diaper.diaperType)} - ${formatAgo(data.careSummary.diaper.loggedAt)}`
              : "Start today"
          }
        />
      ...
      <ChartCard
        title="Diaper count"
        values={DIAPER_TYPES.map(
          (type) =>
            data.diaperLogs.filter((log) => log.diaperType === type).length /
            Math.max(1, data.diaperLogs.length),
        )}
      />
      <List
        items={data.diaperLogs.map(
          (log) =>
            `${formatValue(log.diaperType)} - ${formatAgo(log.loggedAt)}${log.notes ? ` - ${log.notes}` : ""}`,
        )}
      />
    </SectionFrame>
  );
...
function GrowthTab({
  ...
  return (
    <BabyGrowthSection logs={data.growthLogs} onAdd={() => onSheet("growth")} />
  );
}
...
function MilestonesTab({
  ...
  const [category, setCategory] =
    useState<MilestoneCategory>("social_emotional");
  const [status, setStatus] = useState<MilestoneStatus>("observed");
  const [ageCheckpointMonths, setAgeCheckpointMonths] = useState(
    getAgeMonths(profile.dateOfBirth) || 2,
  );
...
function SolidsTab({
  ...
  return (
    <SectionFrame
      button="Add food"
      empty={
        !data.solidsLogs.length
          ? {
              message: "Track foods tried, textures, and notes.",
              title: "Start first foods tracking",
            }
          : undefined
      }
    ...
    >
      <View style={styles.metricGrid}>
        <MetricCard
          label="Foods tried"
          value={`${data.careSummary?.solids.triedCount ?? 0}`}
        />
        <MetricCard
          label="Reaction notes"
          value={`${data.careSummary?.solids.reactionCount ?? 0}`}
        />
      </View>
      <SolidsGrid logs={data.solidsLogs} />
      <List
        items={data.solidsLogs
          .slice(0, 8)
          .map(
            (log) =>
              `${log.foodName} - ${log.allergenCategory ?? "Food"} - ${formatValue(log.likedStatus ?? "unknown")}`,
          )}
      />
    </SectionFrame>
  );
  ...
  return (
    <SectionFrame
      ...
      empty={
        !data.medicineLogs.length
          ? {
              message:
                "Medicine logs will appear here when added from label or healthcare instructions.",
              title: "Track medicine carefully",
            }
          : undefined
      }
    ...
    >
      <View style={styles.metricGrid}>
        <MetricCard
          label="Due"
          value={`${data.medicineLogs.filter((log) => log.status === "due").length}`}
        />
        <MetricCard
          label="Taken"
          value={`${data.medicineLogs.filter((log) => log.status === "taken").length}`}
        />
      </View>
      <AppCard style={styles.darkCard}>
        <Text style={styles.darkTitle}>Medicine instructions</Text>
        <Text style={styles.darkMuted}>
          Keep label or healthcare professional instructions with the log. No
          dose calculation is provided.
        </Text>
      </AppCard>
      <List
        items={data.medicineLogs
          .slice(0, 8)
          .map(
            (log) =>
              `${log.medicineName} - ${formatValue(log.status)}${log.doseInstruction ? ` - ${log.doseInstruction}` : ""}`,
          )}
      />
    </SectionFrame>
  );
    ...
    <SectionFrame
      ...
      empty={
        !data.vaccineRecords.length
          ? {
              message:
                "Vaccine records from your clinic card will appear here.",
    ...
    >
      <View style={styles.metricGrid}>
        ...
        <MetricCard
          label="Next date"
          value={
            data.vaccineRecords.find((record) => record.nextDoseDate)
              ?.nextDoseDate ?? "Set up when ready"
          }
        />
      </View>
      <List
        items={data.vaccineRecords
          .slice(0, 8)
          .map(
            (record) =>
              `${record.vaccineName} - ${record.dateReceived ?? record.completedDate ?? record.status}`,
          )}
      />
    </SectionFrame>
  );
...
function RecordsTab({ profile }: { profile: BabyChildProfile }) {
  ...
  return (
    <View style={styles.stack}>
      <AppSection
        title="Records"
        subtitle="Private documents linked to the Records realm."
      />
      <AppCard style={styles.darkCard}>
        <Text style={styles.darkTitle}>
          Baby records for {profile.displayName}
        </Text>
        <Text style={styles.darkMuted}>
          Birth records, clinic cards, vaccine cards, prescriptions, lab
          results, feeding plans, and notes stay private unless shared through
          Family permissions.
        </Text>
      ...
      <View style={styles.recordGrid}>
        {recordTypes.map((type) => (
          <RecordTypeCard key={type} label={type} />
        ))}
      </View>
...
function ReportsTab({ data }: { data: BabyRealmData }) {
  return (
    <View style={styles.stack}>
      <AppSection
        title="Reports"
        subtitle="Compact summaries based on your baby logs."
      />
      <View style={styles.metricGrid}>
        <MetricCard
          label="Feeding summary"
          value={`${data.report?.feedingCount ?? 0} feeds`}
        />
        <MetricCard
          label="Sleep summary"
          value={formatMinutes(data.report?.totalSleepMinutes ?? 0)}
        />
        <MetricCard
          label="Diaper summary"
          value={`${data.report?.diaperCount ?? 0}`}
        />
        <MetricCard
          label="Latest weight"
          value={
            data.report?.latestWeightKg
              ? `${data.report.latestWeightKg} kg`
              : "Start today"
          }
        />
        <MetricCard
          label="Solids tried"
          value={`${data.report?.solidsTriedCount ?? 0}`}
        />
        <MetricCard
          label="Medicine logs"
          value={`${data.report?.medicineLogsCount ?? 0}`}
        />
      </View>
      <ChartCard
        title="Feed timeline"
        values={data.feedingLogs.map(() => 0.65)}
      />
      <ChartCard
        title="Sleep blocks"
        values={data.sleepLogs.map((log) =>
...
function LearnTab({ cards }: { cards: BabyLearnCard[] }) {
  return (
    <View style={styles.stack}>
      <AppSection
        title="Learn"
        subtitle="Trusted source-backed educational cards."
      />
      {cards.map((card) => (
...
function SettingsTab({
  ...
  return (
    <View style={styles.stack}>
      <AppSection
        title="Settings"
        subtitle="Private by default and parent/guardian managed."
      />
      <AppCard style={styles.darkCard}>
        ...
        <View style={styles.chipRow}>
          ...
          <Chip
            ...
            onPress={() => setPrivacy("shared_selected")}
            selected={privacy === "shared_selected"}
          />
        </View>
...
function FeedSheet({
  ...
  const [feedingType, setFeedingType] =
    useState<BabyFeedingType>("bottle_formula");
  ...
  async function save() {
    const isBottle =
      ...
      feedingType === "bottle_breast_milk" ||
      feedingType === "pumping";
  ...
  return (
    ...
    >
      ...
      <ScrollView
        ...
        contentContainerStyle={styles.chipRow}
      >
        {(["left", "right", "both", "not_applicable"] as const).map((value) => (
          <Chip
            key={value}
      ...
      {manualMode ? (
        <TextInput
          ...
          placeholder="Amount ml or minutes"
        ...
        <>
          <PresetChipGroup
            ...
            presets={[60, 90, 120, 150]}
            selectedValue={amountMl}
            suffix="ml"
          />
          <NumberWheelPicker
            max={240}
...
function SleepSheet({
  ...
  const [sleepType, setSleepType] = useState<"nap" | "night" | "unknown">(
    "nap",
  );
  ...
  return (
    ...
    >
      <ScrollView
        ...
        contentContainerStyle={styles.chipRow}
      >
        {(["nap", "night", "unknown"] as const).map((type) => (
          <Chip
            key={type}
            label={formatValue(type)}
            onPress={() => setSleepType(type)}
            selected={sleepType === type}
          />
        ))}
      </ScrollView>
      <PresetChipGroup
...
function GrowthSheet({
  ...
  const [source, setSource] = useState<
    "home" | "clinic" | "pediatrician" | "other"
  >("home");
  ...
  async function save() {
    ...
    await createBabyGrowthLog({
      childProfileId: childId,
      headCircumferenceCm: toNumber(headCircumferenceCm),
  ...
  return (
    ...
    >
      ...
      <ScrollView
        ...
        contentContainerStyle={styles.chipRow}
      >
        {(["home", "clinic", "pediatrician", "other"] as const).map((item) => (
          <Chip
            key={item}
...
function SolidFoodSheet({
  ...
  const [likedStatus, setLikedStatus] = useState<
    "liked" | "neutral" | "disliked" | "unknown"
  >("unknown");
  ...
  async function save() {
    ...
    await createBabySolidFoodLog({
      allergenCategory: allergenCategory || category,
      childProfileId: childId,
      foodName,
      likedStatus,
      notes,
  ...
  return (
    <BabyQuickLogSheet
      ...
      onSave={save}
      saveTitle="Save food"
      title="Add solid food"
      visible={visible}
    >
      <TextInput
        onChangeText={setFoodName}
        placeholder="Food tried"
        placeholderTextColor="#94a3b8"
        style={styles.darkInput}
        value={foodName}
      />
      ...
      >
        {ALLERGENS.map((item) => (
          <Chip
            key={item}
            label={item}
            onPress={() => setAllergenCategory(item)}
            selected={allergenCategory === item}
          />
        ))}
      </ScrollView>
      <ScrollView
        ...
        contentContainerStyle={styles.chipRow}
      >
        {(["liked", "neutral", "disliked", "unknown"] as const).map((item) => (
          <Chip
            key={item}
            label={formatValue(item)}
            onPress={() => setLikedStatus(item)}
            selected={likedStatus === item}
          />
        ))}
      </ScrollView>
      <TextInput
      ...
      <TextInput
        ...
        placeholder="Reaction note optional"
        placeholderTextColor="#94a3b8"
        style={styles.darkInput}
        value={reactionNote}
      />
...
function MedicineSheet({
  ...
  return (
    ...
    >
      ...
      />
      <TextInput
        onChangeText={setDoseInstruction}
        placeholder="Label/instruction note"
        placeholderTextColor="#94a3b8"
        style={styles.darkInput}
...
function VaccineSheet({
  ...
  const [recordSource, setRecordSource] =
    useState<NonNullable<VaccinationRecord["recordSource"]>>("clinic_card");
  const [routeOrSite, setRouteOrSite] =
    useState<NonNullable<VaccinationRecord["routeOrSite"]>>("not_sure");
  const [notes, setNotes] = useState("");
  const [validationMessage, setValidationMessage] = useState("");
  async function save() {
    if (!vaccineName.trim()) {
  ...
  return (
    ...
    >
      ...
      >
        {(
          ...
          <Chip
            key={item}
            label={formatValue(item)}
            onPress={() => setRecordSource(item)}
            selected={recordSource === item}
          />
        ))}
      </ScrollView>
      ...
      >
        {(
          ...
          <Chip
            key={item}
            label={formatValue(item)}
            onPress={() => setRouteOrSite(item)}
            selected={routeOrSite === item}
          />
        ))}
      </ScrollView>
      <Text style={styles.sheetText}>
        Record the site if it appears on the clinic card or was given by your
        healthcare provider.
      </Text>
      <AppCard style={styles.darkCard}>
        <Text style={styles.darkTitle}>Clinic card or photo</Text>
        <Text style={styles.darkMuted}>
          Document linking will connect to Baby Records later.
        </Text>
      </AppCard>
      <QuickNoteField onChangeText={setNotes} value={notes} />
      {validationMessage ? (
      ...
      <Text style={styles.sheetText}>
        ...
        healthcare provider. Vaccine schedules should be confirmed with your
        clinic, nurse, doctor, pharmacist, or healthcare professional.
      </Text>
    </BabyQuickLogSheet>
  );
}
...
function SetupState({
  ...
  onCreated: (profile: BabyChildProfile) => void;
}) {
    ...
    <View style={styles.stack}>
      <AppCard style={styles.heroCard}>
        ...
        <Text style={styles.heroSubtitle}>
          Track feeds, sleep, diapers, growth, milestones, vaccines, and records
          when you are ready.
        </Text>
      </AppCard>
      <AppCard style={styles.darkCard}>
        <TextInput
          ...
          placeholder="Baby or child name"
          placeholderTextColor="#94a3b8"
          style={styles.darkInput}
          value={displayName}
        />
  ...
  return (
    <AppCard style={styles.darkCard}>
      ...
      <Text style={styles.sourceText}>
        {hasSource ? card.sourceOrganization : "Trusted source to be added."}
      </Text>
      <Text style={styles.sourceText}>Last checked: {card.lastCheckedAt}</Text>
      <Text style={styles.sourceText}>
        {hasSource ? "Open source" : "Source to be added"}
      </Text>
...
function ChartCard({ title, values }: { title: string; values: number[] }) {
  ...
  return (
    <AppCard style={styles.darkCard}>
      ...
      <View
        accessibilityLabel={`${title} chart based on logged baby care data`}
        style={styles.barRow}
      >
  ...
  return (
    <AppCard style={styles.darkCard}>
      ...
      <View style={styles.solidsGrid}>
        {(logs.length
          ? logs.slice(0, 12).map((log) => log.foodName)
          : FOOD_CATEGORIES
        ).map((item) => (
          <View key={item} style={styles.solidTile}>
            <Text style={styles.solidText}>{item}</Text>
          </View>
        ))}
      </View>
    </AppCard>
...
function getNextReminder(data: BabyRealmData) {
  ...
  const vaccine = data.vaccineRecords.find(
...
function enrichBabyEvents(
  ...
  return events.map((event) => {
    if (event.type === "feeding") {
      ...
      if (log) {
        const amount = log.finishedAmountMl
          ? ` · ${log.finishedAmountMl} ml`
          : log.durationMinutes
            ? ` · ${formatMinutes(log.durationMinutes)}`
            : "";
        return {
          ...event,
          label: `${formatValue(log.feedingType ?? log.feedType)}${amount}`,
        };
      }
    }
    if (event.type === "sleep") {
      ...
      if (log)
        return {
          ...event,
          label: `${formatValue(log.sleepType ?? "sleep")} · ${formatMinutes(log.durationMinutes)}`,
        };
    }
    ...
    if (event.type === "medicine") {
      ...
      if (log)
        return {
          ...event,
          label: `${log.medicineName} · ${formatValue(log.status)}`,
        };
    }
    return event;
  });
...
function BabyLoadingState() {
  return (
    <View accessibilityLabel="Loading Baby Care" style={styles.stack}>
      {[110, 220, 180].map((height) => (
        <View key={height} style={[styles.loadingCard, { height }]} />
      ))}
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  profileCard: {
    backgroundColor: "#111827",
    borderColor: "rgba(110,231,200,0.35)",
    borderWidth: 1,
  },
