...
const DEFAULT_QUICK_ACTION_ORDER = ["water", "workout", "note", "medication", "baby", "cycle", "food"];
...
export default function CalendarScreen() {
  ...
  const [calendarChromeHeight, setCalendarChromeHeight] = useState(DEFAULT_CALENDAR_CHROME_HEIGHT);
  ...
  const notebookPanResponder = useMemo(
    () => {
      function settleNotebook(velocityY = 0) {
        const shouldOpen =
          velocityY < -0.35 ||
          (velocityY <= 0.35 && notebookDragValueRef.current > CALENDAR_COLLAPSE_DISTANCE * 0.45);
      ...
      return PanResponder.create({
        onMoveShouldSetPanResponderCapture: (_, gestureState) =>
          ...
          (!notebookOpenRef.current || (gestureState.dy > 0 && agendaScrollOffsetRef.current <= 0)),
        onPanResponderGrant: () => {
        ...
        onPanResponderRelease: (_, gestureState) => settleNotebook(gestureState.vy),
  ...
  const monthLabel = useMemo(
    () => new Intl.DateTimeFormat(undefined, { month: "long" }).format(calendarMonth),
    [calendarMonth]
  );
  const yearLabel = calendarMonth.getFullYear();

  const visibleDays = useMemo(
    () => buildMonthDays(monthStart, allReminders, womensOverlayEnabled ? womensOverlays : []),
    [allReminders, monthStart, womensOverlayEnabled, womensOverlays]
  );
  const weekDays = useMemo(
    () => buildWeekDays(selectedDate, allReminders, womensOverlayEnabled ? womensOverlays : []),
    [allReminders, selectedDate, womensOverlayEnabled, womensOverlays]
  );
  const selectedOverlays = womensOverlayEnabled
    ? womensOverlays.filter((overlay) => overlay.date === toDateKey(selectedDate))
    : [];
  const selectedReminders = useMemo(
    () => allReminders.filter((reminder) => isSameDay(new Date(reminder.dueAt), selectedDate)),
    [allReminders, selectedDate]
  );
  const expandedAgendaTop =
    calendarChromeHeight + CALENDAR_LAYER_GAP + CALENDAR_EXPANDED_HEIGHT + NOTEBOOK_LAYER_GAP;
  const collapsedAgendaTop =
    calendarChromeHeight + CALENDAR_LAYER_GAP + CALENDAR_COLLAPSED_HEIGHT + NOTEBOOK_LAYER_GAP;
  ...
  const loadCalendar = useCallback(async (showLoading = false) => {
    ...
    const canShowWomensOverlay = Boolean(womensSettings.trackingEnabled && womensSettings.overlayEnabled);
  ...
  useFocusEffect(
    useCallback(() => {
      loadCalendar(!hasLoadedCalendarRef.current).catch(() => setIsLoading(false));
    }, [loadCalendar])
  );
  ...
  return (
    <AppMainLayout
      safeBottom={false}
      screenStyle={{ ...styles.calendarScreenFrame, backgroundColor: theme.background }}
      scroll={false}
      subtitle="Plan your day"
      title="Calendar"
    >
      <View style={[styles.calendarViewport, { backgroundColor: theme.background }]}>
        <View {...notebookPanResponder.panHandlers} style={[styles.pinnedCalendarArea, { backgroundColor: theme.background }]}>
          <View
            onLayout={(event) => {
          ...
          >
            ...
            {successMessage ? (
              <Pressable accessibilityRole="button" onPress={() => setSuccessMessage("")} style={styles.successToast}>
        ...
        >
          <View style={styles.agendaPinnedHeader}>
            ...
            <View style={styles.agendaHeaderActions}>
              <IconButton label="Add event" muted onPress={() => openScheduler(selectedDate)} small>
              ...
              <IconButton
                ...
                small
              >
                {notebookOpen ? <ChevronDown color="#e2e8f0" size={17} /> : <ChevronUp color="#e2e8f0" size={17} />}
              </IconButton>
            </View>
          </View>
          ...
          >
            ...
            <AppCard style={[
              ...
              <View style={styles.privacyCopy}>
                <Text style={[styles.privacyTitle, !isDarkTheme ? styles.privacyTitleLight : null]}>Private overlay</Text>
                <Text style={[styles.privacyText, !isDarkTheme ? styles.privacyTextLight : null]}>
                  {notebookOpen
...
function DayCell({
  ...
  const labelParts = [
    new Intl.DateTimeFormat(undefined, { day: "numeric", month: "long", weekday: "long" }).format(day.date),
    selected ? "selected" : "",
    isToday ? "today" : "",
    ...
    >
      ...
      <View style={styles.dayIndicators}>
        {topIndicators.map((reminder) => {
          ...
          return <View key={reminder.id} style={[styles.dayDot, { backgroundColor: accent.color }]} />;
        })}
        {day.reminders.length > 4 ? <Text style={styles.moreText}>+{day.reminders.length - 4}</Text> : null}
      </View>
      {overlay ? <View style={[styles.overlayMark, { backgroundColor: getOverlayColor(overlay) }]} /> : null}
    </Pressable>
  );
}
          ...
          >
            ...
            <View style={styles.weekDots}>
              {day.reminders.slice(0, 3).map((reminder) => (
                <View key={reminder.id} style={[styles.dayDot, { backgroundColor: TYPE_ACCENTS[reminder.type].color }]} />
              ))}
              {day.overlays.slice(0, 1).map((overlay) => (
                <View key={overlay.id} style={[styles.dayDot, { backgroundColor: getOverlayColor(overlay) }]} />
              ))}
            </View>
          </Pressable>
        );
...
function SelectedDayPanel({
  ...
  return (
    <View style={styles.selectedPanel}>
      <View style={[styles.agendaSheet, light ? styles.agendaSheetLight : null]}>
        <View style={styles.agendaSheetBody}>
          <View style={styles.agendaInnerContent}>
            {overlays.length ? (
              <View style={[styles.privateOverlay, light ? styles.privateOverlayLight : null]}>
                <View style={styles.privateOverlayHeader}>
                  ...
                  <View style={styles.privateOverlayCopy}>
                    <Text style={[styles.privateOverlayTitle, light ? styles.privateOverlayTitleLight : null]}>Private overlay</Text>
                    <Text style={[styles.privateOverlayText, light ? styles.privateOverlayTextLight : null]}>
                ...
                <View style={styles.overlayList}>
                  {overlays.slice(0, 4).map((overlay) => (
                    <View key={overlay.id} style={styles.overlayPill}>
                      <View style={[styles.dayDot, { backgroundColor: getOverlayColor(overlay) }]} />
                      <Text style={[styles.overlayText, light ? styles.overlayTextLight : null]}>{getOverlayLabel(overlay)}</Text>
                    </View>
                  ))}
                </View>
              </View>
  ...
  return (
    <View style={styles.stack}>
      <AppCard style={styles.todayHero}>
        <View style={styles.sectionHeaderRow}>
          <View>
            ...
            <Text style={styles.darkMuted}>Plan your day from your life calendar and health timeline.</Text>
          </View>
      ...
      <View style={styles.metricGrid}>
        ...
        <MetricCard label="Upcoming" value={`${reminders.filter((item) => new Date(item.dueAt).getTime() > Date.now()).length}`} />
      </View>
    </View>
  );
}
...
function TimelineView({
  ...
  return (
    <View style={styles.stack}>
      <AppSection subtitle="Past and future items grouped by day." title="Timeline" />
      {dateKeys.map((dateKey) => (
        <View key={dateKey} style={styles.timelineGroup}>
          ...
          {grouped[dateKey].map((reminder) => (
            <TimelineRow key={reminder.id} onOpen={() => onOpen(reminder)} reminder={reminder} />
          ))}
        </View>
      ))}
    </View>
...
function ReminderGroups({
  ...
  const now = Date.now();
  const dueNow = reminders.filter((reminder) => reminder.status === "pending" && new Date(reminder.dueAt).getTime() <= now);
  const upcoming = reminders.filter((reminder) => reminder.status === "pending" && new Date(reminder.dueAt).getTime() > now);
  const completed = reminders.filter((reminder) => reminder.status === "completed");
  const snoozedOrMissed = reminders.filter((reminder) => reminder.status === "skipped" || reminder.status === "cancelled");

  return (
    <View style={styles.stack}>
      <ReminderSection onComplete={onComplete} onOpen={onOpen} onSkip={onSkip} reminders={dueNow} title="Due now" />
      <ReminderSection onComplete={onComplete} onOpen={onOpen} onSkip={onSkip} reminders={upcoming.slice(0, 12)} title="Upcoming" />
      <ReminderSection onComplete={onComplete} onOpen={onOpen} onSkip={onSkip} reminders={completed.slice(0, 8)} title="Completed" />
      <ReminderSection onComplete={onComplete} onOpen={onOpen} onSkip={onSkip} reminders={snoozedOrMissed.slice(0, 8)} title="Snoozed / missed" />
      {!reminders.length ? (
        <EmptyState
          actionLabel="Add event"
          onAction={onAddReminder}
...
function QuickActionSheet({
  ...
  const [reorderingActionKey, setReorderingActionKey] = useState<string | null>(null);
  ...
  const actions: Array<{
    ...
    },
    {
      ...
      label: "Cycle",
      onPress: () => setPlaceholderText("This quick log will open here soon."),
      visible: womensOverlayEnabled && enabledModules.includes("pregnancy_cycle")
    },
    {
      key: "food",
      icon: "food",
  ...
  useEffect(() => {
    ...
    AsyncStorage.getItem(QUICK_ACTION_ORDER_KEY)
      .then((storedOrder) => {
        ...
        const validOrder = parsedOrder.filter((key) => DEFAULT_QUICK_ACTION_ORDER.includes(key));
  ...
  const visibleActionKeys = new Set(actions.filter((action) => action.visible).map((action) => action.key));
  const orderedActions = [
    actions[0],
    ...actionOrder
      .map((key) => actions.find((action) => action.key === key))
      .filter((action): action is (typeof actions)[number] => Boolean(action?.visible))
  ];
  ...
  function moveAction(key: string, direction: -1 | 1) {
    setActionOrder((current) => {
      ...
      while (targetIndex >= 0 && targetIndex < next.length && !visibleActionKeys.has(next[targetIndex])) {
      ...
      AsyncStorage.setItem(QUICK_ACTION_ORDER_KEY, JSON.stringify(next)).catch(() => undefined);
      return next;
    });
  }
  ...
  return (
    <Modal animationType="fade" onRequestClose={onClose} transparent visible={visible}>
      <View style={styles.dayActionOverlay}>
        <Pressable accessibilityRole="button" onPress={onClose} style={styles.schedulerBackdrop} />
        <View style={styles.dayActionPopup}>
          <View style={[styles.sectionHeaderRow, styles.dayActionHeader]}>
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.dayActionHint}>Hold and drag an option left or right to reorder it. Event stays first.</Text>
            </View>
          ...
          <View style={styles.dayActionCarouselFrame}>
            <ScrollView
              ...
              onLayout={(event) => setCarouselViewportWidth(event.nativeEvent.layout.width)}
            ...
            {carouselOffset + carouselViewportWidth < carouselContentWidth - 4 ? <CarouselEdgeShade side="right" /> : null}
          </View>
          ...
          {placeholderText ? (
            <Pressable
              ...
              style={[styles.quickSheetPlaceholder, styles.dayActionPlaceholder]}
            >
              <Text style={styles.quickSheetPlaceholderText}>{placeholderText}</Text>
            </Pressable>
          ) : null}
        </View>
      </View>
...
function ReorderableQuickAction({
  ...
  const panResponder = useMemo(
    () =>
      PanResponder.create({
        onMoveShouldSetPanResponder: (_, gestureState) => reorderingRef.current && Math.abs(gestureState.dx) > 10,
        onMoveShouldSetPanResponderCapture: (_, gestureState) => reorderingRef.current && Math.abs(gestureState.dx) > 10,
  ...
  return (
    <View {...panResponder.panHandlers} style={[styles.quickSheetActionFrame, reordering ? styles.quickSheetActionReordering : null]}>
      <Pressable
        accessibilityHint={fixed ? "Event always stays first" : "Long press, then drag left or right to reorder"}
        ...
        style={({ pressed }) => [styles.quickSheetAction, pressed && !reordering ? styles.pressed : null]}
      >
        <View style={styles.quickSheetIcon}>
          <AppIcon color="#10201d" decorative name={action.icon} size={22} strokeWidth={2.4} />
        </View>
        <Text style={styles.quickSheetActionText}>{action.label}</Text>
        <Text style={styles.quickSheetActionMeta}>{fixed ? "Stays first" : reordering ? "Drag left or right" : "Hold + drag"}</Text>
      </Pressable>
    </View>
  );
}
...
function CarouselEdgeShade({ side }: { side: "left" | "right" }) {
  const opacities = side === "left" ? [0.42, 0.22, 0.08, 0] : [0, 0.08, 0.22, 0.42];

  return (
    <View pointerEvents="none" style={[styles.carouselEdgeShade, side === "left" ? styles.carouselEdgeShadeLeft : styles.carouselEdgeShadeRight]}>
      {opacities.map((opacity, index) => (
        <View key={`${side}-${index}`} style={{ backgroundColor: `rgba(15,23,42,${opacity})`, flex: 1 }} />
      ))}
    </View>
  );
}
...
function FilterSheet({
  ...
  const categories = [
    ...
    { label: "Medication", visible: enabledModules.includes("personal_health") },
    { label: "Baby", visible: enabledModules.includes("child_baby") },
    { label: "Records", visible: true },
    { label: "Women's Health", visible: womensOverlayEnabled && enabledModules.includes("pregnancy_cycle") }
          ...
          <View style={styles.filterGrid}>
            {categories.map((category) => (
              <Pressable
                ...
                key={category.label}
                onPress={onClose}
                style={({ pressed }) => [styles.filterChip, pressed ? styles.pressed : null]}
              >
...
function MonthPickerSheet({
  ...
  const months = Array.from({ length: 12 }, (_, month) => ({
    label: new Intl.DateTimeFormat(undefined, { month: "short" }).format(new Date(draftYear, month, 1, 12)),
    month
  }));
      ...
      <View style={styles.quickSheetOverlay}>
        ...
        <View style={styles.quickSheet}>
          ...
          <View style={styles.yearJumpRow}>
            <MonthJumpButton label="-10" onPress={() => setDraftYear((year) => year - 10)} />
            <MonthJumpButton label="-1" onPress={() => setDraftYear((year) => year - 1)} />
            <MonthJumpButton label="This year" onPress={() => setDraftYear(new Date().getFullYear())} />
            <MonthJumpButton label="+1" onPress={() => setDraftYear((year) => year + 1)} />
            <MonthJumpButton label="+10" onPress={() => setDraftYear((year) => year + 10)} />
          </View>
          ...
          <View style={styles.monthGridPicker}>
            {months.map((month) => {
              const selected = draftYear === currentMonth.getFullYear() && month.month === currentMonthIndex;
...
function MonthJumpButton({ label, onPress }: { label: string; onPress: () => void }) {
  return (
    <Pressable accessibilityLabel={label} accessibilityRole="button" onPress={onPress} style={styles.monthJumpButton}>
  ...
  return (
    <Pressable onPress={onOpen} style={({ pressed }) => [styles.reminderRow, pressed ? styles.pressed : null]}>
      ...
      <View style={{ flex: 1 }}>
        ...
        <Text style={styles.reminderMeta}>{formatReminderTime(reminder.dueAt)} - {accent.label}</Text>
      </View>
      <View style={styles.rowActions}>
        {reminder.status === "pending" ? <SmallAction label="Done" onPress={onComplete} /> : null}
        {reminder.status === "pending" ? <SmallAction label="Later" onPress={onSkip} /> : null}
...
function TimelineRow({ onOpen, reminder }: { onOpen: () => void; reminder: AppReminder }) {
  const accent = TYPE_ACCENTS[reminder.type];
...
function EmptyState({
  ...
  return (
    <AppCard style={[styles.emptyCard, compact ? styles.emptyCardCompact : null, light ? styles.emptyCardLight : null]}>
      <Text style={[styles.emptyTitle, compact ? styles.emptyTitleCompact : null, light ? styles.emptyTitleLight : null]}>{title}</Text>
      <Text style={[styles.emptyText, compact ? styles.emptyTextCompact : null, light ? styles.emptyTextLight : null]}>{text}</Text>
...
function CalendarSkeleton() {
  return (
    <AppCard style={styles.monthCard}>
      <View style={styles.weekHeader}>
        {DAY_NAMES.map((day) => <View key={day} style={styles.skeletonLabel} />)}
      </View>
      <View style={styles.monthGrid}>
        {Array.from({ length: 35 }, (_, index) => <View key={index} style={styles.skeletonCell} />)}
      </View>
    </AppCard>
  );
}
...
function HeaderPill({
  ...
  return (
    ...
    >
      <Text style={[styles.headerPillText, light ? styles.headerPillTextLight : null]}>{label}</Text>
    </Pressable>
  );
}
...
function buildDateOptions(anchorDate: Date) {
  ...
  return Array.from({ length: 8 }, (_, index) => {
    ...
    return {
      ...
      label: isToday
        ? "Today"
        : new Intl.DateTimeFormat(undefined, { day: "numeric", month: "short", weekday: "short" }).format(date)
    };
  });
}
...
function buildSchedulerInitialDate(date: Date) {
  ...
  if (isSameDay(next, now)) {
    const nextHour = now.getMinutes() === 0 ? now.getHours() + 1 : now.getHours() + 1;
...
function SchedulerSheet({
  ...
  const [startMinute, setStartMinute] = useState(() => roundMinute(initialDate.getMinutes()));
  const [endEnabled, setEndEnabled] = useState(false);
  const [endHour, setEndHour] = useState(() => Math.min(23, initialDate.getHours() + 1));
  const [endMinute, setEndMinute] = useState(() => roundMinute(initialDate.getMinutes()));
  const [categoryKey, setCategoryKey] = useState<SchedulerCategoryKey>("personal");
  ...
  const selectedCategory = availableCategories.find((category) => category.key === categoryKey) ?? availableCategories[0];
  ...
  async function saveEvent() {
    ...
    try {
      await createReminder({
        ...
        notify: false,
        priority: selectedCategory.key === "medication" ? "important" : "normal",
          ...
          >
            ...
            <View style={styles.timeGrid}>
              ...
              >
                ...
                <Text style={styles.timeValue}>{formatTimeParts(startHour, startMinute)}</Text>
              </Pressable>
              <Pressable
              ...
              >
                ...
                <Text style={styles.timeValue}>{endEnabled ? formatTimeParts(endHour, endMinute) : "Add end"}</Text>
              </Pressable>
            </View>

            <View style={styles.chipRow}>
              {(["now", "morning", "afternoon", "evening"] as const).map((preset) => (
                <SchedulerChip
                  key={preset}
                  label={preset === "now" ? "Now" : preset[0].toUpperCase() + preset.slice(1)}
              ...
              <TimePickerPanel
                ...
                onConfirm={(hour, minute) => {
                  if (activePicker === "start") {
                    ...
                    if (!endEnabled || withTime(date, endHour, endMinute).getTime() <= withTime(date, hour, minute).getTime()) {
                ...
                title={activePicker === "start" ? "Choose start time" : "Choose end time"}
              />
            ) : null}
            ...
            <View style={styles.fieldCard}>
              ...
              <View style={styles.chipRow}>
                {["No alert", "At time", "10 min before", "1 hour before"].map((option) => (
                  <SchedulerChip
                    key={option}
                    label={option}
            ...
            {errorMessage ? <Text style={styles.schedulerError}>{errorMessage}</Text> : null}
          </ScrollView>
  ...
  return (
    <View style={styles.timePickerPanel}>
      <View style={styles.sectionHeaderRow}>
        ...
        <Text style={styles.timePickerValue}>{formatTimeParts(draftHour, draftMinute)}</Text>
      </View>
      <View style={styles.timePickerColumns}>
        <ScrollView contentContainerStyle={styles.timeColumn} style={styles.timeColumnScroll}>
          {hours.map((nextHour) => (
            <SchedulerChip
              key={nextHour}
      ...
      <View style={styles.timePickerActions}>
        ...
        <Pressable accessibilityRole="button" onPress={() => onConfirm(draftHour, draftMinute)} style={styles.saveButton}>
...
function SchedulerChip({
  ...
  return (
    <Pressable
      ...
      accessibilityState={{ selected }}
      onPress={onPress}
      style={({ pressed }) => [styles.schedulerChip, selected ? styles.schedulerChipSelected : null, pressed ? styles.pressed : null]}
    >
      <Text style={[styles.schedulerChipText, selected ? styles.schedulerChipTextSelected : null]}>{label}</Text>
    </Pressable>
  );
}

function buildMonthDays(monthStart: Date, reminders: AppReminder[], overlays: CalendarHaloOverlay[]) {
  ...
  return Array.from({ length: 42 }, (_, index) => {
    ...
    return {
      ...
      reminders: reminders.filter((reminder) => isSameDay(new Date(reminder.dueAt), date))
    };
  });
}

function buildWeekDays(selectedDate: Date, reminders: AppReminder[], overlays: CalendarHaloOverlay[]) {
...
function filterVisibleReminders(reminders: AppReminder[], enabledModules: AppModuleKey[]) {
  return reminders.filter((reminder) => {
    if (reminder.type === "child_baby") return enabledModules.includes("child_baby");
    if (reminder.type === "elder_care") return enabledModules.includes("elder_care");
    if (reminder.type === "caregiver") return enabledModules.includes("caregiver");
