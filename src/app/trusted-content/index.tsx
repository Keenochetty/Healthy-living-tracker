...
export default function TrustedContentScreen() {
  ...
  const refresh = useCallback(async () => {
    const [sources, cards, reviewQueue, expiring, disclaimers, auditLogs] =
      await Promise.all([
        getTrustedSources(),
        getTrustedHealthContentCards(),
        getContentReviewQueue(),
    ...
    <AppScreen>
      <View style={{ gap: spacing.sm }}>
        ...
        <Text style={{ color: theme.mutedText, fontSize: 15, lineHeight: 22 }}>
          Internal source governance for health education cards, disclaimers,
          quality checks, and assistant source usage.
        </Text>
      </View>
      ...
      <View style={{ flexDirection: "row", flexWrap: "wrap", gap: spacing.sm }}>
        ...
        <SummaryChip
          label="Published"
          value={
            state.cards.filter((card) => card.status === "published").length
          }
        />
      ...
      <AppCard>
        <SectionTitle
          ...
          subtitle={
            state.cards.length
              ? "Developer-managed Learn content cards."
              : "No content cards available yet."
          }
        />
        <View style={{ gap: spacing.sm }}>
          {state.cards.slice(0, 8).map((card) => (
            <Pressable
              ...
              style={{
                borderColor:
                  selectedCard?.id === card.id ? theme.primary : theme.border,
                borderRadius: 12,
                borderWidth: 1,
                gap: spacing.xs,
                padding: spacing.md,
            ...
            >
              ...
              <Text style={{ color: theme.mutedText, fontSize: 12 }}>
                {card.status} · {card.sourceOrganization} · review by{" "}
                {card.nextReviewDate}
              </Text>
            </Pressable>
          ))}
      ...
      <AppCard>
        <SectionTitle
          title="Selected Card"
          subtitle={selectedCard?.title ?? "Select a content card to review."}
        />
        {selectedCard ? (
          <View style={{ gap: spacing.md }}>
            ...
            <Text style={{ color: theme.mutedText, fontSize: 13 }}>
              Source: {selectedCard.sourceOrganization} ·{" "}
            ...
            >
              <AppButton
                disabled={loading}
                label="Run Check"
                onPress={() => runCheck(selectedCard.id)}
                size="sm"
                variant="outline"
              />
              <AppButton
                disabled={loading}
                label="Approve"
                onPress={() => approveAndRefresh(selectedCard.id)}
                size="sm"
                variant="secondary"
              />
              <AppButton
                disabled={loading}
                label="Publish"
                onPress={() => publishAndRefresh(selectedCard.id)}
                size="sm"
              />
            </View>
            {state.latestCheck ? (
        ...
        <SectionTitle
          ...
          subtitle={
            state.sources.length
              ...
              : "Trusted sources need to be added before publishing content."
          }
        />
        <View style={{ gap: spacing.sm }}>
        ...
        <SectionTitle
          ...
          subtitle={
            state.reviewQueue.length
              ? "Cards that need source, wording, or approval review."
              : "No source-backed content available for this topic yet."
          }
        />
        <ListOrEmpty
      ...
      <AppCard>
        <SectionTitle
          title="Disclaimers"
          subtitle={
            state.disclaimers.length
              ? "Reusable safety copy by realm and risk level."
              : "No disclaimer assigned yet."
          }
        />
        <ListOrEmpty
        ...
        <SectionTitle
          ...
          subtitle={
            state.auditLogs.length
              ? "Recent content governance events."
              : "No source review recorded yet."
          }
        />
        <ListOrEmpty
          ...
          renderItem={(log) => (
            <Row
              key={log.id}
              detail={new Date(log.createdAt).toLocaleString()}
      ...
      <Text style={{ color: theme.mutedText, fontSize: 13, lineHeight: 20 }}>
        Trusted content supports education and tracking only. It does not
