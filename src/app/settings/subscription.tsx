...
export default function SubscriptionSettingsScreen() {
  ...
  const [state, setState] = useState<SubscriptionScreenState>({
    kind: "loading",
  });
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setState(
        entitlement
          ? { entitlement, kind: "loaded" }
          : {
              kind: "unknown",
              message: "No verified subscription record was found.",
            },
      );
    } catch (error) {
      setState({
  ...
  const platform =
    entitlement?.platform === "unknown"
      ? currentPlatform()
      : (entitlement?.platform ?? currentPlatform());
    ...
    <AppMainLayout subtitle="Settings" title="Subscription and billing">
        ...
        {state.kind === "loading" ? (
          <AppCard style={styles.loading}>
            ...
            <Text style={[styles.body, { color: theme.mutedText }]}>
              Loading verified subscription status...
            </Text>
          </AppCard>
        ) : state.kind === "signed_out" ? (
          <AppAlertCard
            message="Sign in to view the verified subscription for your account."
      ...
      <AppSection title="Manage billing">
        <AppCard style={styles.actions}>
          <AppButton
            ...
            title={
              platform === "ios"
                ? "Manage with Apple"
                : platform === "android"
                  ? "Manage with Google Play"
                  : "Manage subscription unavailable"
            }
          />
          <Text style={[styles.body, { color: theme.mutedText }]}>
            The app cannot directly cancel an App Store or Google Play
            subscription. Account deletion does not automatically cancel a store
            subscription.
          </Text>
          <AppButton
            disabled={!restore.available}
            iconLeft={<RefreshCw color={theme.primary} size={18} />}
          ...
          <AppButton
            onPress={() => run(() => openBillingSupport(platform))}
      ...
      <AppSection title="Plan benefits">
        <AppCard style={styles.actions}>
          ...
          <Text style={[styles.body, { color: theme.mutedText }]}>
            Premium feature limits and family-plan benefits will appear here
...
function PlanCard({ entitlement }: { entitlement: SubscriptionEntitlement }) {
  const { theme } = useAppTheme();
  const free =
    entitlement.entitlementTier === "free" || entitlement.plan === "free";
  ...
  return (
    <AppCard style={styles.actions} variant={free ? "default" : "soft"}>
      <View style={styles.between}>
        <View style={styles.flex}>
          <Text style={[styles.plan, { color: theme.text }]}>
            {format(entitlement.entitlementTier || entitlement.plan)}
          </Text>
          <Text style={[styles.body, { color: theme.mutedText }]}>
        ...
        <AppChip
          label={
            verified ? "Verified" : free ? "Free plan" : "Verification pending"
          }
          variant={verified ? "success" : free ? "muted" : "warning"}
        />
      </View>
      <StatusRow
        label="Billing platform"
        value={format(entitlement.platform)}
      />
      <StatusRow
        label="Product"
        value={entitlement.productId ?? "Not applicable"}
      />
      ...
      <StatusRow
        ...
        value={entitlement.cancelAtPeriodEnd ? "Yes" : "No"}
      />
      <StatusRow
        label="Last verified"
        value={formatDate(entitlement.lastVerifiedAt)}
      />
    </AppCard>
  );
}
