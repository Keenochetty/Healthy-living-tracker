    ...
    <AppMainLayout subtitle="Settings" title="Privacy Center">
      <AppCard variant="warning">
        ...
        <Text
          style={[
            typography.body,
            { color: appColors.textSecondary, marginTop: 6 },
          ]}
        >
          This phase creates compliance preparation, not final legal approval.
          Final Privacy Policy, Terms, and compliance wording must be reviewed
          by a qualified legal professional before public release.
        </Text>
      </AppCard>
      ...
      {activeTab === "policy" && policy ? (
        <DocumentTab document={policy} sections={PRIVACY_POLICY_SECTIONS} />
      ) : null}
      {activeTab === "terms" && terms ? (
        <DocumentTab document={terms} sections={TERMS_SECTIONS} />
      ) : null}
      {activeTab === "medical" && medical ? (
        <MedicalTab document={medical} />
      ) : null}
      {activeTab === "consents" ? (
      ...
      <Text style={[typography.disclaimer, { color: appColors.textSecondary }]}>
        Privacy and consent controls help you manage your data. This does not
        replace legal advice for the app owner.
      </Text>
    </AppMainLayout>
  );
}
...
function SummaryTab() {
  const cards = [
    [
      "Private by default",
      "Your health information is private by default. You choose which modules to use, what to share, and who can access it.",
    ],
    [
      "You choose what to share",
      "Family and caregiver access is optional and can be revoked.",
    ],
    [
      "Sensitive health data is protected",
      "Women’s Health, Pregnancy, Baby / Child, Men’s Health, medication, biometrics, and records are private by default.",
    ],
    [
      "AI is draft-first",
      "AI suggestions are drafts until you confirm. Sensitive categories default off.",
    ],
    [
      "Device sync is optional",
      "You choose which data types to import, and synced data is never shared automatically.",
    ],
    [
      "Export or delete",
      "You can generate an authenticated export or permanently delete your account in-app.",
    ],
  ];
  ...
  return (
    <>
      ...
      <AppSection title="Manage privacy and data">
        <PrivacyLink
          description="Manage family sharing, Circle permissions, caregiver access, and sharing audit history."
          route="/(tabs)/circle"
          title="Sharing and caregiver permissions"
        />
        <PrivacyLink
          description="Control sensitive lock-screen details and notification preferences."
          route="/settings/notifications"
          title="Notification privacy"
        />
        <PrivacyLink
          description="Review, download where supported, manage, or delete uploaded records."
          route="/records"
          title="Records and uploaded documents"
        />
      </AppSection>
    </>
  );
...
function DocumentTab({
  ...
  return (
    <AppSection
      ...
      subtitle="Draft placeholder for legal review. Final version must be reviewed before public release."
    >
      ...
      {sections.map((section) => (
        <AppCard key={section}>
          ...
          >
            Placeholder wording for legal review. This app does not sell user
            health data, does not use health data for ads, and does not share
    ...
    <AppSection title={document.title}>
      <AppCard variant="warning">
        <Text style={[typography.body, { color: appColors.text }]}>
          This app is for tracking, organization, reminders, and general
          education only. It is not medical advice and does not replace a
          doctor, nurse, clinic, pharmacist, pediatrician, midwife, therapist,
          fertility specialist, emergency service, or healthcare professional.
        </Text>
      </AppCard>
      ...
      <AppCard variant="danger">
        <Text style={[typography.body, { color: appColors.text }]}>
          If you think something is urgent or severe, contact local emergency
          services or a healthcare professional immediately.
        </Text>
      </AppCard>
      <Text style={[typography.disclaimer, { color: appColors.textSecondary }]}>
        This app is for tracking, organization, reminders, and education only.
...
function ConsentTab({
  ...
  onUpdate: (type: ConsentType, action: "grant" | "deny" | "revoke") => void;
}) {
  const byType = new Map(
    consents.map((record) => [record.consentType, record]),
  );
  return (
    <AppSection
          ...
          <AppCard key={category.consentType} style={{ gap: appSpacing.md }}>
            ...
            >
              ...
              <PrivacyBadge
                ...
                type={status === "granted" ? "shared_family" : "private"}
              />
            </View>
            <Text style={[typography.caption, { color: appColors.textMuted }]}>
            ...
            >
              <AppButton
                onPress={() => onUpdate(category.consentType, "grant")}
                size="sm"
                title="I agree"
              />
              <AppButton
                onPress={() => onUpdate(category.consentType, "deny")}
                size="sm"
                title="Not now"
                variant="secondary"
              />
              <AppButton
                onPress={() => onUpdate(category.consentType, "revoke")}
                size="sm"
                title="Revoke"
                variant="outline"
              />
  ...
  return (
    <>
      <AppCard variant="warning" style={{ gap: appSpacing.md }}>
        <Text style={[typography.body, { color: appColors.text }]}>
          Sensitive exports and full account deletion require your current
          password.
        </Text>
        <AppFormInput
      ...
      <AppSection
        ...
        subtitle={
          exports.length
            ? "Export requests are listed below."
            : "No data exports requested."
        }
      >
        <AppCard style={{ gap: appSpacing.md }}>
          <Text style={[typography.body, { color: appColors.textSecondary }]}>
            The secure server export contains records owned by your
            authenticated account. Exports may contain sensitive health
            information; store them safely.
          </Text>
          <AppButton
            disabled={!signedIn}
            loading={working}
            onPress={onExport}
            title="Generate my data export"
          />
          <AppButton
            onPress={() => router.push("/records" as Href)}
            title="Manage uploaded records"
            variant="secondary"
          />
        </AppCard>
        {exports.map((request) => (
      ...
      <AppSection
        title="Delete My Data / Delete Account"
        subtitle={
          deletions.length
            ? "Deletion requests are listed below."
      ...
      >
        <AppCard variant="danger" style={{ gap: appSpacing.md }}>
          ...
          <Text style={[typography.body, { color: appColors.text }]}>
            This deletes your authenticated account and account-owned data.
            Shared family information owned by other people is not yours to
            delete.
          </Text>
          <Text style={[typography.body, { color: appColors.text }]}>
            Security, fraud-prevention, transaction, and legal records may be
            retained where required by law or legitimate compliance obligations.
          </Text>
          <Text style={[typography.body, { color: appColors.text }]}>
            Account deletion is not blocked by a subscription. Apple or Google
            Play subscriptions must be cancelled separately through the store.
          </Text>
          <AppFormInput
            autoCapitalize="characters"
            helperText={`Type ${ACCOUNT_DELETION_CONFIRMATION} exactly.`}
          ...
          <AppButton
            disabled={
              !signedIn || confirmation !== ACCOUNT_DELETION_CONFIRMATION
            }
            loading={working}
            onPress={() => onDeletion("full_account")}
            title="Permanently delete my account"
            variant="danger"
          />
          {deletionCompleted ? (
            <Text style={[typography.body, { color: appColors.success }]}>
              Account deletion completed. Clearing local data and returning to
              sign in.
            </Text>
          ) : null}
        </AppCard>
        {deletions.map((request) => (
          <AppCard key={request.id}>
            <Text style={[typography.cardTitle, { color: appColors.text }]}>
              {request.deletionType.replace(/_/g, " ")}
            </Text>
            <Text
              style={[typography.caption, { color: appColors.textSecondary }]}
            >
...
function AuditTab({ logs }: { logs: PrivacyAuditLog[] }) {
  return (
    <AppSection
      ...
      subtitle={
        logs.length
          ? "Recent privacy-related actions."
          : "No privacy audit logs yet."
      }
    >
      {logs.map((log) => (
        <AppCard key={log.id}>
...
function ChecklistTab() {
  return (
    <AppSection
      title="Developer Checklists"
      subtitle="Prepared for legal and app-store review."
    >
      {[
