...
export default function ProgramsScreen() {
  ...
  const source = live.length
    ? live
    : FITNESS_WORKOUT_PROGRAMS.map(programFromFallback);
  const programs = useMemo(
    () =>
      source.filter(
        (program) =>
          filter === "All" ||
          programHaystack(program).includes(
  ...
  return (
    <AppMainLayout
      ...
      subtitle="Choose a guided plan that fits your goal, time and body."
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <View style={styles.header}>
            ...
            {loading ? <StateCard text="Loading workout programs..." /> : null}
            {!loading && offline ? (
              <StateCard
                text="Live programs are unavailable. Showing built-in plans."
                warning
              />
            ) : null}
            <Text
            ...
            >
              {programs.length} plans
            </Text>
          </View>
        }
        ListEmptyComponent={
          !loading ? (
            <StateCard text="No programs found. Try changing filters." />
          ) : null
        }
      />
    </AppMainLayout>
...
export function programFromFallback(
  program: FitnessWorkoutProgram,
): FitnessProgramContent {
  return {
    audience: [program.audience ?? "Adults"],
    averageMinutes: program.averageMinutes ?? 30,
    days: program.days ?? 7,
    ...
    recoveryDays: program.recoveryDays ?? "Include lighter or rest days",
    title: program.title,
  };
}
    ...
    <AppCard style={[styles.card, { borderColor: theme.border }]}>
      <View style={styles.top}>
        <View style={[styles.icon, { backgroundColor: theme.primarySoft }]}>
          <AppIcon color={theme.primary} decorative name="planning" size={23} />
        </View>
      ...
      <Text style={[styles.meta, { color: theme.mutedText }]}>
        Audience: {program.audience.join(", ") || "Adults"} · Equipment:{" "}
        {program.equipment.join(", ") || "None"}
      </Text>
      <Text style={[styles.meta, { color: theme.mutedText }]}>
        {program.daysPerWeek} days/week · ~{program.averageMinutes} min ·{" "}
        {program.focus.join(", ") || "Balanced movement"}
      </Text>
      {safety ? (
        <Text style={[styles.safety, { color: theme.warning }]}>{safety}</Text>
      ) : null}
      <AppButton
        onPress={() =>
          router.push(`/fitness/program/${program.programId}` as Href)
        }
        size="sm"
        title="View plan"
      />
...
function StateCard({
  ...
  const { theme } = useAppTheme();
  return (
    <AppCard variant={warning ? "warning" : "soft"}>
      <Text
        style={{
          color: warning ? theme.warning : theme.text,
          fontWeight: "800",
        }}
      >
        {text}
...
function programHaystack(program: FitnessProgramContent) {
  return [
    program.title,
    program.description,
    program.goal,
    program.level,
    program.days === 7 ? "7 day" : "",
    program.days === 28 ? "28 day" : "",
    ...program.audience,
    ...program.equipment,
    ...program.focus,
  ]
...
function safetyBadge(program: FitnessProgramContent) {
  const value = programHaystack(program);
  if (
    value.includes("pregnan") ||
    value.includes("postpartum") ||
    value.includes("injur")
  )
    return "General guidance · seek professional advice when needed";
  if (value.includes("advanced") || value.includes("high intensity"))
    return "Advanced training caution";
  return undefined;
}
...
const styles = StyleSheet.create({
  ...
  meta: { fontSize: 11, fontWeight: "700", lineHeight: 17 },
