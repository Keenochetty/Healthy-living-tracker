      ...
      <FlatList
        ...
        ListHeaderComponent={
          <View style={styles.header}>
            ...
            <FilterRail
              items={PRIMARY_FILTERS}
              onSelect={setPrimaryFilter}
              selected={primaryFilter}
            />
            <Text style={[styles.filterTitle, { color: theme.mutedText }]}>
              Refine results
            </Text>
            <FilterGroup
              label="Audience"
            ...
            <FilterGroup
              label="Level"
              items={["All", ...WORKOUT_DIFFICULTIES.map(formatWorkoutLabel)]}
              onSelect={setLevel}
              selected={level}
            />
            <FilterGroup
              label="Equipment"
              items={["All", ...EXERCISE_EQUIPMENT.map(formatWorkoutLabel)]}
              onSelect={setEquipment}
              selected={equipment}
            />
            <FilterGroup
              label="Muscle group"
              items={["All", ...MUSCLE_GROUPS.map(formatWorkoutLabel)]}
              onSelect={setMuscle}
              selected={muscle}
            />
            ...
            {!loading && offline ? (
              <StateCard
                text="Live exercises are unavailable. Showing the built-in exercise library."
                warning
              />
            ) : null}
        ...
        ListEmptyComponent={
          !loading ? (
            <StateCard text="No exercises found. Try changing filters." />
          ) : null
        }
      />
    </AppMainLayout>
    ...
    <AppCard style={styles.card}>
      <View style={styles.cardTop}>
        ...
        <View style={styles.cardCopy}>
          ...
          <Text
            style={[styles.cardBody, { color: theme.mutedText }]}
            numberOfLines={2}
          >
            {exercise.description}
          </Text>
      ...
      <Text style={[styles.muscles, { color: theme.mutedText }]}>
        Main muscles:{" "}
        {exercise.primaryMuscles.map(formatWorkoutLabel).join(", ") ||
          "General movement"}
      </Text>
      <View style={styles.cardFooter}>
        ...
        <Pressable
          onPress={() =>
            router.push(`/fitness/exercise/${exercise.exerciseId}` as Href)
          }
...
function FilterGroup({
  ...
  return (
    <View style={styles.filterGroup}>
      ...
      <FilterRail items={items} onSelect={onSelect} selected={selected} />
    </View>
  );
}
...
function SmallPill({ label }: { label: string }) {
  const { theme } = useAppTheme();
  return (
    <View style={[styles.smallPill, { backgroundColor: theme.primarySoft }]}>
      <Text style={[styles.smallPillText, { color: theme.primary }]}>
        {label}
      </Text>
    </View>
  );
...
function matchesValue(values: string[], selected: string) {
  return (
    selected === "All" ||
    values.some((value) => value.toLowerCase().includes(selected.toLowerCase()))
  );
}
...
function matchesPrimary(exercise: FitnessExerciseContent, selected: string) {
  ...
  if (value === "no equipment")
    return exercise.equipment.some((item) =>
      ["none", "bodyweight"].includes(item.toLowerCase()),
    );
...
function getSafetyBadge(exercise: FitnessExerciseContent) {
  ...
  if (value.includes("child") || value.includes("kid"))
    return "Age-aware guidance";
