...
export default function GoalDetailScreen() {
  ...
  useEffect(() => {
    let mounted = true;
    getGoalProgressions(String(goalId))
      .then(({ data }) => {
        ...
        setSteps(
          data.map((row, index) => ({
            description: String(
              row.description ??
                row.guidance ??
                "Progress gradually and adjust to your body.",
            ),
            title: String(row.title ?? row.name ?? `Step ${index + 1}`),
          })),
        );
      })
      .catch(() => undefined);
  ...
  useEffect(() => {
    getUserFeaturePreferences(activeProfile?.id)
      .then((preferences) => {
        setEnabled(
          shouldShowFeature(
            FITNESS_GOAL_FEATURES[String(goalId)] ?? "fitness",
  ...
  const recommended = FITNESS_WORKOUT_PROGRAMS.filter((program) =>
    [program.goal, program.title, program.subtitle]
      .join(" ")
      .toLowerCase()
      .includes((goal.goalType ?? "").split(" ")[0].toLowerCase()),
  ).slice(0, 2);
  const fallbackRecommended = recommended.length
    ? recommended
    : FITNESS_WORKOUT_PROGRAMS.slice(0, 2);
  return (
    <AppMainLayout
      subtitle="Safe step-by-step fitness progression"
      title={goal.title}
    >
      {!enabled ? (
      ...
      <AppCard style={[styles.hero, { borderColor: theme.border }]}>
        ...
        <View style={styles.chips}>
          <Chip label={goal.timeline ?? "Flexible timeline"} />
          <Chip label={goal.difficulty ?? "Adaptable"} />
          <Chip label={goal.safetyLevel ?? "General guidance"} />
        </View>
        <View style={styles.actions}>
          <AppButton
            onPress={() =>
              setMessage(
                "Goal path started as a preview. No calendar events were created.",
              )
            }
            title="Start this path"
          />
          <AppButton
            onPress={() => setMessage("Goal saved for this preview.")}
            title="Save goal"
            variant="secondary"
          />
          <AppButton
            onPress={() => router.push("/fitness/programs" as Href)}
            title="View matching programs"
            variant="secondary"
          />
          <AppButton
            onPress={() =>
              setMessage(
                "Activation is coming soon. No calendar events were created.",
              )
            }
      ...
      <View style={styles.metrics}>
        <Metric
          label="Current level"
          value={goal.currentLevel ?? "Current routine"}
        />
        <Metric
          label="Target level"
          value={goal.targetLevel ?? goal.subtitle}
        />
        <Metric
          label="Weekly progression"
          value="Progress one controlled step at a time"
        />
        <Metric
          label="Training days"
          value={goal.trainingDays ?? "3 days per week"}
        />
        <Metric
          label="Recovery"
          value={goal.recoveryRequirement ?? "Include lighter days"}
        />
        <Metric
          label="Nutrition support"
          value={
            goal.nutritionSupport ?? "Balanced food planning and hydration"
          }
        />
      </View>
      <AppSection
        title="Step-by-step progression"
        subtitle="Progress only when the current step feels controlled and repeatable."
      />
      <View style={styles.stack}>
        {steps.map((step, index) => (
          <AppCard
            key={`${step.title}-${index}`}
            style={[styles.step, { borderColor: theme.border }]}
          >
            <Text style={[styles.stepIndex, { color: theme.primary }]}>
              Step {index + 1}
            </Text>
      ...
      <AppCard style={{ borderColor: theme.warning, borderWidth: 1 }}>
        <Text style={[styles.body, { color: theme.warning }]}>
          General guidance: adjust to your body and seek professional advice
          when needed. {STOP_GUIDANCE}
        </Text>
      </AppCard>
      <AppSection title="Recommended programs" />
      <View style={styles.stack}>
        {fallbackRecommended.map((program) => (
          <ProgramCard
            key={program.id}
            program={programFromFallback(program)}
          />
        ))}
      </View>
      <AppButton
        onPress={() => router.push("/fitness/goals" as Href)}
        title="Back to goal paths"
        variant="secondary"
      />
    </AppMainLayout>
  );
...
const styles = StyleSheet.create({
  ...
  body: { fontSize: 12, lineHeight: 19, marginTop: 5 },
  ...
  stack: { gap: 11 },
  step: { borderWidth: 1 },
  stepIndex: { fontSize: 10, fontWeight: "900", textTransform: "uppercase" },
  stepTitle: { fontSize: 17, fontWeight: "900", marginTop: 3 },
