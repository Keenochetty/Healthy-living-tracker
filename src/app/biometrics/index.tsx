...
const GLUCOSE_TIMING_OPTIONS: BloodGlucoseTiming[] = [
  "fasting",
  "before_meal",
  "after_meal",
  "bedtime",
...
export default function BiometricsScreen() {
  ...
  const [summary, setSummary] = useState<BiometricDashboardSummary | null>(
    null,
  );
    ...
    <ScreenWrapper backgroundColor="#fffaf0">
      ...
      <AppCard backgroundColor="#eff6ff">
        ...
        <Text style={{ color: "#334155", lineHeight: 21, marginTop: 6 }}>
          Biometric logs are for personal tracking only and are not a diagnosis.
          For concerning symptoms, unusual readings, pregnancy, children,
          medication concerns, or medical conditions, speak to a healthcare
          professional.
        </Text>
      </AppCard>
        ...
        {(summary?.items ?? []).map((item) => (
          ...
          >
            ...
            <Text style={{ color: "#64748b", lineHeight: 19, marginTop: 5 }}>
              {item.latestLoggedAt
                ? `Last logged ${formatRelativeDate(item.latestLoggedAt)}`
                : item.emptyText}
            </Text>
            <Text
              style={{ color: "#92400e", fontWeight: "900", marginTop: 10 }}
            >
              Add Log
            </Text>
          </TouchableOpacity>
      ...
      <AppCard>
        <View style={{ gap: 12 }}>
          ...
          >
            <View style={{ flex: 1 }}>
              ...
              <Text style={{ color: "#64748b", lineHeight: 20, marginTop: 5 }}>
                {latest
                  ? `Last logged ${formatRelativeDate(latest.loggedAt)}`
                  : BIOMETRIC_TYPE_EMPTY_TEXT[type]}
              </Text>
            </View>
            <TouchableOpacity
              activeOpacity={0.85}
          ...
          <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 10 }}>
            <MetricTile
              label="7-day average"
              value={formatTrendAverage(
                trend?.sevenDayAverage,
                trend?.unit,
                type,
              )}
            />
          ...
          <Text style={{ color: "#64748b", lineHeight: 21 }}>
            {trend?.message ?? BIOMETRIC_TYPE_EMPTY_TEXT[type]}
          </Text>
        </View>
      </AppCard>
      ...
      {type === "blood_pressure" || type === "blood_glucose" ? (
        <AppCard backgroundColor="#fff7ed">
          <Text style={{ color: "#9a3412", lineHeight: 21 }}>
            Follow guidance from your healthcare professional when interpreting
      ...
      <AppCard>
        <View style={{ gap: 12 }}>
          ...
          {logs.length ? (
            ...
            <Text style={{ color: "#64748b", lineHeight: 21 }}>
              Add your first log to start seeing trends.
            </Text>
          )}
        </View>
      </AppCard>
...
function BiometricForm({
  ...
  const canSave = useMemo(() => {
    if (type === "energy" || type === "mood" || type === "digestion")
    ...
    switch (type) {
      ...
      case "sleep":
        await createSleepLog({
          durationMinutes: Math.round(
            (Number(primary) || 0) * 60 + (Number(secondary) || 0),
          ),
      ...
      case "symptom":
        await createSymptomLog(
          label,
          Math.min(10, Math.max(1, Number(primary) || 1)),
          notes,
          loggedAtValue,
        );
        break;
      ...
      <View style={{ gap: 12 }}>
        ...
        {type === "body_measurement" ? (
          <>
            ...
            <Field
              keyboardType="numeric"
              label="Thigh cm"
              onChangeText={setQuaternary}
              value={quaternary}
            />
            <Field
              keyboardType="numeric"
              label="Hip cm optional"
              onChangeText={setSenary}
              value={senary}
            />
            <Field
              keyboardType="numeric"
              label="Body fat percentage optional"
              onChangeText={setQuinary}
              value={quinary}
            />
            <Text style={{ color: "#64748b", lineHeight: 20 }}>
              Progress photo placeholder is prepared for later.
            </Text>
          </>
        ) : null}
        ...
        {type === "sleep" ? (
          <>
            ...
            <OptionSelector
              ...
              selected={quality}
              onSelect={(value) => setQuality(value as SleepQuality)}
            />
            <Field
        ...
        {type === "heart_rate" ? (
          <>
            ...
            <Field
              keyboardType="numeric"
              label="Active heart rate optional"
              onChangeText={setSecondary}
              value={secondary}
            />
          </>
        ...
        {type === "blood_glucose" ? (
          <>
            ...
            <OptionSelector
              ...
              selected={unit}
              onSelect={(value) => setUnit(value as "mmol/L" | "mg/dL")}
            />
            <OptionSelector
              options={GLUCOSE_TIMING_OPTIONS}
              selected={timing}
              onSelect={(value) => setTiming(value as BloodGlucoseTiming)}
            />
          </>
        ) : null}
    ...
    >
      {options.map((option) => (
        ...
        >
          <Text
            style={{
              color: selected === option ? "#ffffff" : "#92400e",
              fontWeight: "900",
            }}
          >
