...
const CATEGORY_SETTINGS_KEY = "family_health_phase19_reminder_category_settings";
const SCHEDULED_RECORDS_KEY = "family_health_phase19_scheduled_notification_records";
...
export async function initializeNotifications() {
  ...
  if (Platform.OS === "android") {
    ...
    await Notifications.setNotificationChannelAsync("private-health-reminders", {
      importance: Notifications.AndroidImportance.DEFAULT,
      lightColor: "#8b5cf6",
...
export async function requestNotificationPermission() {
  ...
  if (!Notifications || !(await initializeNotifications())) {
    await updateNotificationSettings({ notificationsEnabled: false, permissionStatus: "unavailable" });
  ...
  if (current.granted || current.ios?.status === Notifications.IosAuthorizationStatus.PROVISIONAL) {
    ...
    await updateNotificationSettings({ notificationsEnabled: true, permissionStatus: status });
  ...
  await updateNotificationSettings({
    notificationsEnabled: permissionStatus === "granted" || permissionStatus === "provisional",
    permissionStatus
  });
...
export async function updateNotificationSettings(partial: Partial<NotificationSettings>) {
  ...
  const settings = await readJsonArray<NotificationSettings>(SETTINGS_KEY);
  await writeJsonArray(SETTINGS_KEY, [next, ...settings.filter((item) => item.userId !== LOCAL_USER_ID)]);
  return next;
}

export async function getReminderCategorySettings(profileId = LOCAL_PROFILE_ID) {
  const stored = await readJsonArray<ReminderCategorySettings>(CATEGORY_SETTINGS_KEY);
  const byCategory = new Map<ReminderCategory, ReminderCategorySettings>();

  REMINDER_CATEGORIES.forEach((category) => byCategory.set(category, defaultCategorySettings(category, profileId)));
  stored
    .filter((item) => item.profileId === profileId || (!item.profileId && profileId === LOCAL_PROFILE_ID))
    .forEach((item) => byCategory.set(item.category, item));

  return Array.from(byCategory.values());
}

export async function updateReminderCategorySettings(category: ReminderCategory, partial: Partial<ReminderCategorySettings>, profileId = LOCAL_PROFILE_ID) {
  const settings = await readJsonArray<ReminderCategorySettings>(CATEGORY_SETTINGS_KEY);
  const current = (await getReminderCategorySettings(profileId)).find((item) => item.category === category) ?? defaultCategorySettings(category, profileId);
  ...
  await writeJsonArray(CATEGORY_SETTINGS_KEY, [
    next,
    ...settings.filter((item) => !(item.category === category && (item.profileId ?? LOCAL_PROFILE_ID) === profileId))
  ]);

  return next;
}

export async function scheduleLocalNotification(input: ScheduleLocalNotificationInput) {
  const settings = await getNotificationSettings();
  const categorySettings = await getCategorySetting(input.category ?? "custom", input.profileId ?? LOCAL_PROFILE_ID);
  const permissionStatus = await getNotificationPermissionStatus();

  if (!settings.notificationsEnabled || !categorySettings.notificationEnabled || permissionStatus === "denied" || permissionStatus === "unavailable") {
    return saveScheduledNotificationRecord(input, undefined, "failed", "Device notifications are off. In-app reminders are still active.");
  }

  const Notifications = await loadNotifications();
  if (!Notifications || !(await initializeNotifications())) {
    return saveScheduledNotificationRecord(input, undefined, "failed", "Notification delivery is unavailable.");
  }

  const scheduledFor = await calculateNextAllowedNotificationTime(input.scheduledAt, categorySettings.quietHoursBehavior);
  const scheduledAt = new Date(scheduledFor);
  if (scheduledAt.getTime() <= Date.now()) {
    return saveScheduledNotificationRecord(input, undefined, "expired", "Reminder time has already passed.");
  }
  ...
  const notificationId = await Notifications.scheduleNotificationAsync({
    content: {
      ...
      categoryIdentifier: notificationCategoryIdentifier(input.category ?? "custom"),
      data: {
        ...(input.data ?? {}),
      ...
      sound: categorySettings.quietHoursBehavior === "silent" && quietNow ? false : undefined,
      title: input.title
    },
    trigger: {
      channelId: SENSITIVE_CATEGORIES.has(input.category ?? "custom") ? "private-health-reminders" : "health-reminders",
      date: scheduledAt,
...
export async function cancelLocalNotification(notificationId?: string) {
  ...
  const records = await readJsonArray<ScheduledNotificationRecord>(SCHEDULED_RECORDS_KEY);
  const updated = records.map((record) =>
    record.notificationId === notificationId ? { ...record, status: "cancelled" as const, updatedAt: new Date().toISOString() } : record
  );
  await writeJsonArray(SCHEDULED_RECORDS_KEY, updated);
  return updated.find((record) => record.notificationId === notificationId) ?? null;
}

export async function cancelNotificationsForReminder(reminderId: string) {
  const records = (await getScheduledNotificationRecords()).filter((record) => record.reminderId === reminderId && record.status === "scheduled");
  await Promise.all(records.map((record) => cancelLocalNotification(record.notificationId)));
  return records.length;
}

export async function rescheduleNotification(input: ScheduleLocalNotificationInput) {
...
export async function clearAllLocalNotificationsForUser() {
  ...
  await writeJsonArray(
    ...
    records.map((record) =>
      record.userId === LOCAL_USER_ID ? { ...record, status: "cancelled", updatedAt: new Date().toISOString() } : record
    )
  );
}
...
export async function createReminderActionLog(input: Omit<ReminderActionLog, "createdAt" | "id" | "userId"> & { userId?: string }) {
...
export function getSensitiveNotificationTitle(category: ReminderCategory, detailLevel: NotificationDetailLevel, detailedTitle?: string) {
  if (detailLevel === "detailed" && detailedTitle) return detailedTitle;
  if (detailLevel === "private") return category === "baby_medicine" || category === "baby_feeding" || category === "baby_sleep" ? "Baby care reminder" : "Health reminder";
...
export function getSensitiveNotificationBody(category: ReminderCategory, detailLevel: NotificationDetailLevel, detailedBody?: string) {
...
export async function applyQuietHoursBehavior(dateValue: string, behavior: QuietHoursBehavior) {
  if (behavior === "deliver") return dateValue;
  if (!(await isWithinQuietHours(dateValue))) return dateValue;
  return behavior === "delay" ? calculateNextAllowedNotificationTime(dateValue, behavior) : dateValue;
}

export async function calculateNextAllowedNotificationTime(dateValue: string, behavior: QuietHoursBehavior) {
  ...
  const inside = startMinutes <= endMinutes ? minutes >= startMinutes && minutes < endMinutes : minutes >= startMinutes || minutes < endMinutes;
...
export async function scheduleReminderNotification(reminder: HealthReminder) {
  ...
  const categorySetting = await getCategorySetting(category, reminder.profileId);
  const detailLevel = reminder.detailLevel ?? categorySetting.detailLevel;
  const scheduledFor = new Date(reminder.dueAt);
  scheduledFor.setMinutes(scheduledFor.getMinutes() - (reminder.leadTimeMinutes ?? categorySetting.defaultLeadTimeMinutes ?? 0));
...
export function buildReminderDeepLink(reminder: Pick<HealthReminder, "params" | "route" | "type">) {
  ...
  switch (reminder.type) {
    ...
    case "contraception":
    case "womens_health":
      return { params: { tab: reminder.type === "contraception" ? "contraception" : "today" }, route: "/cycle" };
    case "pregnancy":
    ...
    case "water":
    case "meal":
      return { params: reminder.type === "water" ? { tab: "water" } : undefined, route: "/food" };
    case "workout":
...
export function parseReminderNotificationPayload(data: Record<string, unknown>) {
  const params = typeof data.paramsJson === "string" ? safeJsonParse(data.paramsJson) : undefined;
  return {
    params,
    reminderId: typeof data.reminderId === "string" ? data.reminderId : undefined,
    route: typeof data.route === "string" ? data.route : undefined,
    sourceRealm: typeof data.sourceRealm === "string" ? data.sourceRealm : undefined,
    sourceId: typeof data.sourceId === "string" ? data.sourceId : undefined
  };
}

export function navigateFromReminderNotification(data: Record<string, unknown>) {
  const payload = parseReminderNotificationPayload(data);
  const route = payload.route || "/health-calendar";
  router.push(payload.params ? ({ pathname: route, params: payload.params } as never) : (route as never));
}
...
export function canShowReminderToViewer(reminder: HealthReminder) {
  return reminder.userId === LOCAL_USER_ID || reminder.sharedWithCaregiver || reminder.sharedWithFamily || reminder.sharedWithPartner;
}

export function filterRemindersByPermission<T extends HealthReminder>(reminders: T[]) {
  return reminders.filter(canShowReminderToViewer);
}

export async function handleReminderAction(reminder: HealthReminder, action: ReminderActionLog["action"], actionSource: ReminderActionLog["actionSource"] = "in_app") {
  return createReminderActionLog({ action, actionSource, profileId: reminder.profileId, reminderId: reminder.id });
}
...
export async function scheduleHealthReminderNotification(input: Omit<ScheduleLocalNotificationInput, "reminderId"> & { reminderId?: string }) {
  return scheduleLocalNotification({
    ...input,
...
export async function cancelHealthReminderNotification(notificationId?: string) {
  return cancelLocalNotification(notificationId);
}

export async function rescheduleHealthReminderNotification(input: Omit<ScheduleLocalNotificationInput, "reminderId"> & { reminderId?: string }) {
...
async function saveScheduledNotificationRecord(
  ...
  const records = await readJsonArray<ScheduledNotificationRecord>(SCHEDULED_RECORDS_KEY);
  await writeJsonArray(SCHEDULED_RECORDS_KEY, [record, ...records.filter((item) => item.reminderId !== record.reminderId)]);
  return record;
}

async function registerNotificationCategories(Notifications: ExpoNotificationsModule) {
  const categories = [
  ...
  await Promise.all(categories.map(([identifier, actions]) =>
    Notifications.setNotificationCategoryAsync(
      identifier,
      actions.map((action) => ({ buttonTitle: actionLabel(action), identifier: action, options: { opensAppToForeground: true } }))
    )
  ));
}

function notificationCategoryIdentifier(category: ReminderCategory) {
  if (category === "medication" || category === "supplement") return "medication-reminder";
...
function defaultCategorySettings(category: ReminderCategory, profileId = LOCAL_PROFILE_ID): ReminderCategorySettings {
  const now = new Date().toISOString();
  ...
  return {
    ...
    quietHoursBehavior: category === "medication" || category === "baby_medicine" || category === "appointment" ? "deliver" : "delay",
    updatedAt: now,
    userId: LOCAL_USER_ID
  };
}

async function getCategorySetting(category: ReminderCategory, profileId: string) {
  return (await getReminderCategorySettings(profileId)).find((item) => item.category === category) ?? defaultCategorySettings(category, profileId);
}
...
function extractNotificationData(input: unknown) {
  ...
  return maybeResponse.notification?.request?.content?.data ?? maybeResponse.request?.content?.data ?? {};
}
