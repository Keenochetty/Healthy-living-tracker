...
export async function buildPlanSchedulePreview(
  programId: string,
  options: PlanActivationOptions,
) {
  const program = getProgram(programId);
  const { data } = await getWorkoutProgramDays(programId);
  const sourceDays = data?.length
    ? data.map((row) => normalizeProgramDayContent(row))
    : fallbackProgramDays(programId);
  const workoutCount = Math.max(
    sourceDays.length,
    Math.ceil((program.days ?? 7) / 7) * (program.daysPerWeek ?? 3),
  );
  const weekdays = options.workoutWeekdays.length
    ? [...options.workoutWeekdays].sort()
    : [1, 3, 5];
...
export async function activateProgramPlan(
  ...
  const duplicate = await supabase
    ...
    .limit(1)
    .maybeSingle();
  if (duplicate.data && !options.duplicateAnyway)
    return { duplicate: duplicate.data, error: null };
  ...
  try {
    ...
    if (planResult.error || !planResult.data)
      throw planResult.error ?? new Error("Could not create imported plan");
    ...
    if (dayResult.error || !dayResult.data)
      throw dayResult.error ?? new Error("Could not create imported plan days");

    for (const importedDay of dayResult.data) {
      const previewDay = preview.find(
        (day) => day.dayNumber === importedDay.source_day_number,
      )!;
    ...
    const historyResult = await recordFitnessHistoryEvent({
    ...
    return {
      data: { importedPlanId, calendarEventCount: createdCalendarIds.length },
      duplicate: null,
      error: null,
    };
  } catch (error) {
...
export async function createCalendarEventForPlanDay(
  day: PlanSchedulePreviewDay,
  options: {
  ...
  if (result.error || !result.data)
    throw result.error ?? new Error("Could not create calendar event");
  return result.data;
}

export async function deactivateFitnessPlan(
  importedPlanId: string,
  options: { removeFutureEvents: boolean },
) {
  const user = await requireUser();
  ...
  const result = await supabase
    .from("user_imported_plans")
    .update({ deactivated_at: new Date().toISOString(), status: "cancelled" })
    .eq("id", importedPlanId)
    .eq("user_id", user.id);
  if (!result.error)
...
function getProgram(programId: string) {
  return (
    FITNESS_WORKOUT_PROGRAMS.find((item) => item.id === programId) ??
  ...
  return [
    {
      ...
      exercises: routine.exerciseIds.map(
...
function programSafetyNote(programId: string) {
  const program = getProgram(programId);
  const value =
    `${program.title} ${program.goal} ${program.audience} ${program.level}`.toLowerCase();
