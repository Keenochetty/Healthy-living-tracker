...
export async function getHealthTimelineEvents(startDate = startOfDay(new Date()), endDate = endOfDay(new Date())) {
  return getTimelineEventsByDateRange(startDate, endDate);
}

export async function getTimelineEventsByDateRange(startDate: Date, endDate: Date) {
  return generateTimelineEventsFromLocalData(startDate, endDate);
}

export async function generateTimelineEventsFromLocalData(startDate: Date, endDate: Date) {
  ...
  for (const day of days) {
    ...
    const [entries, waterLogs, medicationLogs, supplementLogs, dailyNote] = await Promise.all([
  ...
  workoutSessions
    ...
    .forEach((session) => {
      events.push(normalizeTimelineEvent({
        description: session.completed ? "Completed workout" : "Workout session",
  ...
  biometricLogs.forEach((log) => {
    events.push(normalizeTimelineEvent({
      description: [log.label, log.value !== undefined ? `${log.value}${log.unit ? ` ${log.unit}` : ""}` : undefined]
        .filter(Boolean)
        .join(" - "),
      eventAt: log.loggedAt,
  ...
  moodEnergyLogs
    ...
    .forEach((log) => {
      events.push(normalizeTimelineEvent({
        description: [log.mood, log.energyLevel !== undefined ? `Energy ${log.energyLevel}` : undefined].filter(Boolean).join(" - "),
        eventAt: log.date,
  ...
  if (pregnancyProfile?.status === "active") {
    pregnancyAppointments
      .filter((appointment) => isWithinRange(appointment.scheduledAt, startDate, endDate))
      .forEach((appointment) => {
        events.push(normalizeTimelineEvent({
    ...
    pregnancySymptoms.forEach((symptom) => {
      events.push(normalizeTimelineEvent({
        description: [symptom.severity ? formatType(symptom.severity) : undefined, symptom.notes].filter(Boolean).join(" - "),
    ...
    pregnancyQuestions
      .filter((question) => isWithinRange(question.createdAt, startDate, endDate))
      .forEach((question) => {
        events.push(normalizeTimelineEvent({
    ...
    if (pregnancyWeek.estimatedDueDate && isWithinRange(pregnancyWeek.estimatedDueDate, startDate, endDate)) {
      events.push(normalizeTimelineEvent({
  ...
  prescriptions
    .filter((record) => record.dateIssued && isWithinRange(record.dateIssued, startDate, endDate))
    .forEach((record) => {
      events.push(normalizeTimelineEvent({
  ...
  return events.sort((left, right) => new Date(right.eventAt).getTime() - new Date(left.eventAt).getTime());
}
...
export async function getTodayTimelineSummary(): Promise<TodayTimelineSummary> {
  const events = await getTimelineEventsByDateRange(startOfDay(new Date()), endOfDay(new Date()));
