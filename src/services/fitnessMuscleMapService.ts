...
export function scoresFromExerciseFallback(
  exercise?: ExerciseLike | null,
): MuscleScoreMap {
  if (!exercise) return {};

  const primary = muscleKeysFromText(
    exercise.primary_muscles ?? exercise.primaryMuscle,
  );
  const secondary = muscleKeysFromText(
...
export async function getExerciseMuscleScores(
  ...
  try {
    ...
    // Offline and unavailable-table states use the bundled workout library fallback.
  }
...
export async function getMuscleHistoryScores(
  days = 7,
): Promise<MuscleScoreMap> {
  try {
    const {
    ...
    for (const row of data as Array<{
      ...
      const decay = Math.max(0.2, 1 - ageDays / safeDays);
...
export async function recordMuscleLoadForExercise(input: {
  ...
  try {
    ...
    const scores = await getExerciseMuscleScores(
      input.exerciseId,
      input.fallbackExercise,
    );
    const intensityMultiplier = Math.max(0, input.intensityMultiplier ?? 1);
    ...
    return await supabase.from("user_muscle_load_history").insert(rows);
  } catch (error) {
