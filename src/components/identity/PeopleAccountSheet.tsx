  ...
  return (
    ...
    >
      <View style={styles.modal}>
        <Pressable
          accessibilityLabel="Close People and Account"
          ...
          <View style={styles.header}>
            <View style={styles.copy}>
              ...
              <Text style={[styles.subtitle, { color: theme.mutedText }]}>
                Switch the viewed person without changing who is signed in.
              </Text>
            </View>
            <Pressable
              accessibilityLabel="Close People and Account"
              accessibilityRole="button"
              onPress={onClose}
              style={[styles.close, { backgroundColor: theme.primarySoft }]}
            >
          ...
          >
            ...
            <SheetSection title="Switch person">
              ...
              <Text style={[styles.privacyNote, { color: theme.mutedText }]}>
                Only people you are permitted to view are shown. Circle
                membership does not automatically grant health-data access.
              </Text>
            </SheetSection>
            ...
            <SheetSection title="Circle actions">
              <ActionRow
                ...
                label="Open Circle"
                onPress={() => openRoute("/(tabs)/circle" as Href)}
              />
              <ActionRow
                icon={<CirclePlus color={theme.primary} size={20} />}
                label="Invite member"
                onPress={() => openRoute("/(tabs)/circle" as Href)}
              />
              <ActionRow
                icon={<CirclePlus color={theme.primary} size={20} />}
                label="Join Circle"
                onPress={() => openRoute("/scan-invite" as Href)}
              />
              <ActionRow
                icon={<CirclePlus color={theme.primary} size={20} />}
                label="Manage or create Circle"
                onPress={() => openRoute("/(tabs)/circle" as Href)}
              />
            </SheetSection>
            ...
            <SheetSection title="Account actions">
              <ActionRow
                ...
                label="My profile"
                onPress={() => openRoute("/settings/profile-contact" as Href)}
              />
              <ActionRow
                icon={<Settings color={theme.primary} size={20} />}
                label="App settings"
              ...
              <ActionRow
                icon={<Settings color={theme.primary} size={20} />}
                label="Subscription"
                onPress={() => openRoute("/settings/subscription" as Href)}
              />
              <ActionRow
                icon={<HelpCircle color={theme.primary} size={20} />}
                label="Help and support"
                onPress={() => openRoute("/(tabs)/profile" as Href)}
              />
              <ActionRow
                danger
                icon={<LogOut color={theme.danger} size={20} />}
                label="Sign out"
...
function ProfileRow({
  ...
  return (
    ...
    >
      ...
      {trailingLabel ? (
        <Text style={[styles.trailingLabel, { color: theme.primary }]}>
          {trailingLabel}
        </Text>
      ) : active ? (
        <Check color={theme.primary} size={19} />
      ) : (
...
export function formatRelationship(profile: HealthProfile) {
  return profile.profileType === "self"
    ? "Me"
    : profile.profileType
        .replaceAll("_", " ")
