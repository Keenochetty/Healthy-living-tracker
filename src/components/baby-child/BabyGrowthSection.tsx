...
export function BabyGrowthSection({
  logs,
  onAdd,
}: {
  logs: GrowthMeasurement[];
  onAdd: () => void;
}) {
  ...
  return (
    <View style={styles.stack}>
        ...
        {latest ? (
          <>
            <View style={styles.metrics}>
              <Metric
                label="Latest weight"
                value={latest.weight ? `${latest.weight} kg` : "Not added yet"}
              />
              <Metric
                label="Length / height"
                value={latest.height ? `${latest.height} cm` : "Not added yet"}
              />
              <Metric
              ...
              <Metric
                label="Last measured"
                value={formatDate(latest.loggedAt)}
              />
            </View>
            <View style={[styles.tag, { backgroundColor: theme.primarySoft }]}>
              <Text style={[styles.tagText, { color: theme.text }]}>
                {formatValue(latest.measurementSource ?? "home")}
              </Text>
            </View>
          </>
        ) : (
          <View style={styles.emptyCopy}>
            ...
            <Text style={[styles.subtitle, { color: theme.mutedText }]}>
              Track home or clinic measurements over time.
            </Text>
          </View>
        )}
      ...
      <SectionHeading
        subtitle="A simple view of measurements you have recorded."
        title="Your records"
      />
      <AppCard
        style={[
          ...
          {[24, 42, 34, 58, 52, 70].map((height, index) => (
            <View key={`${height}-${index}`} style={styles.chartColumn}>
              <View
                style={[
                  styles.chartLine,
                  { backgroundColor: theme.primarySoft, height },
                ]}
              />
              <View
                style={[styles.chartDot, { backgroundColor: theme.primary }]}
              />
            </View>
          ))}
        </View>
        <Text style={[styles.chartNote, { color: theme.mutedText }]}>
          No percentiles or interpretation are shown.
        </Text>
      </AppCard>
      ...
      {logs.length ? (
        ...
        >
          {logs.slice(0, 8).map((log, index) => (
            <View
              ...
              style={[
                styles.row,
                index < Math.min(logs.length, 8) - 1
                  ? { borderBottomColor: theme.border, borderBottomWidth: 1 }
                  : null,
              ]}
            >
              <View
                style={[styles.rowIcon, { backgroundColor: theme.primarySoft }]}
              >
                <AppIcon
                  color={theme.primary}
                  decorative
              ...
              <Text style={[styles.source, { color: theme.mutedText }]}>
                {formatValue(log.measurementSource ?? "home")}
              </Text>
            </View>
          ))}
        </AppCard>
        ...
        >
          ...
          <Text style={[styles.subtitle, { color: theme.mutedText }]}>
            Add at least one measurement when you are ready.
          </Text>
        </AppCard>
      )}
