  ...
  return (
    <View style={styles.stack}>
        ...
        <View style={styles.heading}>
          ...
          <View style={styles.copy}>
            ...
            <Text style={[styles.subtitle, { color: theme.mutedText }]}>
              Keep care records and clinic notes organized.
            </Text>
          </View>
        </View>
      </AppCard>
      ...
      <View style={styles.grid}>
        <EntranceCard
          action="Add vaccine record"
          helper="Track received vaccines, clinic dates, and documents."
          icon="vaccines"
          onPress={onAddVaccine}
          status="Record from clinic card"
          title="Vaccines"
        />
        <EntranceCard
          action="View medicine"
          helper="Track medicine logs and notes."
          icon="medication"
          onPress={onMedicine}
          status="Follow label instructions"
          title="Medicine"
        />
        <EntranceCard
          action="View records"
          helper="Clinic cards, doctor notes, prescriptions, and documents."
          icon="records"
          onPress={onRecords}
          status="Baby health vault"
          title="Records"
        />
        <EntranceCard
          action="Schedule"
          helper="Add reminders to Calendar."
          icon="calendar"
          onPress={onSchedule}
          status="Plan care visits"
      ...
      <View>
        ...
        <Text style={[styles.subtitle, { color: theme.mutedText }]}>
          Manual records first, based on your clinic card or healthcare
          provider.
        </Text>
      </View>
      ...
      >
        ...
        <Text style={[styles.helper, { color: theme.mutedText }]}>
          Add vaccine records from your clinic card or healthcare provider.
        </Text>
      </AppCard>
      ...
      >
        ...
        <Text style={[styles.helper, { color: theme.mutedText }]}>
          Review vaccine records and future dates with your clinic.
        </Text>
        <View style={[styles.reviewBadge, { backgroundColor: theme.surface }]}>
          <Text style={[styles.reviewBadgeText, { color: theme.text }]}>
            Review with clinic
          </Text>
        </View>
      </AppCard>
      ...
      <View>
        ...
        <Text style={[styles.subtitle, { color: theme.mutedText }]}>
          Record-only age markers. No recommendations are generated.
        </Text>
      </View>
      <AppCard
        style={[
            ...
            <View key={node} style={styles.nodeWrap}>
              <View
                style={[
                  ...
                  {
                    backgroundColor:
                      index === 0 ? theme.primary : theme.primarySoft,
                    borderColor: theme.primary,
                  },
                ]}
              />
              <Text style={[styles.nodeText, { color: theme.mutedText }]}>
                {node}
              </Text>
            </View>
          ))}
      ...
      <View style={styles.sectionRow}>
        <View style={styles.copy}>
          ...
          <Text style={[styles.subtitle, { color: theme.mutedText }]}>
            Information saved for this child only.
          </Text>
        </View>
        <Pressable
      ...
      {vaccineRecords.length ? (
        ...
        >
          {vaccineRecords.slice(0, 8).map((record, index) => (
            <View
              ...
              style={[
                styles.recordRow,
                index < Math.min(vaccineRecords.length, 8) - 1
            ...
            >
              ...
              <View style={styles.copy}>
                ...
                <Text style={[styles.recordMeta, { color: theme.mutedText }]}>
                  {record.dateReceived ?? "Date not added"} ·{" "}
                  {record.recordSource
                    ? sourceLabel(record.recordSource)
                    : "Source to be added"}
                </Text>
              </View>
              <Text style={[styles.recordStatus, { color: theme.mutedText }]}>
                Recorded
              </Text>
            </View>
          ))}
        ...
        >
          ...
          <Text style={[styles.helper, { color: theme.mutedText }]}>
            Record information from a clinic card or healthcare provider.
          </Text>
          <Pressable
            accessibilityRole="button"
            onPress={onAddVaccine}
      ...
      >
        <View
          style={[styles.uploadIcon, { backgroundColor: theme.primarySoft }]}
        >
          <AppIcon color={theme.primary} decorative name="upload" size={24} />
        </View>
        <Text style={[styles.cardTitle, { color: theme.text }]}>
          Clinic card or photo record
        </Text>
        <Text style={[styles.helper, { color: theme.mutedText }]}>
          Document linking will connect to Baby Records later.
        </Text>
        <Text style={[styles.sourceText, { color: theme.mutedText }]}>
          Trusted source to be added.
        </Text>
      </AppCard>
      ...
      >
        ...
        <View style={styles.copy}>
          <Text style={[styles.cardTitle, { color: theme.text }]}>
            Record site, if known
          </Text>
          <Text style={[styles.helper, { color: theme.mutedText }]}>
            Left or right upper arm, left or right thigh, oral, other, or not
            sure.
          </Text>
          <Text style={[styles.sourceText, { color: theme.mutedText }]}>
            Record the site only if it appears on the clinic card or was given
      ...
      >
        <Text style={[styles.safetyText, { color: theme.mutedText }]}>
          Always follow the medicine label or healthcare professional's
          instructions. This app does not calculate or recommend doses.
        </Text>
      </AppCard>
      <Text style={[styles.generalSafety, { color: theme.mutedText }]}>
...
function EntranceCard({
  ...
  return (
    ...
    >
      ...
      <Text style={[styles.entranceHelper, { color: theme.mutedText }]}>
        {helper}
      </Text>
      <Pressable
        accessibilityRole="button"
...
function Tag({ label, selected }: { label: string; selected?: boolean }) {
  ...
  return (
    ...
    >
      <Text
        style={[styles.tagText, { color: selected ? "#ffffff" : theme.text }]}
      >
        {label}
      </Text>
    </View>
