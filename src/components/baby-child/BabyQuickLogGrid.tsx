...
export type BabyQuickLogMode = "feed" | "sleep" | "diaper" | "growth" | "solid" | "medicine" | "vaccine" | "note";

const ACTIONS: Array<{ color: string; icon: string; label: string; mode: BabyQuickLogMode }> = [
...
export function BabyQuickLogGrid({ onSelect }: { onSelect: (mode: BabyQuickLogMode) => void }) {
    ...
    <View style={styles.grid}>
      {ACTIONS.map((action) => (
        <Pressable
          ...
          style={({ pressed }) => [
            styles.action,
            { backgroundColor: isDark(theme.background) ? theme.surfaceSoft ?? theme.surface : action.color, borderColor: theme.border },
            pressed ? styles.pressed : null
          ]}
        >
          <View style={[styles.iconWrap, { backgroundColor: theme.surface }]}>
            <AppIcon color={theme.primary} decorative name={action.icon as never} size={24} />
          </View>
...
const styles = StyleSheet.create({
  action: { alignItems: "center", borderRadius: 22, borderWidth: 1, flexBasis: "22%", flexGrow: 1, gap: 8, justifyContent: "center", minHeight: 96, minWidth: 82, padding: 10 },
  grid: { flexDirection: "row", flexWrap: "wrap", gap: 10 },
  iconWrap: { alignItems: "center", borderRadius: 18, height: 46, justifyContent: "center", width: 46 },
