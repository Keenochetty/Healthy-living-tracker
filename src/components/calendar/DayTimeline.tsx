...
export function DayTimeline({
  ...
  if (variant === "notebook" || variant === "notebook-light") {
    ...
    return (
      <View
        style={[styles.notebookList, light ? styles.notebookListLight : null]}
      >
    ...
    <View style={{ gap: 12 }}>
      {showSummary ? (
        <View>
          ...
          <Text
            style={{
              color: variant === "light" ? "#64748b" : "#94a3b8",
              marginTop: 3,
            }}
          >
      ...
      {times.length ? (
        times.map((time) => (
          <View key={time} style={{ gap: 8 }}>
            <Text
              style={{
                color: variant === "light" ? "#0f766e" : "#6ee7c8",
                fontSize: 13,
                fontWeight: "900",
              }}
            >
        ...
        >
          <Text
            style={{
              color: variant === "light" ? "#475569" : "#cbd5e1",
              lineHeight: 21,
            }}
          >
...
function NotebookReminderRow({
  ...
  return (
    <View
      style={[
        styles.notebookRow,
        light ? styles.notebookRowLight : null,
        last ? styles.notebookRowLast : null,
      ]}
    >
      <Pressable
        ...
        onPress={onOpen}
        style={({ pressed }) => [
          styles.notebookEntry,
          pressed ? styles.notebookPressed : null,
        ]}
      >
        <View
          style={[styles.notebookIcon, { backgroundColor: `${type.colour}18` }]}
        >
          <View
            style={[styles.notebookIconDot, { backgroundColor: type.colour }]}
          />
        </View>
        <View style={styles.notebookCopy}>
          <View style={styles.notebookTitleRow}>
            <Text
              ...
              style={[
                ...
                light ? styles.notebookTitleLight : null,
              ]}
            >
              {reminder.title}
            </Text>
          ...
          <Text style={[styles.notebookTime, { color: type.colour }]}>
          ...
          {reminder.notes ? (
            <Text
              numberOfLines={2}
              style={[
                styles.notebookNotes,
                light ? styles.notebookNotesLight : null,
              ]}
            >
              {reminder.notes}
            </Text>
          ...
          <View style={styles.notebookMeta}>
            ...
            <Text
              style={[
                styles.notebookStatus,
                light ? styles.notebookStatusLight : null,
              ]}
            >
              {pending ? "Planned" : reminder.status}
            </Text>
          </View>
        </View>
      </Pressable>
      {pending ? (
        <View style={styles.notebookActions}>
          <NotebookAction
            icon={<Check color="#0f766e" size={14} />}
            label="Complete"
            light={light}
            onPress={onComplete}
          />
          <NotebookAction
            icon={
              <CircleSlash color={light ? "#64748b" : "#94a3b8"} size={14} />
            }
            label="Skip"
            light={light}
            onPress={onSkip}
...
function NotebookAction({
  ...
  return (
    ...
    >
      ...
      <Text
        style={[
          styles.notebookActionText,
          light ? styles.notebookActionTextLight : null,
        ]}
      >
        {label}
      </Text>
