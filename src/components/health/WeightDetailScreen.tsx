...
type WeightPoint = {
  activity: Extract<GeneralHealthActivityEntry, { type: "weight" }>;
  id: string;
  recordedAt: string;
...
const RANGE_OPTIONS: { label: string; value: WeightRange }[] = [
  { label: "7D", value: 7 },
  { label: "30D", value: 30 },
  { label: "90D", value: 90 },
  { label: "All", value: "all" },
...
export function WeightDetailScreen() {
  ...
  // TODO: Replace feature-local weight entries with the approved profile health data layer and supported connected-scale sources.
  // TODO: Read and persist weight display units through the selected profile's country and unit settings.
  // TODO: Let the shared Add Weight sheet use the selected profile's preferred entry unit while preserving canonical kilograms.
  // TODO: Normalize legacy weight activities only when their stored unit can be identified without guessing.
  const allPoints = useMemo(
    () => getWeightEntriesForProfile(activities, selectedProfileId),
    [activities, selectedProfileId],
  );
  const points = useMemo(
    () => filterWeightEntriesByRange(allPoints, range, openedAt),
    [allPoints, openedAt, range],
  );
  const summary = useMemo(() => calculateWeightSummary(points), [points]);
  const selectedPoint =
    points.find((point) => point.id === selectedId) ?? points.at(-1) ?? null;
  function saveWeight(draft: GeneralHealthLogDraft) {
    const activity = createGeneralHealthActivity(
      draft,
      selectedProfileId,
      selectedProfileName,
    );
  ...
  return (
    <View style={[styles.screen, { backgroundColor: theme.background }]}>
      <HealthScreenContainer bottomSpacing={96} contentStyle={styles.content}>
        ...
        {SCREEN_STATE === "error" ? (
          <ScreenState
            ...
            title="We couldn't load weight history"
          />
        ) : null}
        {SCREEN_STATE === "ready" && !allPoints.length ? (
          <ScreenState
            action="Add weight entry"
            message="Add an entry to begin building your personal history."
        ...
        {SCREEN_STATE === "ready" && allPoints.length ? (
          <>
            ...
            {!points.length ? (
              <ScreenState
                action="Show all"
                message="Choose another range or add a new entry."
                onAction={() => {
                  setRange("all");
                  setSelectedId(null);
                }}
                title="No entries in this time range"
              />
            ) : (
              <>
                <WeightChart
                ...
                />
                {selectedPoint ? (
                  <SelectedEntry point={selectedPoint} unit={unit} />
                ) : null}
      ...
      <GeneralHealthLogSheet
        key={logOpen ? "weight-open" : "weight-closed"}
        logType={logOpen ? "weight" : null}
    ...
    <View style={styles.header}>
      ...
      <View style={styles.headerCopy}>
        ...
        <Text style={[styles.subtitle, { color: theme.mutedText }]}>
          Saved entries and personal trends
        </Text>
      </View>
      <View
      ...
      >
        ...
        <Text
          numberOfLines={1}
          style={[styles.profileName, { color: theme.mutedText }]}
        >
          {profileName}
        </Text>
      </View>
...
function LatestCard({
  ...
  const previous = points.at(-2),
    changeKg = previous
      ? latest.valueKilograms - previous.valueKilograms
      : null,
    displayChange = changeKg === null ? null : convertKilograms(changeKg, unit);
  const comparison =
    displayChange === null
      ? "Add more entries to see changes over time."
      : Math.abs(changeKg!) < 0.05
        ? "No change from your previous saved entry."
        : `${formatNumber(Math.abs(displayChange))} ${unit} ${displayChange > 0 ? "higher" : "lower"} than your previous saved entry.`;
  return (
    <AppCard
      style={[
        styles.hero,
    ...
    >
      <View pointerEvents="none" style={styles.heroLines}>
        <View style={[styles.heroLine, { backgroundColor: theme.primary }]} />
        <View
          style={[
            styles.heroLine,
            styles.heroLineTwo,
      ...
      <Text
        accessibilityLabel={`Latest weight entry, ${formatWeightSpeech(latest.valueKilograms, unit)}, saved ${formatDateTime(latest.recordedAt)}. ${comparison}`}
        adjustsFontSizeToFit
        numberOfLines={1}
        style={[styles.latestValue, { color: theme.text }]}
      >
        {formatNumber(convertKilograms(latest.valueKilograms, unit))}
    ...
    <View style={styles.selectorBlock}>
      ...
      <View style={styles.unitRow}>
        {(["kg", "lb"] as const).map((option) => {
          ...
          return (
            <Pressable
              accessibilityLabel={`${option === "kg" ? "Kilograms" : "Pounds"}, ${active ? "selected" : "not selected"}`}
              accessibilityRole="button"
...
function WeightChart({
  ...
  const width = Math.max(160, Math.min(screenWidth - 70, 410)),
    height = 210,
    plot = getChartPlot(points, width, height),
    labels = getXAxisLabels(plot.positions),
    selectedPoint =
      plot.positions.find((point) => point.id === selectedId) ?? null;
  const accessibilityLabel = `Weight trend for ${range === "all" ? "all saved time" : `the last ${range} days`}. ${summary.count} saved entries. Lowest saved weight ${formatWeightSpeech(summary.lowestKg, unit)}, highest ${formatWeightSpeech(summary.highestKg, unit)}, average ${formatWeightSpeech(summary.averageKg, unit)}.`;
    ...
    <AppCard style={[styles.chartCard, { borderColor: theme.border }]}>
      ...
      >
        ...
        {plot.ticks.map((tick) => (
          ...
          >
            {formatNumber(convertKilograms(tick.valueKg, unit))}
          </Text>
        ))}
        {selectedPoint ? (
          <View
            ...
            style={[
              styles.chartTooltip,
              {
                ...
                top: clamp(selectedPoint.y - 43, 2, height - 38),
              },
            ]}
          >
            <Text
              style={[styles.chartTooltipText, { color: theme.background }]}
            >
              {formatWeight(selectedPoint.valueKilograms, unit)}
            </Text>
          </View>
        ) : null}
        {plot.positions.map((point) => (
          <Pressable
            accessibilityLabel={`${formatWeightSpeech(point.valueKilograms, unit)}, ${formatFullDateTime(point.recordedAt)}, added manually`}
            accessibilityRole="button"
      ...
      {points.length === 1 ? (
        <Text style={[styles.singleText, { color: theme.mutedText }]}>
          Add more entries to see a trend.
        </Text>
      ) : null}
    </AppCard>
  );
...
function SelectedEntry({
  ...
  return (
    <View
      accessibilityLabel={`Selected weight entry, ${formatWeightSpeech(point.valueKilograms, unit)}, ${formatFullDateTime(point.recordedAt)}, added manually.${point.activity.details.notes ? ` Note: ${point.activity.details.notes}` : ""}`}
      accessible
      style={[
        styles.selected,
        {
    ...
    >
      ...
      <Text style={[styles.selectedValue, { color: theme.text }]}>
        {formatWeight(point.valueKilograms, unit)}
      </Text>
...
function SavedSummary({
  ...
  const { theme } = useAppTheme();
  const change = convertKilograms(summary.changeKg, unit);
  const metrics = [
    {
      accessibilityLabel: `Lowest saved weight, ${formatWeightSpeech(summary.lowestKg, unit)}`,
      label: "Lowest saved",
      value: formatWeight(summary.lowestKg, unit),
    },
    {
      accessibilityLabel: `Highest saved weight, ${formatWeightSpeech(summary.highestKg, unit)}`,
      label: "Highest saved",
      value: formatWeight(summary.highestKg, unit),
    },
    {
      accessibilityLabel: `Average saved weight, ${formatWeightSpeech(summary.averageKg, unit)}`,
      label: "Average saved",
      value: formatWeight(summary.averageKg, unit),
    },
    {
      accessibilityLabel: `${summary.count} saved ${summary.count === 1 ? "entry" : "entries"}`,
      label: "Entries",
      value: `${summary.count}`,
    },
    ...(summary.count > 1
      ? [
          {
            accessibilityLabel: `Change from first to latest visible entry, ${formatSignedWeightSpeech(change, unit)}`,
            label: "Change",
            value: `${change > 0 ? "+" : ""}${formatNumber(change)} ${unit}`,
          },
        ]
      : []),
  ];
  return (
    <AppSection
      subtitle="Calculated from the entries visible in this time range."
      ...
      <View style={styles.readingStack}>
        {entries.map((point) => (
          <Pressable
            ...
            accessibilityLabel={`${formatWeightSpeech(point.valueKilograms, unit)}, ${formatFullDateTime(point.recordedAt)}, added manually${point.activity.details.notes ? ", includes a note" : ""}`}
            accessibilityRole="button"
            key={point.id}
  ...
  return (
    <View style={[styles.safety, { borderColor: theme.border }]}>
      ...
      <Text style={[styles.safetyText, { color: theme.mutedText }]}>
        This screen helps you organize the weight entries you choose to save. It
        does not judge body size or provide a medical interpretation. Contact a
        healthcare professional if you have concerns about unexpected body
        changes.
      </Text>
    </View>
  );
...
function ScreenState({
  ...
  return (
    <AppCard style={[styles.empty, { borderColor: theme.border }]}>
      <AppIcon color={theme.primary} container name="weight" size={24} />
...
function getWeightEntriesForProfile(
  ...
  return entries
    .flatMap((entry) => {
      if (
        ...
        entry.type !== "weight" ||
        !entry.details.weight
      )
        return [];
      const valueKilograms = Number(entry.details.weight),
        recordedTime = new Date(entry.createdAt).getTime();
      return Number.isFinite(valueKilograms) &&
        valueKilograms > 0 &&
...
function filterWeightEntriesByRange(
  points: WeightPoint[],
  range: WeightRange,
  reference: Date,
) {
...
function calculateWeightSummary(points: WeightPoint[]): WeightSummary {
  const values = points.map((point) => point.valueKilograms);
  return {
    averageKg: values.reduce((sum, value) => sum + value, 0) / values.length,
    changeKg: values.at(-1)! - values[0],
    count: values.length,
    highestKg: Math.max(...values),
    lowestKg: Math.min(...values),
  };
}
function kilogramsToPounds(value: number) {
  return value * 2.2046226218;
}
function convertKilograms(value: number, unit: WeightUnit) {
  return unit === "kg" ? value : kilogramsToPounds(value);
}
function formatNumber(value: number) {
  return value.toFixed(1);
}
function formatWeight(valueKg: number, unit: WeightUnit) {
  return `${formatNumber(convertKilograms(valueKg, unit))} ${unit}`;
}
function formatWeightSpeech(valueKg: number, unit: WeightUnit) {
  return `${formatNumber(convertKilograms(valueKg, unit))} ${unit === "kg" ? "kilograms" : "pounds"}`;
}
function formatSignedWeightSpeech(value: number, unit: WeightUnit) {
  const sign = value > 0 ? "plus " : value < 0 ? "minus " : "";
  return `${sign}${formatNumber(Math.abs(value))} ${unit === "kg" ? "kilograms" : "pounds"}`;
}
function getChartPlot(points: WeightPoint[], width: number, height: number) {
  const left = 48,
    right = 24,
    top = 30,
    bottom = 22,
    valuesKg = points.map((point) => point.valueKilograms),
    rawMinKg = Math.min(...valuesKg),
    rawMaxKg = Math.max(...valuesKg),
    dataSpanKg = Math.max(rawMaxKg - rawMinKg, 2),
    paddingKg = Math.max(0.5, dataSpanKg * 0.2),
    intervalKg = 1,
    minKg = Math.floor((rawMinKg - paddingKg) / intervalKg) * intervalKg,
    maxKg = Math.ceil((rawMaxKg + paddingKg) / intervalKg) * intervalKg,
    spanKg = Math.max(maxKg - minKg, intervalKg),
      ...
      points.length === 1
        ...
        : left +
          ((new Date(point.recordedAt).getTime() - firstTime) / timeSpan) *
            (width - left - right),
    y:
      top + ((maxKg - point.valueKilograms) / spanKg) * (height - top - bottom),
  }));
  const ticks = [maxKg, (maxKg + minKg) / 2, minKg].map((valueKg) => ({
    valueKg,
    y: top + ((maxKg - valueKg) / spanKg) * (height - top - bottom),
  }));
...
function getXAxisLabels<T extends WeightPoint>(points: T[]) {
...
function formatAxisDate(value: string, range: WeightRange) {
...
const styles = StyleSheet.create({
  ...
  chartTooltipText: { fontSize: 11, fontWeight: "900" },
  ...
  heroLineTwo: { right: 20, top: 52, width: 115 },
  heroSupport: { fontSize: 13, lineHeight: 20, marginTop: 5, maxWidth: "82%" },
  heroTime: { fontSize: 13 },
  latestUnit: { fontSize: 24, fontWeight: "800" },
  latestValue: { fontSize: 42, fontWeight: "900", lineHeight: 50 },
  loading: { gap: 14 },
  pointTarget: { height: 44, position: "absolute", width: 44 },
  pressed: { opacity: 0.74 },
  profile: { alignItems: "center", flexShrink: 1, gap: 4, maxWidth: 72 },
  profileIcon: {
    alignItems: "center",
    borderRadius: 12,
    height: 32,
