...
const QUICK_ACTIONS: QuickAction[] = [
  {
    accessibilityLabel: "Log period day",
    available: true,
    description: "Save period dates privately on this device for now.",
    icon: "pregnancy_cycle",
    label: "Log period",
    type: "period",
  },
  {
    accessibilityLabel: "Add Women's Health symptom",
    available: true,
    description: "Use the private symptom chips on this screen.",
    icon: "vitals",
    label: "Add symptom",
    type: "symptom",
  },
  {
    accessibilityLabel: "Add flow entry",
    available: false,
    description: "Flow-only logging will be added after period logging.",
    icon: "water",
    label: "Add flow",
    type: "flow",
  },
  {
    ...
    available: false,
    description: "Mood tracking will be added in a later step.",
    icon: "ai_assistant",
    label: "Add mood",
    type: "mood",
  },
  {
    accessibilityLabel: "Add private Women's Health note",
    available: false,
    description: "Private notes will be added in a later step.",
    icon: "edit",
    label: "Add note",
    type: "note",
  },
...
const RECENT_ENTRIES: PreviewEntry[] = [
  {
    ...
    icon: "vitals",
    id: "symptom-cramps",
    note: "Lower abdomen, short note saved.",
  ...
  },
  {
    ...
    icon: "edit",
    id: "note-sleep",
    note: "Sleep changes after a busy day.",
    source: "Added manually",
    time: "7 Jun at 21:15",
    title: "Sleep changes",
  },
...
const REMINDERS: ReminderEntry[] = [
  {
    icon: "pregnancy_cycle",
    id: "period-estimate",
    source: "Cycle estimate",
    time: "In 16 days",
    title: "Period estimate begins",
  },
  {
    icon: "reminder",
    id: "contraception-reminder",
  ...
  },
  {
    ...
    source: "Appointment",
    time: "Friday",
    title: "Women's Health appointment",
  },
];
const CALENDAR_DAYS: CalendarDay[] = Array.from({ length: 30 }, (_, index) => {
  ...
  return {
    date,
    iso: `2026-06-${String(date).padStart(2, "0")}`,
    isSelected: date === 12,
    isToday: date === 9,
    marker,
  };
...
const FLOW_OPTIONS: readonly FlowSummary[] = [
  "None",
  "Light",
  "Medium",
  "Heavy",
...
export function WomensHealthScreen() {
    ...
    () =>
      new Set(
        periodEntries
          ...
          .flatMap((entry) => entry.loggedDates),
      ),
    [periodEntries],
  );
  const calendarDays = useMemo(
    () =>
      CALENDAR_DAYS.map((day) =>
        loggedDateSet.has(day.iso)
          ? { ...day, marker: "logged-period" as MarkerType }
          : day,
      ),
    [loggedDateSet],
  );
  const selectedDay = useMemo(
    () =>
      calendarDays.find((day) => day.iso === selectedIso) ?? calendarDays[11],
    [calendarDays, selectedIso],
  );
  const cycleSummary = useMemo(
    () => buildCycleSummary(periodEntries),
    [periodEntries],
  );
  ...
  function handleQuickAction(type: QuickLogType) {
    ...
    if (type === "symptom") {
      setQuickLogOpen(false);
      setPlaceholderMessage(
        "Use the symptom quick log chips below to save private symptom context.",
      );
      return;
    }
    setPlaceholderMessage(
      `${action?.label ?? "This log type"} will be added in a later Women's Health step.`,
    );
  }

  function savePeriod(
    draft: Pick<
      WomensHealthPeriodEntry,
      "endDate" | "flow" | "loggedDates" | "note" | "startDate"
    >,
  ) {
    ...
    setPlaceholderMessage("");
    setSavedMessage(
      "Period dates saved locally for now. They are private to this screen until persistence is added.",
    );
  }
      ...
      >
        ...
        {SCREEN_STATE === "error" ? (
          <WomensHealthState
            ...
            title="We couldn't load Women's Health"
          />
        ) : null}
        {SCREEN_STATE === "empty" ? (
          <WomensHealthState
            action="Set up Women's Health"
            icon="pregnancy_cycle"
            message="Choose what you would like to track, such as cycle dates, symptoms, contraception or personal notes."
            title="Start your private Women's Health space"
          />
        ) : null}
        {!FEATURE_ENABLED ? (
          <WomensHealthState
            ...
            icon="privacy"
            message="This optional feature can be enabled from the profile's health settings."
            title="Women's Health is not enabled for this profile"
          />
        ) : null}
        {SCREEN_STATE === "ready" && FEATURE_ENABLED ? (
          <>
            ...
            {placeholderMessage && !quickLogOpen ? (
              <InlineStatusMessage
                tone="info"
                message={placeholderMessage}
                onDismiss={() => setPlaceholderMessage("")}
              />
            ) : null}
            <PrivateStatusHero />
            ...
            <QuickLogActions
              ...
              onOpenSelector={() => setQuickLogOpen(true)}
              onQuickAction={handleQuickAction}
            />
            <SymptomQuickLog
              onSave={() => {
                setSavedMessage(
                  `${selectedSymptoms.length} symptom ${selectedSymptoms.length === 1 ? "entry" : "entries"} saved privately for this session.`,
                );
                setSelectedSymptoms([]);
              }}
              onToggle={(symptom) =>
            ...
            />
            {periodSheetOpen ? (
              <WomensHealthPeriodLogSheet
                key={`period-${selectedIso}`}
                onCancel={() => setPeriodSheetOpen(false)}
                onSave={savePeriod}
    ...
    <AppCard style={[styles.privateHero, { borderColor: `${ACCENT}35` }]}>
      ...
      <View style={styles.privateHeroTop}>
        <View style={styles.privateHeroIcon}>
          <AppIcon color={ACCENT} decorative name="privacy" size={25} />
        </View>
        <View style={styles.privateHeroCopy}>
          ...
          <Text style={[styles.privateHeroTitle, { color: theme.text }]}>
            Your private health space
          </Text>
          <Text style={[styles.privateHeroBody, { color: theme.mutedText }]}>
            Cycle, pregnancy, contraception, symptoms, and notes stay private
  ...
  return (
    <View style={styles.header}>
      <View style={styles.headerTop}>
        <Pressable
          accessibilityHint="Returns to the Health overview"
        ...
        <View
          accessibilityLabel={`Active profile, ${PROFILE.name}. ${PROFILE.privacyStatus}.`}
          accessible
          style={[
            styles.profilePill,
            { backgroundColor: theme.surface, borderColor: theme.border },
        ...
        >
          <View
            style={[styles.profileIcon, { backgroundColor: `${ACCENT}18` }]}
          >
      ...
      <View style={styles.titleBlock}>
        ...
        <Text style={[styles.subtitle, { color: theme.mutedText }]}>
          Private cycle, symptom and contraception tracking
        </Text>
        <View
          accessibilityLabel={`Privacy status, ${PROFILE.privacyStatus}`}
          accessible
          style={[
            styles.privacyPill,
        ...
        >
          ...
          <Text style={[styles.privacyPillText, { color: ACCENT }]}>
  ...
  return (
    <AppCard
      style={[
        ...
        {
          backgroundColor:
            theme.background === "#0f172a" ? "rgba(67,40,55,0.82)" : "#fff7f4",
    ...
    >
      ...
      <Text style={[styles.eyebrow, { color: ACCENT }]}>
        Cycle / pregnancy overview
      </Text>
      <View style={styles.cycleOrbRow}>
        <View
          accessibilityLabel={`Cycle day ${summary.cycleDay} of an estimated ${summary.averageLengthDays}-day cycle`}
          style={[
            styles.cycleOrb,
            { backgroundColor: theme.surface, borderColor: `${ACCENT}38` },
          ]}
        >
          <Text style={[styles.cycleOrbLabel, { color: theme.mutedText }]}>
            Cycle day
          </Text>
          <Text style={[styles.cycleOrbValue, { color: theme.text }]}>
            {summary.cycleDay}
          </Text>
          <Text style={[styles.cycleOrbMeta, { color: ACCENT }]}>
            of est. {summary.averageLengthDays}
          </Text>
        </View>
        <View style={styles.cycleModeCopy}>
          <Text style={[styles.cardTitle, { color: theme.text }]}>
            Estimated {summary.phase.toLowerCase()} phase
          </Text>
          <Text style={[styles.cardBody, { color: theme.mutedText }]}>
            Cycle estimates may shift as you add private logs.
          </Text>
          <Pressable
            accessibilityRole="button"
            onPress={() => router.push("/pregnancy")}
          >
            <Text style={[styles.inlineAction, { color: ACCENT }]}>
      ...
      <View style={styles.heroGrid}>
        <HeroStat
          ...
          value={summary.nextPeriodEstimate}
        />
      ...
      {summary.loggedDayCount > 0 ? (
        <Text style={[styles.heroSupport, { color: theme.mutedText }]}>
          {summary.loggedDayCount} period{" "}
          {summary.loggedDayCount === 1 ? "day" : "days"} saved locally in this
          session.
        </Text>
      ) : null}
    </AppCard>
...
function SymptomQuickLog({
  ...
  return (
    <AppSection
      subtitle="Choose any symptoms you want to remember. No automatic interpretation is added."
      title="Symptom quick log"
    >
          ...
          return (
            <Pressable
              ...
              style={[
                styles.symptomChip,
                {
                  backgroundColor: active ? `${ACCENT}16` : theme.surface,
            ...
            >
              <Text
                style={[
                  ...
                  { color: active ? ACCENT : theme.mutedText },
                ]}
              >
                {symptom}
              </Text>
      ...
      <Pressable
        ...
        onPress={onSave}
        style={[styles.symptomSave, { opacity: selected.length ? 1 : 0.48 }]}
      >
        <Text style={styles.symptomSaveText}>
          {selected.length
            ? `Save ${selected.length} privately`
            : "Choose symptoms to save"}
        </Text>
      </Pressable>
    </AppSection>
  );
...
function CompactCalendar({
  ...
  return (
    <AppCard
      style={[
        styles.calendarCard,
        { backgroundColor: theme.surface, borderColor: `${ACCENT}30` },
      ]}
    >
      <View style={styles.calendarHeader}>
        <View>
          ...
          <Text style={[styles.sectionMeta, { color: theme.mutedText }]}>
            Private cycle context
          </Text>
        </View>
        <View
          accessibilityLabel={`Selected date, ${formatDateSpeech(selectedDay.iso)}`}
          accessible
          style={[styles.selectedDateBadge, { backgroundColor: `${ACCENT}12` }]}
        >
          <Text style={[styles.selectedDateText, { color: ACCENT }]}>
            Selected {selectedDay.date}
          </Text>
        </View>
      </View>
      <View
        accessibilityLabel="June 2026 Women's Health calendar"
        style={styles.weekdays}
      >
        {WEEKDAYS.map((day) => (
          <Text
            key={day.key}
            style={[styles.weekday, { color: theme.mutedText }]}
          >
            {day.label}
          </Text>
        ))}
      ...
      <View style={styles.dayGrid}>
        {days.map((day) => (
          <CalendarDayButton
            day={{ ...day, isSelected: day.iso === selectedIso }}
            key={day.iso}
...
function CalendarDayButton({
  ...
  return (
    ...
    >
      {day.marker ? (
        <View pointerEvents="none" style={[styles.dayHalo, markerStyle]} />
      ) : null}
      <Text
        style={[
          styles.dayNumber,
          { color: day.isSelected ? ACCENT : theme.text },
        ]}
      >
        {day.date}
      </Text>
      {day.isToday ? (
        <View
          pointerEvents="none"
          style={[styles.todayDot, { backgroundColor: TEAL }]}
        />
      ) : null}
    </Pressable>
  );
...
function QuickLogActions({
  ...
  compact: boolean;
  onOpenSelector: () => void;
  onQuickAction: (type: QuickLogType) => void;
}) {
  const { theme } = useAppTheme();
  return (
    <AppSection
      actionAccessibilityHint="Opens all Women's Health quick log choices"
      actionAccessibilityLabel="Open Women's Health quick log selector"
      actionLabel="Quick log"
      onActionPress={onOpenSelector}
      subtitle="Log period dates now. Other private entries are prepared for later steps."
      title="Quick log"
    >
        ...
        {QUICK_ACTIONS.map((action) => (
          <Pressable
            accessibilityHint={
              action.available
                ? action.description
                : "This planned quick log action will show a prepared message for now."
            }
            ...
            style={({ pressed }) => [
              styles.quickAction,
              compact ? styles.quickActionCompact : null,
              { backgroundColor: theme.surface, borderColor: theme.border },
              pressed ? styles.pressed : null,
            ]}
          >
            <View
              style={[styles.quickIcon, { backgroundColor: `${ACCENT}14` }]}
            >
...
function TodayContextCard({ selectedDay }: { selectedDay: CalendarDay }) {
  const { theme } = useAppTheme();
  const markerText = selectedDay.marker
    ? getMarkerLabel(selectedDay.marker)
    : "No entry yet";
  return (
    <AppCard
      style={[
    ...
    >
      ...
      <Text style={[styles.phaseText, { color: theme.text }]}>
        Estimated {CYCLE.phase.toLowerCase()} phase
      </Text>
      <Text style={[styles.cardBody, { color: theme.mutedText }]}>
        This is based on saved cycle dates and may change when new information
        is added.
      </Text>
      <View style={[styles.contextRow, { borderColor: theme.border }]}>
        <Text style={[styles.contextLabel, { color: theme.mutedText }]}>
          Selected date
        </Text>
        <Text style={[styles.contextValue, { color: theme.text }]}>
          {formatDateSpeech(selectedDay.iso)} - {markerText}
        </Text>
      </View>
    </AppCard>
  );
...
function RecentSymptomsAndNotes() {
  return (
    <AppSection
      subtitle="User-entered context only, without automatic interpretation."
      title="Recent symptoms and notes"
    >
      <View style={styles.listStack}>
        {RECENT_ENTRIES.map((entry) => (
          <PreviewRow entry={entry} key={entry.id} />
        ))}
      </View>
    </AppSection>
  );
...
function PreviewRow({ entry }: { entry: PreviewEntry }) {
  ...
  return (
    <Pressable
      accessibilityLabel={`${entry.title}, ${entry.category.toLowerCase()} entry, saved ${entry.time}. ${entry.source}.`}
      accessibilityRole="button"
      onPress={() => undefined}
      style={({ pressed }) => [
        styles.previewRow,
    ...
    >
      <View style={[styles.rowIcon, { backgroundColor: `${ACCENT}12` }]}>
        <AppIcon color={ACCENT} decorative name={entry.icon} size={18} />
      </View>
      <View style={styles.rowCopy}>
        ...
        {entry.note ? (
          <Text
            numberOfLines={2}
            style={[styles.rowNote, { color: theme.mutedText }]}
          >
            {entry.note}
          </Text>
        ) : null}
        <Text style={[styles.rowSource, { color: ACCENT }]}>
          {entry.source}
        </Text>
      </View>
    </Pressable>
  ...
  return (
    <AppSection title="Contraception">
      ...
      >
        <View
          accessible
          accessibilityLabel="Daily pill. Next reminder 20:00 today. Last logged 08:05 today."
          style={styles.contraceptionTop}
        >
          <View style={[styles.rowIcon, { backgroundColor: `${TEAL}14` }]}>
            <AppIcon color={TEAL} decorative name="reminder" size={19} />
          </View>
          <View style={styles.rowCopy}>
            <Text style={[styles.cardTitle, { color: theme.text }]}>
              Daily pill
            ...
            <Text style={[styles.cardBody, { color: theme.mutedText }]}>
              Next reminder: 20:00 today
            </Text>
            <Text style={[styles.cardBody, { color: theme.mutedText }]}>
              Last logged: 08:05 today
            </Text>
          </View>
        </View>
        <Pressable
          accessibilityHint="Opens private contraception tracking."
          accessibilityLabel="View contraception tracking"
          accessibilityRole="button"
          onPress={() => router.push("/cycle?tab=contraception")}
        >
          <Text style={[styles.inlineAction, { color: TEAL }]}>
            View tracking
          </Text>
        </Pressable>
      </AppCard>
...
function ReminderRow({ reminder }: { reminder: ReminderEntry }) {
  ...
  return (
    <View
      accessibilityLabel={`${reminder.title}. ${reminder.time}. Source: ${reminder.source}.`}
      accessible
      style={[
        styles.reminderRow,
        { backgroundColor: theme.surface, borderColor: theme.border },
    ...
    >
      <View style={[styles.rowIcon, { backgroundColor: `${WARM}12` }]}>
        <AppIcon color={WARM} decorative name={reminder.icon} size={18} />
      </View>
      <View style={styles.rowCopy}>
        <Text
          numberOfLines={2}
...
function PrivacyCard() {
  ...
  return (
    ...
    >
      <View
        accessible
        accessibilityLabel={`Privacy. This Women's Health information is private to this profile. Nothing is shared unless permission is explicitly given. ${PROFILE.privacyStatus}.`}
        style={styles.infoRow}
      >
        <View style={[styles.infoIcon, { backgroundColor: `${ACCENT}14` }]}>
          <AppIcon color={ACCENT} decorative name="privacy" size={19} />
        </View>
        <View style={styles.rowCopy}>
          <Text style={[styles.cardTitle, { color: theme.text }]}>Privacy</Text>
          <Text style={[styles.cardBody, { color: theme.mutedText }]}>
            This Women{"'"}s Health information is private to this profile.
          </Text>
          <Text style={[styles.cardBody, { color: theme.mutedText }]}>
            Nothing is shared unless permission is explicitly given.
          </Text>
          ...
          >
            <Text style={[styles.inlineAction, { color: ACCENT }]}>
              Review sharing and privacy controls
            </Text>
          </Pressable>
        </View>
      </View>
  ...
  return (
    ...
    >
      <View style={styles.infoRow}>
        <View style={[styles.infoIcon, { backgroundColor: `${GOLD}14` }]}>
          <AppIcon color={GOLD} decorative name="health" size={19} />
        </View>
        <View style={styles.rowCopy}>
          <Text style={[styles.cardTitle, { color: theme.text }]}>
          ...
          <Text style={[styles.cardBody, { color: theme.mutedText }]}>
            Cycle and fertility dates are estimates based on the information
            entered. They can change and should not be used as medical
            confirmation or contraception advice.
          </Text>
          <Pressable
            accessibilityHint="Educational content will be added in a later Women's Health brick."
            accessibilityLabel="Learn how estimates work"
            accessibilityRole="button"
            onPress={() => undefined}
          >
            <Text style={[styles.inlineAction, { color: GOLD }]}>
  ...
  return (
    <View style={[styles.safetyNote, { borderColor: `${ACCENT}35` }]}>
      ...
      <Text style={[styles.safetyText, { color: theme.mutedText }]}>
        This app helps organize information you choose to track. It does not
        diagnose conditions or replace advice from a doctor, nurse, pharmacist,
        clinic or other healthcare professional. Cycle and fertility estimates
        may be inaccurate.
      </Text>
    </View>
  );
...
function WomensHealthLoadingState() {
  ...
  return (
    <View
      accessible
      accessibilityLabel="Loading Women's Health"
      accessibilityRole="progressbar"
      style={styles.loadingStack}
    >
      {[96, 190, 260, 112, 132].map((height) => (
        <View
          key={height}
          style={[
            styles.skeleton,
...
function WomensHealthState({
  ...
  return (
    ...
    >
      <View style={[styles.stateIcon, { backgroundColor: `${ACCENT}14` }]}>
        <AppIcon color={ACCENT} decorative name={icon} size={25} />
      </View>
      <Text
        accessibilityRole="header"
      ...
      <Text style={[styles.stateMessage, { color: theme.mutedText }]}>
        {message}
      </Text>
      <AppButton
        accessibilityHint="This planned setup action will be connected in a later Women's Health brick."
  ...
  return (
    ...
    >
      <AppIcon
        color={color}
        decorative
        name={tone === "success" ? "checkin" : "health"}
        size={18}
      />
      <Text style={[styles.inlineStatusText, { color: theme.text }]}>
        {message}
      </Text>
...
function WomensHealthQuickLogSheet({
  message,
  onClose,
  onSelect,
  visible,
  ...
  message: string;
  onClose: () => void;
  onSelect: (type: QuickLogType) => void;
  visible: boolean;
}) {
  ...
  return (
    ...
    >
      <View style={styles.modal}>
        <Pressable
          accessibilityLabel="Close quick log selector"
          accessibilityRole="button"
          onPress={onClose}
          style={styles.scrim}
        />
        <View pointerEvents="box-none" style={styles.keyboard}>
          ...
          >
            ...
            <SheetHeader
              icon="pregnancy_cycle"
              profileName={PROFILE.name}
              subtitle="Choose what to add to this private Women's Health space."
              title="Quick log"
            />
                ...
                {QUICK_ACTIONS.map((action) => (
                  <Pressable
                    accessibilityHint={
                      action.available
                        ? action.description
                        : "This option is prepared but not active yet."
                    }
                    accessibilityLabel={action.accessibilityLabel}
                    accessibilityRole="button"
                    key={action.type}
                  ...
                  >
                    ...
                    >
                      <AppIcon
                        color={action.available ? ACCENT : theme.mutedText}
                        decorative
                        name={action.icon}
                        size={19}
                      />
...
function WomensHealthPeriodLogSheet({
  ...
  const insets = useSafeAreaInsets();
  const { theme } = useAppTheme();
  const defaultIso = CALENDAR_DAYS.some((day) => day.iso === selectedIso)
    ? selectedIso
    : "2026-06-09";
  ...
  return (
    ...
    >
      <View style={styles.modal}>
        <Pressable
          accessibilityLabel="Cancel period log"
          accessibilityRole="button"
          onPress={cancel}
          style={styles.scrim}
        />
        ...
        >
          ...
          >
            ...
            <SheetHeader
              ...
              subtitle="Save the first and last day for this period entry."
              title="Log period"
            />
            <ScrollView
            ...
            >
              ...
              <ChipSelector
                label="Flow summary (optional)"
                onSelect={setFlow}
                options={FLOW_OPTIONS}
                selected={flow}
              />
              <AppFormInput
                accessibilityLabel="Private period note"
                helperText="Saved locally in this session only."
              ...
              >
                ...
                <Text style={[styles.helperText, { color: theme.mutedText }]}>
                  This first version saves locally on this screen. Individual
                  day exclusion and editing will be added later.
                </Text>
              </View>
            </ScrollView>
            <View
            ...
            >
              ...
              <Pressable
                accessibilityHint="Validates dates and saves this period entry locally"
...
function SheetHeader({
  ...
  return (
    <View style={[styles.sheetHeader, { borderBottomColor: theme.border }]}>
      ...
      <Text style={[styles.sheetSubtitle, { color: theme.mutedText }]}>
        {subtitle}
      </Text>
      <View
        accessibilityLabel={`Active profile, ${profileName}, private Women's Health log`}
        accessible
        style={[
          styles.sheetProfile,
...
function InlineSheetMessage({ message }: { message: string }) {
  ...
  return (
    ...
    >
      ...
      <Text style={[styles.sheetMessageText, { color: theme.text }]}>
        {message}
      </Text>
    </View>
  );
          ...
          return (
            <Pressable
              ...
              style={({ pressed }) => [
                styles.dateChip,
                {
                  backgroundColor: active ? `${ACCENT}16` : theme.background,
                  borderColor: active ? ACCENT : theme.border,
                },
                pressed ? styles.pressed : null,
              ]}
...
function SelectedRangeSummary({ dates }: { dates: string[] }) {
  ...
  return (
    <View
      accessibilityLabel={`Selected period range, ${dates.length} ${dates.length === 1 ? "day" : "days"}`}
      accessible
      style={[
        styles.rangeSummary,
        { backgroundColor: `${ACCENT}0D`, borderColor: `${ACCENT}30` },
      ]}
    >
      ...
      <Text style={[styles.rangeSummaryMeta, { color: theme.mutedText }]}>
        {dates.length
          ? `${formatDateSpeech(dates[0])} to ${formatDateSpeech(dates[dates.length - 1])}`
          : "Choose dates"}
      </Text>
    </View>
  );
          ...
          return (
            <Pressable
              ...
              style={({ pressed }) => [
                styles.chip,
                {
                  backgroundColor: active ? `${ACCENT}14` : theme.background,
                  borderColor: active ? ACCENT : theme.border,
                },
                pressed ? styles.pressed : null,
              ]}
...
function buildCycleSummary(entries: WomensHealthPeriodEntry[]): CycleSummary {
  ...
  const loggedDayCount = new Set(
    profileEntries.flatMap((entry) => entry.loggedDates),
  ).size;
  const latest = profileEntries.reduce<WomensHealthPeriodEntry | undefined>(
    (current, entry) => {
  ...
  return {
    ...
    dataStatus:
      loggedDayCount > 0
        ? "Based on the period dates saved locally in this session. Estimates have not been recalculated yet."
        : "Based on the cycle information you have logged.",
...
function validatePeriodDates(startDate: string, endDate: string) {
  ...
  if (!getDateRange(startDate, endDate).length)
    return "Choose at least one period day.";
  return "";
}
