...
const NOTE_GROUP_ORDER = ["Today", "Yesterday", "Earlier this week", "Earlier this month", "Older"] as const;

type HealthNoteRange = (typeof TIME_RANGES)[number]["value"];
type HealthNoteCategory = Exclude<(typeof CATEGORY_OPTIONS)[number]["value"], "all">;
...
export function HealthNotesScreen() {
  ...
  const [selectedNote, setSelectedNote] = useState<GeneralHealthActivityEntry | null>(null);
  ...
  const visibleNotes = useMemo(
    () => filterHealthNotesByCategory(filterHealthNotesByRange(profileNotes, range, openedAt), category),
    [category, openedAt, profileNotes, range]
  );
  const noteGroups = useMemo(() => groupHealthNotesByDate(visibleNotes, openedAt), [openedAt, visibleNotes]);
  const selectedVisibleNote = selectedNote?.profileId === selectedProfileId ? selectedNote : null;
  const visibleSavedMessage = savedMessageProfileId === selectedProfileId ? savedMessage : "";
  const categoryOptions = useMemo(() => {
    const existingCategories = new Set(profileNotes.map((note) => note.category));
    return CATEGORY_OPTIONS.filter((option) => option.value === "all" || existingCategories.has(option.value));
  }, [profileNotes]);
    ...
    <View style={[styles.screen, { backgroundColor: theme.background }]}>
      <HealthScreenContainer contentStyle={styles.content} maxWidth={470}>
        ...
        {NOTES_STATE === "ready" ? (
          <>
            {profileNotes.length ? (
              <HealthNotesSummaryCard latestNote={profileNotes[0]} profileName={selectedProfileName} total={profileNotes.length} />
            ) : null}
            <HealthNotesFilters category={category} categoryOptions={categoryOptions} range={range} setCategory={setCategory} setRange={setRange} />
            ...
            {profileNotes.length ? (
              visibleNotes.length ? (
                <GroupedHealthNotes groups={noteGroups} onOpen={(note) => setSelectedNote(note.activity)} />
              ) : (
            ...
            <AppButton accessibilityLabel="Add health note" fullWidth label="Add health note" onPress={() => setActiveLogType("note")} />
            <HealthNotesPrivacyNote />
          </>
        ) : null}
      </HealthScreenContainer>
      <GeneralHealthActivityDetailSheet activity={selectedVisibleNote} onClose={() => setSelectedNote(null)} />
      <GeneralHealthLogSheet key={activeLogType ?? "closed"} logType={activeLogType} onCancel={() => setActiveLogType(null)} onSave={saveHealthNote} profileName={selectedProfileName} />
    </View>
  );
}
  ...
  return (
    <View style={styles.header}>
      <View style={styles.headerTop}>
        ...
        <View accessibilityLabel={`Active profile, ${profileName}`} accessible style={[styles.profilePill, { backgroundColor: theme.surface, borderColor: theme.border }]}>
      ...
      <View>
        <Text accessibilityRole="header" style={[styles.title, { color: theme.text }]}>Health notes</Text>
        <Text style={[styles.subtitle, { color: theme.mutedText }]}>Personal notes and saved context</Text>
      </View>
    </View>
  );
}

function HealthNotesSummaryCard({ latestNote, profileName, total }: { latestNote: HealthNoteItem; profileName: string; total: number }) {
  const { theme } = useAppTheme();
  return (
    <AppCard
      style={[styles.summaryCard, { backgroundColor: `${ACCENT}10`, borderColor: `${ACCENT}35` }]}
    >
      <View accessible accessibilityLabel={`${total} health ${total === 1 ? "note" : "notes"} saved for ${profileName}. Latest note: ${latestNote.title}, saved ${formatAccessibleDateTime(latestNote.createdAt)}.`} style={styles.summaryContent}>
        <View style={styles.summaryTop}>
          <View>
            <Text style={[styles.summaryEyebrow, { color: ACCENT }]}>Health notes</Text>
            <Text style={[styles.summaryCount, { color: theme.text }]}>{formatSavedNoteCount(total)}</Text>
          </View>
          <View style={[styles.summaryIcon, { backgroundColor: theme.surface }]}>
            <AppIcon color={ACCENT} decorative name="edit" size={22} />
          </View>
        </View>
        <View style={styles.latestBlock}>
          <Text style={[styles.latestLabel, { color: theme.mutedText }]}>Latest note</Text>
          <Text numberOfLines={2} style={[styles.latestTitle, { color: theme.text }]}>{latestNote.title}</Text>
          <Text style={[styles.latestTime, { color: theme.mutedText }]}>Saved {formatRelativeDateTime(latestNote.createdAt)}</Text>
        </View>
        <Text style={[styles.summarySupport, { color: theme.mutedText }]}>Notes help you remember useful context over time.</Text>
      </View>
    </AppCard>
  );
}
...
function HealthNotesFilters({
  ...
  categoryOptions: readonly { label: string; value: HealthNoteCategoryFilter }[];
  ...
  return (
    <View style={styles.filters}>
      ...
      <FilterGroup label="Category">
        <ScrollView contentContainerStyle={styles.categoryChips} horizontal showsHorizontalScrollIndicator={false}>
...
function FilterChip({ accessibilityLabel, label, onPress, selected }: { accessibilityLabel: string; label: string; onPress: () => void; selected: boolean }) {
  const { theme } = useAppTheme();
  return (
    <Pressable
      ...
      onPress={onPress}
      style={({ pressed }) => [styles.chip, selected ? styles.chipSelected : null, { backgroundColor: selected ? theme.primarySoft : theme.surface, borderColor: selected ? theme.primary : theme.border }, pressed ? styles.pressed : null]}
    >
      <View style={[styles.chipMarker, { borderColor: selected ? theme.primary : theme.border, backgroundColor: selected ? theme.primary : "transparent" }]}>
        {selected ? <AppIcon color="#ffffff" decorative name="success" size={11} strokeWidth={3} /> : null}
      </View>
      <Text style={[styles.chipText, selected ? styles.chipTextSelected : null, { color: selected ? theme.primary : theme.text }]}>{label}</Text>
    </Pressable>
  );
}

function GroupedHealthNotes({ groups, onOpen }: { groups: { entries: HealthNoteItem[]; label: HealthNoteGroupLabel }[]; onOpen: (note: HealthNoteItem) => void }) {
  const { theme } = useAppTheme();
  return (
      ...
      {groups.map((group) => (
        <View key={group.label} style={styles.group}>
          <View style={styles.groupHeading}>
            <Text accessibilityRole="header" style={[styles.groupLabel, { color: theme.mutedText }]}>{group.label}</Text>
            <Text style={[styles.groupCount, { color: theme.mutedText }]}>{group.entries.length} {group.entries.length === 1 ? "note" : "notes"}</Text>
          </View>
          <View style={styles.noteStack}>
            {group.entries.map((note) => <HealthNoteCard key={note.id} note={note} onPress={() => onOpen(note)} />)}
          </View>
        </View>
      ))}
    </View>
...
function HealthNoteCard({ note, onPress }: { note: HealthNoteItem; onPress: () => void }) {
  const { theme } = useAppTheme();
  ...
  return (
    <Pressable
      ...
      accessibilityRole="button"
      onPress={onPress}
      style={({ pressed }) => [styles.noteCard, { backgroundColor: theme.surface, borderColor: theme.border }, pressed ? styles.pressedCard : null]}
    >
      <View style={[styles.noteIcon, { backgroundColor: theme.primarySoft }]}>
        <AppIcon color={theme.primary} decorative name={getHealthNoteCategoryIcon(note.category)} size={20} />
      </View>
      <View style={styles.noteCopy}>
        <View style={styles.noteHeading}>
          <Text numberOfLines={2} style={[styles.noteTitle, { color: theme.text }]}>{note.title}</Text>
          <Text style={[styles.chevron, { color: theme.mutedText }]}>{">"}</Text>
        </View>
        {note.details ? <Text numberOfLines={3} style={[styles.notePreview, { color: theme.mutedText }]}>{note.details}</Text> : null}
        <View style={styles.noteMetaRow}>
          <Text style={[styles.noteMeta, { color: theme.mutedText }]}>{categoryLabel}</Text>
          <Text style={[styles.noteMetaSeparator, { color: theme.mutedText }]}>-</Text>
          <Text style={[styles.noteMeta, { color: theme.mutedText }]}>{formatRelativeDateTime(note.createdAt)}</Text>
        </View>
        <View style={styles.noteOwner}>
          <View style={[styles.ownerIcon, { backgroundColor: theme.primarySoft }]}>
            <AppIcon color={theme.primary} decorative name="profile" size={12} />
          </View>
          <Text style={[styles.ownerName, { color: theme.text }]}>{note.profileName}</Text>
        </View>
      </View>
    </Pressable>
  );
...
function HealthNotesEmptyState({ actionLabel, message, onAction, title }: { actionLabel?: string; message: string; onAction?: () => void; title: string }) {
  const { theme } = useAppTheme();
  return (
    <AppCard style={[styles.empty, { borderColor: theme.border }]}>
      ...
      {actionLabel && onAction ? <AppButton accessibilityLabel={actionLabel === "Reset filters" ? "Reset health note filters" : "Add health note"} label={actionLabel} onPress={onAction} variant="secondary" /> : null}
    </AppCard>
  );
}
...
function HealthNotesLoadingState() {
  const { theme } = useAppTheme();
  return (
    <View accessible accessibilityLabel="Loading health notes" accessibilityRole="progressbar" style={styles.stateStack}>
      {[86, 138, 112, 108].map((height) => <View key={height} style={[styles.skeleton, { backgroundColor: theme.surface, borderColor: theme.border, height }]} />)}
    </View>
  );
}

function HealthNotesErrorState() {
  return <HealthNotesEmptyState message="Please return to General Health and open notes again." title={"We couldn't load health notes"} />;
}
  ...
  return (
    <View style={[styles.privacyNote, { borderColor: `${ACCENT}40` }]}>
      ...
      <View style={styles.privacyCopy}>
        <Text style={[styles.privacyText, { color: theme.mutedText }]}>Health notes are private to the selected profile unless that person explicitly shares them.</Text>
        <Text style={[styles.privacyText, { color: theme.mutedText }]}>Notes help store personal context and are not medical findings or diagnoses.</Text>
      </View>
    </View>
  );
}
...
function getHealthNotesForProfile(entries: GeneralHealthActivityEntry[], profileId: string) {
  return entries
...
function normalizeHealthNote(entry: Extract<GeneralHealthActivityEntry, { type: "note" }>): HealthNoteItem | null {
  const createdAt = new Date(entry.createdAt);
  if (!Number.isFinite(createdAt.getTime())) return null;
  if (!entry.id.trim() || !entry.profileId.trim() || !entry.profileName.trim()) return null;
  const title = typeof entry.details.noteTitle === "string" && entry.details.noteTitle.trim()
    ? entry.details.noteTitle.trim()
    : typeof entry.summary === "string" && entry.summary.trim() && entry.summary.trim() !== "Health note"
      ? entry.summary.trim()
      : "";
  ...
  const details = typeof entry.details.details === "string" ? entry.details.details.trim() : "";
  return { activity: entry, category, createdAt, details, id: entry.id, profileName: entry.profileName.trim(), title };
}

function filterHealthNotesByRange(notes: HealthNoteItem[], range: HealthNoteRange, referenceDate: Date) {
  if (range === "all") return notes;
  const cutoff = new Date(referenceDate.getTime() - range * 24 * 60 * 60 * 1000);
  return notes.filter((note) => note.createdAt >= cutoff);
}

function filterHealthNotesByCategory(notes: HealthNoteItem[], category: HealthNoteCategoryFilter) {
...
function normalizeHealthNoteCategory(value?: string): HealthNoteCategory {
  ...
  if (normalized === "symptom" || normalized === "energy" || normalized === "sleep" || normalized === "medication" || normalized === "appointment" || normalized === "general") {
    return normalized;
  }
  return "general";
}
...
function getResultSummary(count: number, range: HealthNoteRange, category: HealthNoteCategoryFilter) {
  const rangeLabel = range === "all" ? "" : ` in the last ${range} days`;
  const categoryLabel = category === "all" ? "" : `${formatHealthNoteCategory(category)} `;
...
const styles = StyleSheet.create({
  ...
  chip: { alignItems: "center", borderRadius: 18, borderWidth: 1, flexDirection: "row", gap: 7, minHeight: 42, paddingHorizontal: 12 },
  chipMarker: { alignItems: "center", borderRadius: 999, borderWidth: 1.5, height: 18, justifyContent: "center", width: 18 },
  ...
  noteCard: { alignItems: "flex-start", borderRadius: 20, borderWidth: 1, flexDirection: "row", gap: 12, padding: 15 },
  noteCopy: { flex: 1, gap: 8, minWidth: 0 },
  noteHeading: { alignItems: "flex-start", flexDirection: "row" },
  noteIcon: { alignItems: "center", borderRadius: 15, height: 42, justifyContent: "center", width: 42 },
  noteMeta: { fontSize: 12, fontWeight: "800", lineHeight: 18 },
  noteMetaRow: { alignItems: "center", flexDirection: "row", flexWrap: "wrap", gap: 6 },
  ...
  ownerIcon: { alignItems: "center", borderRadius: 10, height: 22, justifyContent: "center", width: 22 },
  ownerName: { fontSize: 12, fontWeight: "900" },
  pressed: { opacity: 0.74 },
  pressedCard: { opacity: 0.78, transform: [{ scale: 0.995 }] },
  privacyCopy: { flex: 1, gap: 5 },
  privacyNote: { alignItems: "flex-start", borderLeftWidth: 3, flexDirection: "row", gap: 10, paddingHorizontal: 14, paddingVertical: 2 },
  ...
  summaryCount: { fontSize: 22, fontWeight: "900", lineHeight: 28, marginTop: 4 },
  summaryEyebrow: { fontSize: 12, fontWeight: "900", textTransform: "uppercase" },
  summaryIcon: { alignItems: "center", borderRadius: 17, height: 48, justifyContent: "center", width: 48 },
