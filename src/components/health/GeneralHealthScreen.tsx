...
export function GeneralHealthScreen() {
  ...
  const [activeLogType, setActiveLogType] =
    useState<GeneralHealthLogType | null>(null);
  const [savedMessage, setSavedMessage] = useState("");
  const [savedMessageProfileId, setSavedMessageProfileId] = useState("");
  const [bodyMetrics, setBodyMetrics] = useState(() =>
    GENERAL_HEALTH_BODY_METRICS.map((item) => ({ ...item })),
  );
  const [bodyMetricsProfileId, setBodyMetricsProfileId] = useState("");
  const [vitals, setVitals] = useState(() =>
    GENERAL_HEALTH_VITALS.map((item) => ({ ...item })),
  );
  const [vitalsProfileId, setVitalsProfileId] = useState("");
  const visibleBodyMetrics =
    bodyMetricsProfileId === selectedProfileId
      ? bodyMetrics
      : GENERAL_HEALTH_BODY_METRICS;
  const visibleVitals =
    vitalsProfileId === selectedProfileId ? vitals : GENERAL_HEALTH_VITALS;
  ...
  function saveLocalLog(draft: GeneralHealthLogDraft) {
    ...
    if (draft.logType === "vitals") {
      ...
      setVitals((current) =>
        current.map((item) => {
          if (item.label === "Heart rate" && draft.heartRate)
            return {
              ...item,
              status: "saved",
              updated,
              value: `${draft.heartRate} bpm`,
            };
          if (
            ...
            return {
              ...
              status: "saved",
              updated,
              value: `${draft.systolic} / ${draft.diastolic} mmHg`,
            };
          if (item.label === "Oxygen saturation" && draft.oxygen)
            return {
              ...item,
              status: "saved",
              updated,
  ...
  return (
    <View style={[styles.screen, { backgroundColor: theme.background }]}>
      <HealthScreenContainer bottomSpacing={120} contentStyle={styles.content}>
        ...
        {MOCK_GENERAL_HEALTH_STATE === "loading" ? (
          <GeneralHealthLoadingState />
        ) : null}
        {MOCK_GENERAL_HEALTH_STATE === "empty" ? (
          <GeneralHealthEmptyState onAction={() => setActiveLogType("note")} />
        ) : null}
        {MOCK_GENERAL_HEALTH_STATE === "error" ? (
          <GeneralHealthErrorState />
        ) : null}
        {MOCK_GENERAL_HEALTH_STATE === "ready" ? (
          <GeneralHealthReadyContent
            activity={activities}
            bodyMetrics={visibleBodyMetrics}
            onOpenLog={setActiveLogType}
    ...
    <View style={styles.header}>
      ...
      <View style={styles.heading}>
        ...
        <Text style={[styles.subtitle, { color: theme.mutedText }]}>
          Vitals, body metrics, notes, and trends
        </Text>
      </View>
    </View>
  );
...
function GeneralHealthReadyContent({
  ...
  return (
    <>
      ...
      <AppSection
        subtitle="Simple reading previews with no medical interpretation."
        title="Vitals"
      >
        <View style={styles.grid}>
          {vitals.map((item) => (
            <VitalPreviewCard item={item} key={item.label} stacked={stacked} />
          ))}
        </View>
      </AppSection>
      <AppSection
        subtitle="Saved measurements and future calculated fields."
        title="Body metrics"
      >
        <View style={styles.grid}>
          {bodyMetrics.map((item) => (
            <BodyMetricCard item={item} key={item.label} stacked={stacked} />
          ))}
        </View>
      </AppSection>
...
function RecentHealthNotesPreview({
  activity,
  profileId,
}: {
  activity: GeneralHealthActivityEntry[];
  ...
  return (
    ...
    >
      ...
      >
        <View
          style={[styles.notesPreviewIcon, { backgroundColor: `${ACCENT}14` }]}
        >
          <AppIcon color={ACCENT} decorative name="edit" size={20} />
        </View>
        <View style={styles.notesPreviewCopy}>
          <Text style={[styles.notesPreviewTitle, { color: theme.text }]}>
            Health notes
          </Text>
          <Text style={[styles.notesPreviewMeta, { color: theme.mutedText }]}>
            {notes.length
          ...
          {latestTitle ? (
            <Text
              numberOfLines={2}
              style={[styles.notesPreviewLatest, { color: theme.text }]}
            >
              {latestTitle}
            </Text>
          ) : null}
        </View>
        <Text style={[styles.notesPreviewArrow, { color: theme.mutedText }]}>
          {">"}
        </Text>
      </Pressable>
    </AppSection>
    ...
    >
      <View style={styles.snapshotHeading}>
        <View style={styles.snapshotCopy}>
          ...
          <Text
            accessibilityRole="header"
            style={[styles.snapshotTitle, { color: theme.text }]}
          >
            Today{"'"}s health snapshot
          </Text>
          <Text style={[styles.snapshotSubtitle, { color: theme.mutedText }]}>
            4 items tracked - 1 reminder later
          </Text>
        </View>
      ...
      <View style={styles.snapshotGrid}>
        {GENERAL_HEALTH_SNAPSHOT.map((item) => (
          <SnapshotItem item={item} key={item.label} stacked={stacked} />
        ))}
      </View>
    </AppCard>
  );
...
function SnapshotItem({
  ...
  item: GeneralHealthSnapshotItem;
  stacked: boolean;
}) {
  const { theme } = useAppTheme();
  return (
    <View
      accessible
      accessibilityLabel={`${item.label}, ${item.value}, ${item.status}`}
      style={[
        styles.snapshotItem,
        stacked ? styles.fullWidth : null,
        { backgroundColor: theme.surface, borderColor: theme.border },
      ]}
    >
      <AppIcon color={ACCENT} decorative name={item.icon} size={18} />
      <Text style={[styles.snapshotLabel, { color: theme.mutedText }]}>
        {item.label}
      </Text>
      <Text style={[styles.snapshotValue, { color: theme.text }]}>
        {item.value}
      </Text>
      <Text style={[styles.snapshotStatus, { color: ACCENT }]}>
        {item.status}
      </Text>
    </View>
  );
...
function QuickLogActions({
  ...
  return (
    <AppSection
      subtitle="Add a local entry for the active profile."
      title="Quick log"
    >
      <View style={styles.grid}>
        {GENERAL_HEALTH_QUICK_ACTIONS.map((action) => (
          ...
          >
            ...
            >
              <AppIcon color={ACCENT} decorative name={action.icon} size={20} />
            </View>
...
function VitalPreviewCard({
  item,
  stacked,
}: {
  item: GeneralHealthVital;
  stacked: boolean;
}) {
  const { theme } = useAppTheme();
  ...
  const interactive =
    isHeartRate || isBloodPressure || isTemperature || isOxygenSaturation;
  const route = isHeartRate
    ? "/health/general/heart-rate"
    : isBloodPressure
      ? "/health/general/blood-pressure"
      : isTemperature
        ? "/health/general/temperature"
        : "/health/general/oxygen-saturation";
  const accessibilityLabel = isHeartRate
    ? "Open heart rate trends"
    : isBloodPressure
      ? "Open blood pressure trends"
      : isTemperature
        ? "Open temperature trends"
        : "Open oxygen saturation trends";
  const accessibilityHint = isHeartRate
    ? "Opens saved heart rate readings and trends for the selected profile"
    : isBloodPressure
      ? "Opens saved blood pressure readings and trends for the selected profile"
      : isTemperature
        ? "Opens saved temperature readings and trends for the selected profile"
        : "Opens saved oxygen saturation readings and trends for the selected profile";
  const content = (
    <View
      accessible={!interactive}
      accessibilityLabel={`${item.label}, ${item.value}, ${item.status}, ${item.updated}`}
      style={styles.cardContent}
    >
      <View style={styles.previewTop}>
        <View style={[styles.previewIcon, { backgroundColor: `${ACCENT}14` }]}>
          <AppIcon color={ACCENT} decorative name={item.icon} size={20} />
        </View>
        <View
          style={[styles.cardStatusBadge, { backgroundColor: `${ACCENT}12` }]}
        >
          <Text style={[styles.previewStatus, { color: ACCENT }]}>
            {item.status}
          </Text>
        </View>
      </View>
      ...
      <Text style={[styles.updated, { color: theme.mutedText }]}>
        {item.updated}
      </Text>
    </View>
  );
...
function BodyMetricCard({
  ...
  return (
    ...
    >
      {isWeight ? (
        <Pressable
          accessibilityHint="Opens saved weight entries and personal trends for the selected profile"
          accessibilityLabel="Open weight trends"
          accessibilityRole="button"
          onPress={() => router.push("/health/general/weight")}
          style={({ pressed }) => (pressed ? styles.pressed : null)}
        >
          {content}
        </Pressable>
...
function WeeklyTrendPreviewCard() {
  ...
  return (
    <AppSection
      subtitle="A lightweight preview of saved general health activity."
      title="Weekly trends"
    >
      <AppCard style={[styles.trendCard, { borderColor: theme.border }]}>
        <Text style={[styles.trendSummary, { color: theme.text }]}>
          4 of 7 days have saved health activity.
        </Text>
        <View
          accessible
          accessibilityLabel="Monday, Wednesday, Thursday, and Saturday have saved health activity. Four of seven days saved."
          style={styles.bars}
        >
              ...
              >
                <View
                  style={[
                    styles.barFill,
                    {
                      backgroundColor: item.logged ? ACCENT : theme.border,
                      height: `${item.value}%`,
                    },
                  ]}
                />
              </View>
              <Text style={[styles.barDay, { color: theme.mutedText }]}>
                {item.day}
              </Text>
              <Text
                style={[
                  styles.barState,
                  { color: item.logged ? ACCENT : theme.mutedText },
                ]}
              >
                {item.logged ? "Saved" : "None"}
              </Text>
            </View>
          ))}
        </View>
...
function SafeHealthNote() {
  ...
  return (
    <View style={[styles.safeNote, { borderColor: `${ACCENT}45` }]}>
      ...
      <Text style={[styles.safeNoteText, { color: theme.mutedText }]}>
        This app helps you organize health information. Contact a healthcare
...
function GeneralHealthLoadingState() {
  ...
  return (
    <View
      accessible
      accessibilityLabel="Loading General Health"
      accessibilityRole="progressbar"
      style={styles.stateStack}
    >
      {[220, 130, 180].map((height) => (
        <View
          key={height}
          style={[
            styles.skeleton,
...
function GeneralHealthEmptyState({ onAction }: { onAction: () => void }) {
  return (
    <GeneralHealthState
      icon="add"
      message="Start with a quick note, weight entry, or vital reading."
      title="No general health logs yet"
...
function GeneralHealthErrorState() {
  return (
    <GeneralHealthState
      icon="warning"
      message="Your local General Health preview could not be shown."
      title="We couldn't load general health"
    />
  );
}

function GeneralHealthState({
  action,
  icon,
  message,
  onAction,
  ...
  return (
    <AppCard style={[styles.stateCard, { borderColor: theme.border }]}>
      <View style={[styles.stateIcon, { backgroundColor: `${ACCENT}14` }]}>
        <AppIcon color={ACCENT} decorative name={icon} size={26} />
      </View>
      <Text
        accessibilityRole="header"
