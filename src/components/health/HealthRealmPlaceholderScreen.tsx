...
export function HealthRealmPlaceholderScreen({
  realm,
}: {
  realm: HealthRealm;
}) {
  ...
  return (
    <View style={[styles.screen, { backgroundColor: theme.background }]}>
      ...
      >
        ...
        <View
          accessible
          accessibilityLabel={`${realm.title}. ${realm.subtitle}`}
          style={[
            styles.hero,
            {
              backgroundColor: `${realm.accent}12`,
              borderColor: `${realm.accent}45`,
            },
          ]}
        >
          <View style={styles.heroTop}>
            <View
              style={[
                styles.heroIcon,
                { backgroundColor: `${realm.accent}20` },
              ]}
            >
              <AppIcon
                color={realm.accent}
            ...
            <View
              style={[
                styles.status,
                {
                  ...
                  borderColor: `${realm.accent}35`,
                },
              ]}
            >
              <Text style={[styles.statusText, { color: realm.accent }]}>
                {realm.status}
              </Text>
            </View>
          </View>
          <Text style={[styles.eyebrow, { color: realm.accent }]}>
            {realm.eyebrow}
          </Text>
          <Text
        ...
        >
          <View
            style={[styles.introMarker, { backgroundColor: realm.accent }]}
          />
          <View style={styles.introCopy}>
            <Text
              accessibilityRole="header"
              style={[styles.introTitle, { color: theme.text }]}
            >
              {realm.introTitle}
            </Text>
        ...
        <View style={styles.previewSection}>
          <View style={styles.sectionHeading}>
            ...
            <Text style={[styles.sectionSubtitle, { color: theme.mutedText }]}>
              A preview of what will live in this realm.
            </Text>
          </View>
          <View style={styles.grid}>
            {realm.previews.map((preview) => (
              <RealmPreviewCard
                accent={realm.accent}
                key={preview.title}
                preview={preview}
                stacked={stackedPreviews}
              />
            ))}
          </View>
        </View>
        ...
        >
          <View
            style={[
              styles.plannedIcon,
              { backgroundColor: `${realm.accent}18` },
            ]}
          >
            <AppIcon
              color={realm.accent}
          ...
          <View style={styles.plannedCopy}>
            <Text style={[styles.plannedTitle, { color: theme.text }]}>
              Planned area
            </Text>
            <Text style={[styles.plannedText, { color: theme.mutedText }]}>
              Tracking and organizing actions are planned for a later phase.
            </Text>
          </View>
        </AppCard>
        ...
        {realm.safetyNote ? (
          <View
            accessible
            accessibilityLabel={`Health note: ${realm.safetyNote}`}
            style={[styles.note, { borderColor: `${realm.accent}40` }]}
          >
            <Text style={[styles.noteTitle, { color: realm.accent }]}>
              Health note
            </Text>
...
function RealmPreviewCard({
  accent,
  preview,
  stacked,
}: {
  accent: string;
  preview: HealthRealmPreview;
  stacked: boolean;
}) {
  const { theme } = useAppTheme();
  return (
    <AppCard
      style={[
        styles.preview,
        stacked ? styles.previewStacked : null,
        { backgroundColor: theme.surface, borderColor: `${accent}35` },
      ]}
    >
      ...
      <Text style={[styles.previewDescription, { color: theme.mutedText }]}>
        {preview.description}
      </Text>
      <View
        style={[styles.previewStatusBadge, { backgroundColor: `${accent}12` }]}
      >
