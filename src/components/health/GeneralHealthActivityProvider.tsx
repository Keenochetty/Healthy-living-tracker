...
const GeneralHealthActivityContext =
  createContext<GeneralHealthActivityContextValue | null>(null);

export function GeneralHealthActivityProvider({
  children,
}: {
  children: ReactNode;
}) {
  const [activities, setActivities] = useState<GeneralHealthActivityEntry[]>(
    () => GENERAL_HEALTH_ACTIVITY.map((entry) => ({ ...entry })),
  );
...
export function useGeneralHealthActivity() {
  ...
  if (!context) {
    throw new Error(
      "useGeneralHealthActivity must be used within GeneralHealthActivityProvider",
    );
  }
  return context;
}
