...
export function HeartRateDetailScreen() {
  ...
  const allPoints = useMemo(
    () => getHeartRateReadingsForProfile(activities, selectedProfileId),
    [activities, selectedProfileId],
  );
  const points = useMemo(
    () => filterHeartRateReadingsByRange(allPoints, range, openedAt),
    [allPoints, openedAt, range],
  );
  const summary = useMemo(
    () => calculateHeartRateSavedSummary(points),
    [points],
  );
  const selectedPoint =
  ...
  return (
    <View style={[styles.screen, { backgroundColor: theme.background }]}>
      <HealthScreenContainer bottomSpacing={40} contentStyle={styles.content}>
        ...
        {SCREEN_STATE === "ready" && allPoints.length ? (
          <>
            ...
            <AppButton
              accessibilityLabel="Add heart rate reading"
              fullWidth
              label="Add heart rate reading"
      ...
      <GeneralHealthLogSheet
        key={logOpen ? "heart-rate-open" : "heart-rate-closed"}
...
function HeartRateLatestCard({ points }: { points: HeartRatePoint[] }) {
  ...
  return (
    ...
    >
      ...
      <Text
        accessibilityLabel={`Latest heart rate reading, ${latest.value} beats per minute, saved ${formatDateTime(latest.recordedAt)}`}
        style={[styles.latestValue, { color: theme.text }]}
      >
        {latest.value} <Text style={styles.latestUnit}>bpm</Text>
      </Text>
...
function HeartRateRangeSelector({
  range,
  setRange,
}: {
  range: HeartRateRange;
  setRange: (value: HeartRateRange) => void;
}) {
  const { theme } = useAppTheme();
  return (
    <View style={styles.rangeRow}>
      {RANGE_OPTIONS.map((option) => {
        ...
        return (
          <Pressable
            accessibilityLabel={`${option.label === "All" ? "All time" : `${option.value} days`}, ${active ? "selected" : "not selected"}`}
            accessibilityRole="button"
  ...
  return (
    <AppCard style={[styles.chartCard, { borderColor: theme.border }]}>
      ...
      >
        ...
        {plot.positions.map((point) => (
          <Pressable
            accessibilityLabel={`${point.value} beats per minute, ${formatFullDateTime(point.recordedAt)}. Added manually.`}
            accessibilityRole="button"
      ...
      >
        {xLabels.map((point) => (
          ...
          >
            {formatShortDate(point.recordedAt)}
          </Text>
        ))}
      </View>
      {points.length === 1 ? (
...
function SelectedPointCard({ point }: { point: HeartRatePoint }) {
  ...
  return (
    <View
      accessibilityLabel={`Selected reading, ${point.value} beats per minute, ${formatFullDateTime(point.recordedAt)}, added manually${point.activity.details.notes ? `, note: ${point.activity.details.notes}` : ""}`}
      accessible
      style={[
        styles.selected,
        { backgroundColor: theme.surface, borderColor: theme.border },
    ...
    >
      ...
      <Text style={[styles.selectedValue, { color: theme.text }]}>
        {point.value} <Text style={styles.selectedUnit}>bpm</Text>
      </Text>
...
function SavedRangeSummary({ summary }: { summary: HeartRateSummary }) {
  ...
  const metrics = [
    {
      accessibilityLabel: `Lowest saved reading, ${summary.lowest} beats per minute`,
      label: "Lowest saved",
      value: `${summary.lowest} bpm`,
    },
    {
      accessibilityLabel: `Highest saved reading, ${summary.highest} beats per minute`,
      label: "Highest saved",
      value: `${summary.highest} bpm`,
    },
    {
      accessibilityLabel: `Average saved reading, ${summary.average} beats per minute`,
      label: "Average saved",
      value: `${summary.average} bpm`,
    },
    {
...
function RecentHeartRateReadings({
  entries,
  onOpen,
}: {
  entries: HeartRatePoint[];
      ...
      <View style={styles.readingStack}>
        {entries.map((point) => (
          <Pressable
            ...
            accessibilityLabel={`${point.value} beats per minute, ${formatFullDateTime(point.recordedAt)}, added manually${point.activity.details.notes ? ", includes a note" : ""}`}
            accessibilityRole="button"
            key={point.id}
          ...
          >
            <View style={styles.readingCopy}>
              ...
              <Text style={[styles.readingMeta, { color: theme.mutedText }]}>
                {formatDateTime(point.recordedAt)} - Added manually
              </Text>
              {point.activity.details.notes ? (
                <Text style={[styles.noteIndicator, { color: theme.primary }]}>
                  Has note
                </Text>
              ) : null}
            </View>
...
function HeartRateLoading() {
  const { theme } = useAppTheme();
  return (
    <View style={styles.loading}>
      {[150, 260, 160, 90].map((height) => (
        <View
          key={height}
          style={[
            styles.skeleton,
...
function HeartRateState({
  ...
  return (
    <AppCard style={[styles.empty, { borderColor: theme.border }]}>
      <AppIcon color={theme.primary} container name="vitals" size={24} />
...
function getHeartRateReadingsForProfile(
  ...
  return entries
    .flatMap((entry) => {
      ...
      return entry.profileId === profileId &&
        entry.type === "vitals" &&
        entry.details.heartRate &&
        Number.isFinite(Number(entry.details.heartRate)) &&
        ...
        ? [
            {
              ...
              value: Number(entry.details.heartRate),
            },
          ]
        : [];
    })
...
function filterHeartRateReadingsByRange(
  points: HeartRatePoint[],
  range: HeartRateRange,
  reference: Date,
) {
...
function calculateHeartRateSavedSummary(
  points: HeartRatePoint[],
): HeartRateSummary {
...
const styles = StyleSheet.create({
  ...
  latestUnit: { fontSize: 20, fontWeight: "900" },
  latestValue: { fontSize: 42, fontWeight: "900", lineHeight: 48 },
  loading: { gap: 14 },
  noteIndicator: { fontSize: 12, fontWeight: "900", marginTop: 5 },
  ...
  selected: { borderRadius: 18, borderWidth: 1, gap: 5, padding: 15 },
  ...
  selectedUnit: { fontSize: 15 },
  selectedValue: { fontSize: 22, fontWeight: "900" },
  ...
  summaryLabel: { fontSize: 12, fontWeight: "800", lineHeight: 17 },
