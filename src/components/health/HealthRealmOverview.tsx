    ...
    <AppCard style={styles.hero}>
      ...
      <View style={styles.heroTop}>
        <View style={styles.heroCopy}>
          ...
          <Text style={styles.heroBody}>
            See what is logged, spot gentle trends, and choose one useful next
            step.
          </Text>
        </View>
        <View
          accessible
          accessibilityLabel={`${completedSignals} of 7 health areas have information`}
      ...
      <View style={styles.heroActions}>
        <HeroAction
          icon="add"
          label="Log health"
          onPress={() => router.push("/health/general" as Href)}
          primary
        />
        <HeroAction
...
function HeroAction({
  ...
  return (
    <Pressable
      ...
      onPress={onPress}
      style={[styles.heroAction, primary ? styles.heroActionPrimary : null]}
    >
      <AppIcon
        color={primary ? "#06231c" : "#f0fdfa"}
        decorative
        name={icon}
        size={17}
      />
      <Text
        style={[
          ...
          primary ? styles.heroActionTextPrimary : null,
        ]}
      >
        {label}
      </Text>
...
function VitalsGrid({
  ...
  const cards = [
    ...
    },
    {
      ...
      value: records?.recentVisits.length
        ? `${records.recentVisits.length} saved`
        : "No visits",
    },
  ];
  ...
  return (
    <AppSection
      subtitle="Tap a card to open its detail view."
        ...
        {cards.map((item) => (
          ...
          >
            <View style={styles.vitalIcon}>
              <AppIcon color={ACCENT} decorative name={item.icon} size={19} />
            </View>
            <Text style={[styles.vitalLabel, { color: theme.mutedText }]}>
              {item.label}
            </Text>
            <Text style={[styles.vitalValue, { color: theme.text }]}>
              {item.value}
            </Text>
            <Text style={[styles.vitalDetail, { color: theme.mutedText }]}>
              {item.detail}
            </Text>
          </Pressable>
        ))}
...
function TrendsSection() {
  return (
    <AppSection
      subtitle="Labeled previews from your saved readings."
      title="Trends"
    >
      <View style={styles.trendStack}>
        ...
        />
        <TrendCard
          bars={GENERAL_HEALTH_WEEK.map((item) => ({
            label: item.day,
            value: item.value,
          }))}
          label="Weight"
...
function TrendCard({
  ...
  return (
    <Pressable
      accessibilityLabel={`${label}, ${summary}, ${range}, measured in ${unit}`}
      accessibilityRole="button"
    ...
    >
      <AppCard style={[styles.trendCard, { borderColor: theme.border }]}>
        <View style={styles.trendHeader}>
          <View>
            <Text style={[styles.trendLabel, { color: theme.mutedText }]}>
              {label}
            </Text>
        ...
        <Text style={[styles.unitLabel, { color: theme.mutedText }]}>
...
function QuickLogSection() {
  return (
    <AppSection
      subtitle="Capture context now, then add detail when ready."
      title="Symptoms and notes"
    >
      <View style={styles.rowStack}>
        <ActionRow
          description="Record what you noticed and when it started."
          icon="health"
          label="Log a symptom"
          route="/health/general/notes"
        />
        <ActionRow
          description="Save a private note for your own reference."
          icon="edit"
          label="Add a health note"
          route="/health/general/notes"
        />
      </View>
    </AppSection>
  ...
  return (
    ...
    >
      <View style={styles.rowIcon}>
        <AppIcon color={ACCENT} decorative name={icon} size={20} />
      </View>
      <View style={styles.rowCopy}>
        <Text style={[styles.rowTitle, { color: theme.text }]}>{label}</Text>
        <Text style={[styles.rowDescription, { color: theme.mutedText }]}>
          {description}
        </Text>
      </View>
...
function RecordsShortcut({
  ...
  return (
    <AppSection
      subtitle="Documents, appointments, results and follow-ups."
          ...
          <View style={styles.recordsCopy}>
            ...
            <Text style={[styles.recordsBody, { color: theme.mutedText }]}>
              {recordCount || appointmentCount
                ? `${recordCount} recent records | ${appointmentCount} appointments`
                : "Add your first document or appointment to build a useful history."}
            </Text>
          </View>
...
function ConnectedRealms({ data }: { data: RealmData }) {
  return (
    <AppSection
      subtitle="A quick summary from the areas connected to Health."
    ...
    >
      <View style={styles.connectedGrid}>
        <ConnectedCard
          accent="#b45309"
          detail={
            data.nutrition?.foodLogCount
              ? `${data.nutrition.foodLogCount} meals | ${Math.round(data.nutrition.waterMl)} ml water`
              : "Log a meal or water next"
          }
          icon="nutrition"
          label="Nutrition"
          route="/food"
          value={
            data.nutrition?.calories
              ? `${Math.round(data.nutrition.calories)} kcal`
              : "No meals yet"
          }
        />
        <ConnectedCard
          ...
          detail={
            data.fitness?.workoutsThisWeek
              ? `${data.fitness.workoutsThisWeek} workouts this week`
              : "Start with a short movement"
          }
          icon="fitness"
          label="Fitness"
          route="/fitness"
          value={
            data.fitness?.activeMinutesToday
              ? `${data.fitness.activeMinutesToday} active min`
              : "No activity yet"
          }
        />
        <ConnectedCard
          accent="#7c3aed"
          detail={
            data.medication?.totalCount
              ? `${data.medication.takenCount} of ${data.medication.totalCount} taken today`
              : "Add medication only when needed"
          }
          icon="medication"
          label="Medication"
          route="/medication"
          value={
            data.medication?.dueCount
              ? `${data.medication.dueCount} due`
              : "Nothing due"
          }
        />
      </View>
...
function ConnectedCard({
  ...
  return (
    ...
    >
      <View style={[styles.connectedIcon, { backgroundColor: `${accent}18` }]}>
        <AppIcon color={accent} decorative name={icon} size={20} />
      </View>
      <Text style={[styles.connectedLabel, { color: theme.mutedText }]}>
        {label}
      </Text>
      <Text style={[styles.connectedValue, { color: theme.text }]}>
        {value}
      </Text>
      <Text style={[styles.connectedDetail, { color: theme.mutedText }]}>
        {detail}
      </Text>
    </Pressable>
  );
