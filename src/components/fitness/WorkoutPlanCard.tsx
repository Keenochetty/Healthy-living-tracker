...
export function WorkoutPlanCard({ onDelete, onEdit, onStart, plan }: WorkoutPlanCardProps) {
  ...
  return (
    <View style={{ backgroundColor: "#ffffff", borderRadius: 24, gap: 12, padding: 15 }}>
      <View style={{ alignItems: "center", flexDirection: "row", gap: 12 }}>
        ...
        <View style={{ flex: 1 }}>
          ...
          {targets.length ? (
            <Text style={{ color: "#64748b", marginTop: 3 }}>{targets.join(" - ")}</Text>
          ) : null}
        </View>
      </View>
...
function ActionButton({
  ...
  return (
    ...
    >
      <Text style={{ color: primary ? "#ffffff" : danger ? "#dc2626" : "#166534", fontWeight: "900" }}>
        {label}
      </Text>
    </TouchableOpacity>
  );
