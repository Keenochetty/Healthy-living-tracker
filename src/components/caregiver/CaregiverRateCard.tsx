...
export function CaregiverRateCard({
  caregiverId,
  currency,
  onChange,
}: CaregiverRateCardProps) {
    ...
    <AppCard>
      <View style={{ gap: 12 }}>
        ...
        <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}>
          {(Object.keys(CAREGIVER_RATE_TYPE_LABELS) as CaregiverRateType[]).map(
            (type) => (
              <CaregiverChip
                key={type}
                label={type}
                onPress={() => setRateType(type)}
                selected={rateType === type}
              />
            ),
          )}
        </View>
