...
export function EmptyRecordsState({ text }: { text: string }) {
  return (
    <View style={recordsSharedStyles.empty}>
      <RecordsText muted>{text}</RecordsText>
    </View>
  );
}

export function RecordsLink({ label, onPress }: { label: string; onPress?: () => void }) {
  const mode: HealthOSColorMode = useColorScheme() === "dark" ? "dark" : "light";
  const palette = getHealthOSPalette(mode);
  return (
    <Pressable accessibilityRole="button" onPress={onPress}>
      <Text style={[healthOSTypography.buttonLabel, { color: palette.records }]}>{label}</Text>
    </Pressable>
  );
}

export const recordsSharedStyles = StyleSheet.create({
  actions: {
    flexDirection: "row",
    flexWrap: "wrap",
