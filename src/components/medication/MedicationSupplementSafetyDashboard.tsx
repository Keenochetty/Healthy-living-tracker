...
export function MedicationSupplementSafetyDashboard({
  ...
  const [medicationMatch, setMedicationMatch] =
    useState<MedicationStandardMatch | null>(null);
  const [supplementMatch, setSupplementMatch] =
    useState<SupplementIngredientMatch | null>(null);
  ...
  const loadSafety = useCallback(async () => {
    ...
    setMatchQuery(
      itemType === "medication" ? item.name : getSupplementIngredientName(item),
    );
  }, [item, itemType]);
  ...
  async function saveManualMatch() {
    ...
    if (itemType === "medication") {
      ...
      await saveMedicationStandardMatch(
        item.id,
        candidate ?? {
          displayName: matchQuery,
          ingredientName: matchQuery,
          source: "manual" as const,
        },
      );
    } else {
      const [candidate] = await searchSupplementIngredients(matchQuery);
      await saveSupplementIngredientMatch(
        item.id,
        candidate ?? {
          displayName: matchQuery,
          ingredientName: matchQuery,
    ...
    setSavedMessage(
      "Match saved for review. Confirm it matches your label or professional guidance.",
    );
    await loadSafety();
    onChange?.();
  }
    ...
    <View style={{ gap: 12 }}>
      <AppCard backgroundColor="#fff7ed">
        ...
        <Text style={{ color: "#9a3412", lineHeight: 21, marginTop: 6 }}>
          Safety checks are for organization and awareness only. They are not
          medical advice and do not replace a doctor, pharmacist, or healthcare
          professional. Always follow your prescription label, product label,
      ...
      <Section title="Review Items">
        ...
        {!item ? (
          <EmptyText
            text={
              itemType === "medication"
                ? "Add a medication to review safety notes."
                : "Add a supplement to review safety notes."
            }
          />
        ) : null}
      </Section>
      ...
      <Section
        title={
          itemType === "medication" ? "Name Matching" : "Ingredient Matching"
        }
      >
        <Text style={{ color: "#64748b", lineHeight: 21 }}>
          {itemType === "medication"
        ...
        <TextInput
          ...
          placeholder={
            itemType === "medication" ? "Medication name" : "Main ingredient"
          }
        ...
        {medicationMatch ? (
          <MatchLine
            ...
            value={medicationMatch.displayName}
          />
        ) : null}
        {supplementMatch ? (
          <MatchLine
            label="Supplement match"
            value={supplementMatch.displayName}
          />
        ) : null}
      ...
      <Section title="Needs Confirmation">
        {notices.filter((notice) => notice.type === "confirm").length ? (
          notices
            .filter((notice) => notice.type === "confirm")
            .map((notice) => (
              <NoticeCard
                key={notice.id}
                notice={notice}
                onDismiss={() =>
                  dismissSafetyNotice(notice.id).then(loadSafety)
                }
              />
            ))
        ) : (
      ...
      <Section title="Food Timing Notes">
        <NoticeGroup
          ...
          notices={notices}
          onDismiss={loadSafety}
        />
      </Section>

      <Section title="Allergy Notes">
        <Text style={{ color: "#64748b", lineHeight: 21 }}>
          Check labels and ingredients if you have allergies or sensitivities.
        </Text>
        <TextInput
          onChangeText={setAllergyName}
          placeholder="Allergy or sensitivity note"
          placeholderTextColor="#94a3b8"
          style={INPUT_STYLE}
          value={allergyName}
        />
        ...
        {allergies.slice(0, 4).map((allergy) => (
          <Text key={allergy.id} style={{ color: "#64748b", lineHeight: 21 }}>
            {allergy.allergyName} - {allergy.allergyType}
          </Text>
        ))}
        <NoticeGroup
          category="allergy"
      ...
      <Section title="Data Source Status">
        <NoticeGroup
          category={itemType === "medication" ? "name_match" : "data_source"}
          notices={notices}
          onDismiss={loadSafety}
        />
      </Section>
      ...
      <Section title="Safety Checklist">
        {checklist.map((item) => (
          <TouchableOpacity
            ...
            key={item.id}
            onPress={() =>
              completeSafetyChecklistItem(item.id, !item.isCompleted).then(
                loadSafety,
              )
            }
            style={{
          ...
          >
            <Text
              style={{
                color: item.isCompleted ? "#047857" : "#475569",
                fontWeight: "900",
              }}
            >
      ...
      <Section title="Ask a Professional">
        <View style={{ flexDirection: "row", gap: 10 }}>
          <ActionButton
            label="Ask Pharmacist"
            onPress={() => askProfessional("pharmacist")}
          />
          <ActionButton
            label="Ask Doctor"
            onPress={() => askProfessional("doctor")}
          />
        </View>
        <SecondaryButton
          label="Export Medication/Supplement List"
          onPress={() =>
            setExportMessage("Export will be added in a later phase.")
          }
        />
        {savedMessage ? (
          <Text style={{ color: "#047857", lineHeight: 21 }}>
            {savedMessage}
          </Text>
        ) : null}
        {exportMessage ? (
      ...
      <Text style={{ color: "#64748b", fontSize: 12, lineHeight: 18 }}>
        If you experience severe symptoms or a medical emergency, contact local
    ...
    <View style={{ gap: 8 }}>
      {matchingNotices.map((notice) => (
        <NoticeCard
          key={notice.id}
          notice={notice}
          onDismiss={() => dismissSafetyNotice(notice.id).then(onDismiss)}
        />
      ))}
    </View>
  );
...
function NoticeCard({
  ...
  return (
    <View
      style={{
        backgroundColor: getNoticeBackground(notice.type),
        borderRadius: 16,
        padding: 12,
      }}
    >
...
function getSupplementIngredientName(item: Medication | Supplement) {
  return "mainIngredient" in item
    ? (item.mainIngredient ?? item.name)
    : item.name;
}
