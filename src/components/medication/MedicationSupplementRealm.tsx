...
export function MedicationSupplementRealm({
  itemType,
}: {
  itemType: ItemType;
}) {
  ...
  const [selectedReminder, setSelectedReminder] =
    useState<HealthScheduleReminder | null>(null);
  const [sheetMode, setSheetMode] = useState<SheetMode>(null);
  const [toast, setToast] = useState("");
  const [companionItems, setCompanionItems] = useState<
    Array<Medication | Supplement>
  >([]);
  const [companionReminders, setCompanionReminders] = useState<
    HealthScheduleReminder[]
  >([]);
  ...
  const loadRealm = useCallback(async () => {
    ...
    const schedules = (
      await Promise.all(
        items.map((item) => getSchedulesByItem(itemType, item.id)),
      )
    ).flat();
    const logs = (
      await Promise.all(
        items.map((item) => getDoseLogsByItem(itemType, item.id)),
      )
    ).flat();
    const documents = (
      await Promise.all(
        items.map((item) => getHealthDocumentsByItem(itemType, item.id)),
      )
    ).flat();
    ...
    setData({
      ...
      logs,
      notes,
      reminders: todaySummary.reminders,
      schedules,
    });
  ...
  return (
    <View style={styles.stack}>
      <ConnectedRealmOverview
        ...
        onAction={(reminder) => {
          if (reminder.itemType !== itemType) {
            router.push(
              `/${reminder.itemType === "medication" ? "medication" : "supplements"}` as Href,
            );
            return;
          }
        ...
        onAddOther={() =>
          router.push(
            `/${itemType === "medication" ? "supplements/add" : "medication/add"}` as Href,
          )
        }
        onOpenRefills={() =>
      ...
      {activeTab === "logs" ? (
        <LogsTab data={data} itemType={itemType} />
      ) : null}
      {activeTab === "refills" ? (
        <RefillsTab data={data} onSheet={setSheetMode} />
      ) : null}
      {activeTab === "ingredients" ? <IngredientsTab data={data} /> : null}
      {activeTab === "safety" ? (
        <SafetyTab item={selectedItem} itemType={itemType} />
      ) : null}
      {activeTab === "records" ? (
        <RecordsTab data={data} itemType={itemType} onSheet={setSheetMode} />
      ) : null}
      {activeTab === "reports" ? (
        <ReportsTab data={data} itemType={itemType} onTab={setActiveTab} />
      ) : null}
      {activeTab === "learn" ? <LearnTab itemType={itemType} /> : null}
      {activeTab === "settings" ? <SettingsTab itemType={itemType} /> : null}

      <Footer
        text={itemType === "medication" ? MEDICATION_FOOTER : SUPPLEMENT_FOOTER}
      />
      ...
      <DocumentSheet
        ...
        onClose={() => setSheetMode(null)}
        onSaved={afterSaved}
        visible={sheetMode === "document"}
      />
    </View>
  );
}

export function MedicationSupplementDetail({
  itemId,
  itemType,
}: {
  itemId: string;
  ...
  const loadDetail = useCallback(async () => {
    const nextItem =
      itemType === "medication"
        ? await getMedicationById(itemId)
        : await getSupplementById(itemId);
    const [nextSchedules, nextDocuments, nextNotes, nextLogs, nextAdherence] =
      await Promise.all([
        ...
        itemType === "medication"
          ? getMedicationAdherenceSummary(itemId)
          : getSupplementAdherenceSummary(itemId),
      ]);
  ...
  return (
    <View style={styles.stack}>
      <AppCard
        style={[styles.heroCard, { borderColor: getAccentColor(itemType) }]}
      >
        ...
        <Text style={styles.heroBody}>
          {itemType === "medication"
            ? "Dose as written on your label or by your healthcare professional."
            : "Serving as written on your label or by your healthcare professional."}
        </Text>
      </AppCard>
      <View style={styles.metricGrid}>
        <MetricCard label="Form" value={formatValue(item.form)} />
        <MetricCard
          label="Strength / serving"
          value={item.strength ?? getServingText(item)}
        />
      ...
      <List
        items={notes
          .slice(0, 5)
          .map((note) => `${formatDate(note.loggedAt)} - ${note.note}`)}
      ...
      <AppButton
        onPress={async () => {
          ...
          router.replace(
            `/${itemType === "medication" ? "medication" : "supplements"}` as Href,
          );
        }}
        title="Pause / Archive"
      />
    ...
    <View style={styles.realmOverview}>
      ...
      >
        {allReminders.length ? (
          <View style={styles.timeline}>
            {allReminders.slice(0, 6).map((reminder, index) => (
              <Pressable
                ...
                key={`${reminder.scheduleId ?? index}-${reminder.itemId}-${reminder.scheduledAt}`}
                onPress={() => onAction(reminder)}
                style={styles.softDoseCard}
              >
                <View style={styles.doseTimePill}>
          ...
          <PremiumEmptyState
            message="Add a schedule when you want gentle dose reminders."
      ...
      <AppSection
        actionLabel="Manage"
        onActionPress={onOpenRefills}
        subtitle="Keep labels, prescription notes, and schedules easy to review."
        title="Refills and reminders"
      >
        <AppCard style={styles.softReminderCard}>
          <View style={styles.softReminderIcon}>
            <AppIcon color="#6d28d9" decorative name="reminder" size={22} />
          </View>
          <View style={{ flex: 1 }}>
            <Text style={styles.softCardTitle}>
              {data.schedules.length} active schedule
              {data.schedules.length === 1 ? "" : "s"}
            </Text>
            <Text style={styles.softCardBody}>
              {data.documents.length
                ? `${data.documents.length} linked label or prescription record${data.documents.length === 1 ? "" : "s"}.`
                : "Add labels or prescription notes when useful."}
            </Text>
          </View>
        </AppCard>
      </AppSection>
      ...
      <AppSection
        subtitle="Informational notes only. Always confirm important guidance."
        title="Food and exercise interactions"
      >
        <AppCard style={styles.interactionCard}>
          <AppIcon color="#b45309" decorative name="warning" size={21} />
          <Text style={styles.interactionText}>
            Some medicines or supplements may have food, activity, or timing
            instructions. Follow the label and check with a pharmacist or
            healthcare professional if unsure.
          </Text>
        </AppCard>
      </AppSection>
      ...
      <View style={styles.addActionRow}>
        <Pressable
          accessibilityRole="button"
          onPress={itemType === "medication" ? onAddCurrent : onAddOther}
        ...
        <Pressable
          accessibilityRole="button"
          onPress={itemType === "supplement" ? onAddCurrent : onAddOther}
          style={styles.addAction}
        >
...
function ConnectedItemLists({
  medications,
  supplements,
}: {
  medications: Array<Medication | Supplement>;
  supplements: Array<Medication | Supplement>;
}) {
  return (
    <View style={styles.connectedLists}>
      <AppSection
        subtitle="Prescription and over-the-counter items you entered."
        title="Medications"
      >
        <View style={styles.softItemList}>
          {medications.length ? (
            medications
              .slice(0, 3)
              .map((item) => (
                <SoftItemCard item={item} itemType="medication" key={item.id} />
              ))
          ) : (
            <SoftEmptyCard text="No medications added yet." />
          )}
        </View>
      </AppSection>
      <AppSection
        subtitle="Supplements remain visually separate but connected."
        title="Supplements"
      >
        {supplements.length ? (
          <ScrollView
            horizontal
            showsHorizontalScrollIndicator={false}
            contentContainerStyle={styles.supplementChipRow}
          >
            {supplements.slice(0, 6).map((item) => (
              <View key={item.id} style={styles.supplementChip}>
                <Text style={styles.supplementChipText}>{item.name}</Text>
              </View>
            ))}
          </ScrollView>
        ) : (
...
function SoftItemCard({
  ...
  return (
    <AppCard style={styles.softItemCard}>
      <View style={styles.softItemIcon}>
        <AppIcon
          color="#6d28d9"
          decorative
          name={itemType === "medication" ? "medication" : "nutrition"}
          size={20}
        />
      </View>
      <View style={{ flex: 1 }}>
        ...
        <Text style={styles.softCardBody}>
          {item.strength ?? formatValue(item.form)} |{" "}
          {getSafetyStatusLabel(item.safetyStatus)}
        </Text>
      </View>
      <PrivacyBadge />
    </AppCard>
...
function SoftEmptyCard({ text }: { text: string }) {
  return (
    <AppCard style={styles.softEmptyCard}>
      <Text style={styles.softCardBody}>{text}</Text>
    </AppCard>
  );
}
...
function NextCard({
  ...
  return (
    <AppCard style={styles.darkCard}>
      <View style={styles.sectionHeaderRow}>
        <View>
          <Text style={styles.darkTitle}>
            {itemType === "medication" ? "Next dose" : "Next supplement"}
          </Text>
          <Text style={styles.darkMuted}>
            {reminder
              ? `${reminder.itemName} · ${formatReminderStatus(reminder)}`
              : "No reminder due right now."}
          </Text>
        </View>
        <PrivacyBadge />
      </View>
      {reminder ? (
        <View style={styles.actionRow}>
          <GhostButton label="Action" onPress={() => onAction(reminder)} />
        </View>
      ) : null}
    </AppCard>
  );
...
function QuickActions({
  ...
  const actions = [
    ...
    {
      icon: "add",
      label: itemType === "medication" ? "Add Medication" : "Add Supplement",
      mode: "item" as SheetMode,
    },
    {
      icon: "records",
      label:
        itemType === "medication" ? "Add Label / Prescription" : "Add Label",
      mode: "document" as SheetMode,
    },
  ];
  return (
...
function ScheduleTab({
  ...
  return (
    <View style={styles.stack}>
      <AppSection
        title="Schedule"
        subtitle="Gentle reminder timeline with quick actions."
      />
      <View style={styles.actionRow}>
        <AppButton onPress={() => onSheet("schedule")} title="Add reminder" />
      </View>
      <ScheduleTimeline
        itemType={itemType}
        onAction={onAction}
...
function ItemsTab({
  ...
  return (
    <View style={styles.stack}>
      <AppSection
        title={itemType === "medication" ? "Medications" : "Supplements"}
        subtitle="Private item list with schedules, status, labels, and detail links."
      />
      {!data.items.length ? (
        <PremiumEmptyState
          button={
          ...
          message={
            itemType === "medication"
              ? "Add medication reminders, labels, prescriptions, and notes when you are ready."
              : "Track supplements, reminders, labels, and review notes."
          }
      ...
      {data.items.map((item) => (
        <ItemCard
          documents={data.documents.filter(
            (document) => document.relatedId === item.id,
          )}
          item={item}
          itemType={itemType}
          key={item.id}
          onPress={() => {
            onSelect(item.id);
            router.push(
              `/${itemType === "medication" ? "medication" : "supplements"}/${item.id}` as Href,
            );
          }}
          schedule={data.schedules.find(
            (schedule) => schedule.itemId === item.id,
          )}
...
function LogsTab({ data, itemType }: { data: RealmData; itemType: ItemType }) {
  return (
    <View style={styles.stack}>
      <AppSection
        title={itemType === "medication" ? "Dose Logs" : "Supplement Logs"}
        subtitle="Taken, skipped, missed, and snoozed history."
      />
      <View style={styles.metricGrid}>
        <MetricCard
          label="Taken"
          value={`${data.logs.filter((log) => log.status === "taken").length}`}
        />
        <MetricCard
          label="Skipped"
          value={`${data.logs.filter((log) => log.status === "skipped").length}`}
        />
        <MetricCard
          label="Snoozed"
          value={`${data.logs.filter((log) => log.status === "snoozed").length}`}
        />
        <MetricCard
          label="Missed"
          value={`${data.logs.filter((log) => log.status === "missed").length}`}
        />
      </View>
      <List
        items={data.logs
          ...
          .map(
            (log) =>
              `${formatDate(log.takenAt ?? log.scheduledAt ?? log.createdAt)} · ${formatValue(log.status)}${log.notes ? ` · ${log.notes}` : ""}`,
          )}
        empty="Logs will appear here."
      />
    </View>
  );
}
...
function RefillsTab({
  ...
  return (
    <View style={styles.stack}>
      <AppSection
        title="Refills"
        subtitle="Prescription, label, pharmacy, and renewal reminders."
      />
      <RefillPreview data={data} onSheet={onSheet} />
      <PremiumEmptyState
        button="Add document"
        message="Add a prescription, pharmacy note, or label for reference."
        onPress={() => onSheet("document")}
        title="No refill dates entered"
      />
    </View>
  );
}
...
function IngredientsTab({ data }: { data: RealmData }) {
  return (
    <View style={styles.stack}>
      <AppSection
        title="Ingredients"
        subtitle="Ingredient notes are user-entered and for review only."
      />
      <IngredientsPreview data={data} />
...
function SafetyTab({
  ...
  return (
    <View style={styles.stack}>
      <AppSection
        title="Safety Review"
        subtitle="Cautious prompts only. No safe or unsafe decisions."
      />
      <SafetyReviewCards itemType={itemType} item={item ?? undefined} />
      <MedicationSupplementSafetyDashboard
        item={item}
        itemType={itemType}
        onChange={() => undefined}
      />
  ...
  return (
    <View style={styles.stack}>
      <AppSection
        ...
        subtitle={
          itemType === "medication"
            ? "Labels, prescriptions, pharmacy notes, and documents."
            : "Labels, photos, supplement notes, and documents."
        }
      />
      <DocumentPreview
        documents={data.documents}
      ...
      <View style={styles.recordGrid}>
        {[
          ...
          <RecordTypeCard key={label} label={label} />
        ))}
      </View>
...
function ReportsTab({
  ...
  if (!data.logs.length) {
    return (
      <PremiumEmptyState
        ...
        message="Mark items taken, skipped, or snoozed to see trends."
        onPress={() => onTab("schedule")}
  ...
  return (
    <View style={styles.stack}>
      <AppSection
        title="Reports"
        subtitle="Adherence and reminder history only. No health outcome interpretation."
      />
      <ReportsSummary
        adherence={data.adherence}
        itemType={itemType}
      ...
      <ChartCard
        ...
        values={["taken", "skipped", "missed", "snoozed"].map(
          (status) =>
            data.logs.filter((log) => log.status === status).length /
            Math.max(1, data.logs.length),
        )}
      />
      <ChartCard
        ...
        values={data.logs
          .slice(0, 8)
          .map((log) =>
            log.status === "taken" ? 1 : log.status === "skipped" ? 0.5 : 0.25,
          )}
      />
      <SafetyReviewCards itemType={itemType} />
    </View>
  ...
  const cards =
    itemType === "medication"
      ? [
          [
            "Medication tracking",
            "Use reminders and logs to organize what you take. Follow your prescription label or healthcare professional instructions.",
            "App education",
          ],
          [
            "Reading product labels",
            "Save labels and questions so you can discuss details with a pharmacist, doctor, nurse, clinic, or healthcare professional.",
            "App education",
          ],
          [
            "Pharmacy questions",
            "Prepare questions about timing, labels, refills, and what to confirm professionally.",
            "App education",
          ],
        ]
      : [
          [
            "Supplement caution",
            "Supplement tracking is for organization only. Discuss supplement use if you take medication, are pregnant, or are unsure.",
            "App education",
          ],
          [
            "Duplicate ingredients",
            "Ingredient lists may overlap. Review labels with a healthcare professional if concerned.",
            "App education",
          ],
          [
            "Label notes",
            "Save supplement labels and serving text as written. This app does not claim benefits.",
            "App education",
          ],
        ];
  return (
    <View style={styles.stack}>
      <AppSection
        title="Learn"
        subtitle="Short source-ready cards with safe wording."
      />
      {cards.map(([title, summary, source]) => (
        <LearnCard
          key={title}
          source={source}
          summary={summary}
    ...
    <View style={styles.stack}>
      ...
      <AppCard style={styles.privacyCard}>
        <Text style={styles.privacyTitle}>
          {itemType === "medication" ? "Medication" : "Supplement"} privacy
        </Text>
        <Text style={styles.privacyBody}>
          Items, schedules, logs, records, and notes stay private unless you
          explicitly share selected details.
        </Text>
      </AppCard>
    </View>
  );
...
function ScheduleTimeline({
  ...
  if (!reminders.length) {
    return (
      <PremiumEmptyState
        message="Add a schedule if you want reminders."
...
function ReminderCard({
  ...
  return (
    <Pressable
      accessibilityLabel={`${reminder.itemName}. ${formatReminderStatus(reminder)}`}
      accessibilityRole="button"
      onPress={() => onAction(reminder)}
    ...
    >
      <View
        style={[
          styles.timelineIcon,
          { backgroundColor: `${getAccentColor(itemType)}22` },
        ]}
      >
        <AppIcon
          color={getAccentColor(itemType)}
...
function ItemCard({
  ...
  return (
    <Pressable
      ...
      style={[
        styles.itemCard,
        selected ? { borderColor: getAccentColor(itemType) } : null,
      ]}
    >
      <View style={styles.sectionHeaderRow}>
        <View style={{ flex: 1 }}>
          ...
          <Text style={styles.itemMeta}>
            {formatValue(item.form)} ·{" "}
            {schedule ? formatTiming(schedule.timing) : "No schedule set"}
          </Text>
          {itemType === "supplement" && supplement.mainIngredient ? (
            <Text style={styles.itemMeta}>
              Ingredient: {supplement.mainIngredient}
            </Text>
          ) : null}
        </View>
...
function SafetyReviewCards({
  ...
  return (
    <View style={styles.stack}>
      {cards.map((title) => (
        <CautionCard key={title} text={title} />
      ))}
      {item?.notes ? (
        <CautionCard text="Item notes are user-entered. Confirm important timing or review questions with a healthcare professional." />
      ) : null}
      {itemType === "supplement" ? (
        <CautionCard text="Ingredient lists may overlap. Review labels with a healthcare professional if concerned." />
      ) : null}
    </View>
  );
}
    ...
    <AppCard style={styles.darkCard}>
      <View style={styles.sectionHeaderRow}>
        <View>
          ...
          <Text style={styles.darkMuted}>
            Save labels, prescriptions, pharmacy notes, and renewal reminders.
          </Text>
        </View>
...
function IngredientsPreview({ data }: { data: RealmData }) {
  const ingredients = data.items
    .map((item) => (item as Supplement).mainIngredient)
    .filter(Boolean);
  return (
    <AppCard style={styles.darkCard}>
      ...
      <Text style={styles.darkMuted}>
        {ingredients.length
          ? ingredients.join(", ")
          : "Add ingredients from the label when you are ready."}
      </Text>
        ...
        <View>
          ...
          <Text style={styles.darkMuted}>
            {documents.length
              ...
              : "No labels or prescriptions linked."}
          </Text>
        </View>
        {onSheet ? (
          <GhostButton
      ...
      <View style={styles.recordGrid}>
        {(documents.length
          ...
          : [
              {
                id: "placeholder",
                title: "Add a photo, prescription, or note for reference.",
              } as HealthDocument,
            ]
        ).map((document) => (
  ...
  return (
    <AppCard style={styles.darkCard}>
      ...
      <View style={styles.metricGrid}>
        ...
        <MetricCard
          label="Total logs"
          value={`${logs.length || adherence.total}`}
        />
      </View>
    </AppCard>
  );
...
function ItemSheet({
  ...
  async function save() {
    ...
    await onSaved(
      itemType === "medication" ? "Medication added" : "Supplement added",
    );
  }
  ...
  return (
    <QuickLogBottomSheet
      onClose={onClose}
      title={itemType === "medication" ? "Add medication" : "Add supplement"}
      visible={visible}
    >
      <TextInput
      ...
      <TextInput
        ...
        placeholder={
          itemType === "medication"
            ? "Dose as written on your label or by your healthcare professional"
            : "Serving text as written on the label"
        }
        placeholderTextColor="#94a3b8"
        style={styles.darkInput}
        value={strength}
...
function ScheduleSheet({
  ...
  onSaved,
  visible,
}: SheetProps & { item: Medication | Supplement | null; itemType: ItemType }) {
  ...
  return (
    ...
    >
      {!item ? (
        <Text style={styles.sheetText}>Add an item first.</Text>
      ) : (
        <Text style={styles.sheetText}>Reminder for {item.name}</Text>
      )}
      <ChipRow
        options={SCHEDULE_TIMING_OPTIONS.map((option) => ({
          key: option.key,
          label: option.label,
        }))}
        selected={timing}
        onSelect={(value) => setTiming(value as ScheduleTiming)}
        tint={getAccentColor(itemType)}
      />
      <ManualEntryToggle
        enabled={manual}
      ...
      {manual ? (
        ...
        <TimeWheelPicker
          onChange={(minutes) => setTime(minutesToTime(minutes))}
          valueMinutes={timeToMinutes(time)}
        />
      )}
      <ChipRow
        options={FOOD_TIMING_OPTIONS.map((option) => ({
          key: option.key,
          label: option.label,
        }))}
        selected={foodTiming}
        onSelect={(value) => setFoodTiming(value as FoodTiming)}
        tint={getAccentColor(itemType)}
      />
      <QuickNoteField
...
function ActionSheet({
  ...
  itemType: ItemType;
  reminder: HealthScheduleReminder | null;
}) {
  const [statusAction, setStatusAction] = useState<
    "taken" | "skipped" | "snoozed" | "note"
  >("taken");
  ...
  async function save() {
    ...
    if (statusAction === "snoozed")
      await snoozeDoseReminder({
        ...input,
        notes: [notes, `Snoozed for ${snoozeMinutes} minutes`]
          .filter(Boolean)
          .join(". "),
      });
    if (statusAction === "note")
      await createMedicationSupplementNote({
        loggedAt: new Date().toISOString(),
        note: notes || "Reminder note",
        relatedId: reminder.itemId,
        relatedType: itemType,
      });
    await onSaved(
      statusAction === "snoozed"
        ? "Reminder snoozed"
  ...
  return (
    <QuickLogBottomSheet
      onClose={onClose}
      title={itemType === "medication" ? "Dose action" : "Supplement action"}
      visible={visible}
    >
      {reminder ? (
        ...
        <Text style={styles.sheetText}>Select a reminder first.</Text>
      )}
      <ChipRow
        options={["taken", "skipped", "snoozed", "note"].map((item) => ({
          key: item,
          label: item === "note" ? "Add Note" : formatValue(item),
        }))}
        selected={statusAction}
        onSelect={(value) => setStatusAction(value as typeof statusAction)}
        tint={getAccentColor(itemType)}
      />
      {statusAction === "taken" ? (
        <TimeWheelPicker
      ...
      {statusAction === "snoozed" ? (
        <ChipRow
          options={SNOOZE_OPTIONS.map((option) => ({
            key: String(option.minutes),
            label: option.label,
          }))}
          selected={String(snoozeMinutes)}
          onSelect={(value) => setSnoozeMinutes(Number(value))}
          tint={getAccentColor(itemType)}
        />
      ) : null}
      <QuickNoteField onChangeText={setNotes} value={notes} />
...
function NoteSheet({
  ...
  async function save() {
    await createMedicationSupplementNote({
      ...
      note,
      relatedId: item?.id,
      relatedType: item?.id ? itemType : "general",
    });
...
function DocumentSheet({
  ...
  const [title, setTitle] = useState(
    itemType === "medication" ? "Prescription label" : "Supplement label",
  );
...
function Chip({
  ...
  return (
    <Pressable
      ...
      style={[
        styles.chip,
        selected ? { backgroundColor: tint, borderColor: tint } : null,
      ]}
    >
      <Text
...
function ChartCard({ title, values }: { title: string; values: number[] }) {
  ...
  return (
    <AppCard style={styles.darkCard}>
      ...
      <Text style={styles.darkMuted}>
        Chart summary is based on marked reminders only.
      </Text>
    </AppCard>
  );
}
  ...
  return (
    <AppCard style={styles.learnCard}>
      ...
      <Text style={styles.learnBody}>
        Education and organization only. Speak to a healthcare professional if
        unsure.
      </Text>
    </AppCard>
  );
...
function PrivacyBadge() {
  return (
    <View
      accessibilityLabel="Privacy status: private by default"
...
function formatFoodTiming(foodTiming: FoodTiming) {
  return foodTiming === "none"
    ? "No food timing note"
    : formatValue(foodTiming);
}
...
function timeToMinutes(value: string) {
  ...
  return (
    (Number.isFinite(hours) ? hours : 8) * 60 +
...
function getServingText(item: Medication | Supplement) {
  const supplement = item as Supplement;
  if (supplement.servingAmount && supplement.servingUnit)
    return `${supplement.servingAmount} ${supplement.servingUnit}`;
